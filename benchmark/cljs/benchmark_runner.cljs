...
(simple-benchmark [arr (to-array (range 1000000))]
                  (reset! x (areduce arr i ret 0 (+ ret (aget arr i))))
                  1)

(println ";; amap")
(simple-benchmark [arr (to-array (range 1000000))]
                  (amap arr i ret (* 10 (aget arr i)))
                  1)

(println ";; js-keys")
(simple-benchmark [obj (js-obj "a" 1 "b" 2) f js-keys] (f obj) 400000)
(simple-benchmark [obj (js-obj "a" 1 "b" 2 "c" 3 "d" 4 "e" 5 "f" 6) f js-keys]
                  (f obj)
                  400000)
...
(simple-benchmark [coll (specify [1 2 3] IFoo (foo [_] :bar))]
                  (foo coll)
                  1000000)
(println)
...
(simple-benchmark [[a b c] (take 3 (repeatedly #(rand-int 10)))]
                  (-count [a b c])
                  1000000)
(simple-benchmark [[a b c] (take 3 (repeatedly #(rand-int 10)))]
                  (-count (vec #js [a b c]))
...
(simple-benchmark [coll (reduce conj [] (range (+ 32768 32)))]
                  (conj coll :foo)
                  100000)
(simple-benchmark [coll (reduce conj [] (range 40000))]
                  (assoc coll 123 :foo)
                  100000)
(simple-benchmark [coll (reduce conj [] (range (+ 32768 33)))]
                  (pop coll)
                  100000)
(println)
...
(time (let [v (transient [])]
        (loop [i 0
               v v]
          (if (> i 1000000) (persistent! v) (recur (inc i) (conj! v i))))))

(println ";;; vector equality")
(simple-benchmark [a (into [] (range 1000000)) b (into [] (range 1000000))]
                  (= a b)
                  1)
(println)

(println ";;; keyword compare")
(let [seed ["amelia" "olivia" "jessica" "emily" "lily" "ava" "isla" "sophie"
            "mia" "isabella" "evie" "poppy" "ruby" "grace" "sophia" "chloe"
            "freya" "isabelle" "ella" "charlotte" "scarlett" "daisy" "lola"
            "holly" "eva" "lucy" "millie" "phoebe" "layla" "maisie" "sienna"
            "alice" "florence" "lilly" "ellie" "erin" "elizabeth" "imogen"
            "summer" "molly" "hannah" "sofia" "abigail" "jasmine" "matilda"
            "megan" "rosie" "lexi" "lacey" "emma" "amelie" "maya" "gracie"
            "emilia" "georgia" "hollie" "evelyn" "eliza" "amber" "eleanor"
            "bella" "amy" "brooke" "leah" "esme" "harriet" "anna" "katie" "zara"
            "willow" "elsie" "annabelle" "bethany" "faith" "madison" "isabel"
            "rose" "julia" "martha" "maryam" "paige" "heidi" "maddison" "niamh"
            "skye" "aisha" "mollie" "ivy" "francesca" "darcey" "maria" "zoe"
            "keira" "sarah" "tilly" "isobel" "violet" "lydia" "sara" "caitlin"]]
  (simple-benchmark [arr
                     (into-array (repeatedly 10000 #(keyword (rand-nth seed))))]
                    (.sort arr compare)
                    100)
  (simple-benchmark
    [arr
     (into-array (repeatedly 10000 #(keyword (rand-nth seed) (rand-nth seed))))]
    (.sort arr compare)
    100))
(println)
...
(simple-benchmark [f (fn [a b c d e f g h i j & more])]
                  (apply f (range 32))
                  1000000)
(println)
...
(simple-benchmark [coll (array-map :foo 1) ks [:foo]]
                  (update-in coll ks inc)
                  1000000)
(println)
...
(simple-benchmark [coll (obj-map :foo :bar :baz :woz)]
                  (-lookup coll :baz)
                  1000000)
(simple-benchmark [coll (obj-map :foo :bar :baz :woz :lol :rofl)]
                  (-lookup coll :lol)
                  1000000)
(println)
...
(simple-benchmark [coll (array-map :foo :bar :baz :woz)]
                  (-lookup coll :baz)
                  1000000)
(simple-benchmark [coll (array-map :foo :bar :baz :woz :lol :rofl)]
                  (-lookup coll :lol)
                  1000000)
(println)
...
(simple-benchmark
  [coll {:foo 1, :bar 2}]
  (loop [i 0 m coll] (if (< i 100000) (recur (inc i) (assoc m :foo 2)) m))
  1)
...
(simple-benchmark
  [coll (Foo. 1 2)]
  (loop [i 0 m coll] (if (< i 1000000) (recur (inc i) (assoc m :bar 2)) m))
  1)
(println)

(println ";;; zipmap")
(simple-benchmark [m {:a 1, :b 2, :c 3}]
...
(def pmap
  (into cljs.core.PersistentHashMap.EMPTY
        [[:a 0] [:b 1] [:c 2] [:d 3] [:e 4] [:f 5] [:g 6] [:h 7] [:i 8] [:j 9]
         ...
         [:t 19] [:u 20] [:v 21] [:w 22] [:x 23] [:y 24] [:z 25] [:a0 26]
         [:b0 27] [:c0 28] [:d0 29] [:e0 30] [:f0 31]]))
...
(def hash-coll-test
  (loop [i 0 r []] (if (< i 1000) (recur (inc i) (conj r (str "foo" i))) r)))
(simple-benchmark [coll hash-coll-test] (hash-coll coll) 100)
(simple-benchmark [coll hash-coll-test] (hash-ordered-coll coll) 100)
(def hash-imap-test
  (loop [i 0
         r {}]
    (if (< i 1000) (recur (inc i) (conj r [(keyword (str "foo" i)) i])) r)))
(def hash-imap-int-test
  (loop [i 0 r {}] (if (< i 1000) (recur (inc i) (conj r [i i])) r)))
...
(simple-benchmark [coll pmap]
                  (-lookup ^not-native hash-imap-test :foo500 nil)
                  1000000)
(simple-benchmark [coll pmap]
                  (-lookup ^not-native hash-imap-int-test 500 nil)
                  1000000)
(simple-benchmark [coll pmap] (assoc coll :g0 32) 1000000)
(simple-benchmark
  [coll pmap]
  (loop [i 0 m coll] (if (< i 1000000) (recur (inc i) (assoc m :a 1)) m))
  1)
(simple-benchmark [coll cljs.core.PersistentHashMap.EMPTY]
                  (assoc coll :f0 1)
                  1000000)

(println)
(print "transient map, conj! 100000 items")
(time (let [m (transient cljs.core.PersistentHashMap.EMPTY)]
        (loop [i 0
               m m]
          (if (> i 100000) (persistent! m) (recur (inc i) (assoc! m i i))))))
...
(println ";;; reader")
(def strings (take 10 (iterate (fn [s] (str s "string")) "string")))
...
(defn ints-seq
  ([n] (ints-seq 0 n))
  ([i n] (when (< i n) (lazy-seq (cons i (ints-seq (inc i) n))))))
...
(simple-benchmark [s (seq ipmap)]
                  (let [iter (seq-iter s)]
                    (loop [v nil] (if (.hasNext iter) (recur (.next iter)) v)))
                  1000)
...
(simple-benchmark [a (Box. 0) xs (range 512)]
                  (doseq [x xs y xs] (set! a -val (+ (.-val a) x)))
                  4)
(simple-benchmark [a (Box. 0) xs (vec (range 512))]
                  (doseq [x xs y xs] (set! a -val (+ (.-val a) x)))
                  4)
(println)

(println ";; reducers")
(simple-benchmark [xs (into [] (range 1000000))]
                  (r/reduce + (r/map inc (r/map inc (r/map inc xs))))
                  1)

(println ";; transducers")
(simple-benchmark [xs (into [] (range 1000000))]
                  (transduce (comp (map inc) (map inc) (map inc)) + 0 xs)
                  1)
...
(simple-benchmark [xs (range 1000000)]
                  (reduce + 0 (map inc (map inc (map inc xs))))
                  1)
...
(simple-benchmark [v (into [] (range 1000000))]
                  (loop [[x & xs] v] (if-not (nil? xs) (recur xs) x))
                  10)
