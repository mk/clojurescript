...
(defn weights
  ...
  [nodes child-fn]
  (reduce (fn [m n] (assoc m n (weight n child-fn))) {} nodes))
...
(defn layout
  ...
  ([nodes weight-fn child-fn] (layout nodes weight-fn child-fn 1 0 360 #{}))
  ([nodes weight-fn child-fn radius a1 a2 seen]
   (let [slice (- a2 a1)
         total-weight
           (reduce +
             (map #(or (weight-fn %) (throw (str "No weight for " %))) nodes))
...
(defn polar->cartesian
  ...
  [polar-map max-rings]
  (reduce (fn [m [k {:keys [radius angle]}]]
            (let [r (/ radius (+ 0.5 max-rings) 2)]
              (assoc m
                k {:x (+ 0.5 (math/angleDx angle r)),
