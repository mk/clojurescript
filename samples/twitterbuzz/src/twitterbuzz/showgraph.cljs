...
; fail whale
;(def default-avatar
;"http://farm3.static.flickr.com/2562/4140195522_e207b97280_s.jpg")
; google+ silhouette
(def default-avatar
  "http://ssl.gstatic.com/s2/profiles/images/silhouette48.png")
(defn debug [_])
...
(defn unit-to-pixel
  [unit-arg canvas-size]
  (+ (* unit-arg (- canvas-size avatar-size)) (/ avatar-size 2)))

(defn log [& args] (js/console.log (apply pr-str args)))

(def avatar-hover
  (doto (goog.ui/HoverCard. (js-obj)) ; svg IMAGE tags don't work here
    (.setElement (dom/get-element :avatar-hover))))

(defn hide-tooltip [event] (.setVisible avatar-hover false))

(defn attach-tooltip
  [img canvas-offset px py tweet]
...
(defn draw-graph
  ...
  (let [canvas-size (. g (getPixelSize))
    ...
    (let [text (if (empty? locs) "No locations to graph" text)]
      (when text
        (.drawTextOnLine g
                         text
                         5
...
(buzz/register
  :graph-update
  (fn [data] (reset! graph-data data) (draw-graph (layout/radial data) nil)))

(events/listen (dom/get-element :network)
