...
(defn append
  ...
  [parent & children]
  (do (doseq [child children] (dom/appendChild parent child)) parent))
...
(defn set-text
  ...
  [e s]
  (let [e (if (keyword? e) (get-element e) e)] (doto e (dom/setTextContent s))))
...
(defn element
  ...
  (let [[tag attrs children] (normalize-args tag args)
        parent (dom/createDom
                 (name tag)
                 (reduce (fn [o [k v]] (aset o k v))
                   (js-obj)
...
(defn remove-children
  ...
  [id]
  (let [parent (dom/getElement (name id))] (do (dom/removeChildren parent))))
...
(defn- element-arg? [x] (or (keyword? x) (map? x) (string? x)))
