...
(defn random-loc [] {:x (ann/random), :y (ann/random)})
...
(defn roots
  ...
  (let [parents (reduce (fn [ret [k {:keys [mentions]}]]
                          (if (pos? (count mentions)) (conj ret k) ret))
                  #{}
                  mentions-data)]
...
(defn radial
  ...
  (let [mentions #(rad/get-mentions mentions-data %)
        weights (rad/weights (into (set (roots mentions-data))
                                   (mapcat mentions (keys mentions-data)))
                             mentions)]
    {:mentions mentions-data,
...
(defn permute-swap
  ...
  (let [xys (vec (vals locs))
    ...
    {:locs (zipmap (keys locs) xys), :mentions mentions}))
  ...
  (let [adj #(min 1.0 (max 0 (+ % (- (* (ann/random) 0.1) 0.05))))
        move (fn [{:keys [x y], :as loc}]
               (if true ;;(> (ann/random) 0.8)
                 ...
                 loc))
        xys (vec (vals locs))]
    {:locs (zipmap (keys locs) (map move (vals locs))), :mentions mentions}))

(comment (def test-data {})
