...
#_(deftest should-recompile
    (let [src (File. "test/hello.cljs")
      ...
      (with-redefs [util/*clojurescript-version*
                      {:major 0, :minor 0, :qualifier 42}]
...
(deftest fn-scope-munge
  (is (= (comp/munge (get-in (analyze aenv '(defn foo [] (fn bar [])))
                             [:init :name]))
         'cljs$user$foo))
  (is (= (comp/munge (get-in (analyze aenv '(defn foo [] (fn bar [])))
                             [:init :methods 0 :body :ret :local]))
         'cljs$user$foo_$_bar))
  (is (= (comp/munge (get-in (analyze aenv '(fn [] (fn console [])))
                             [:methods 0 :body :ret :local]))
         'cljs$user$console)))

(deftest test-js-negative-infinity
  (is (= (with-out-str (emit (analyze (assoc aenv :context :expr)
                                      'js/-Infinity)))
         "-Infinity")))

(deftest test-cljs-2352
  (are [form result]
       (= (with-out-str (emit (analyze (assoc aenv :context :expr) form)))
          result)
       Double/NaN
       "NaN"
...
(deftest test-munge-dotdot
  ...
  (is (= 'cljs.core._DOT__DOT_
         (ana/no-warn (env/with-compiler-env
                        cenv
                        (comp/munge (:info (analyze {:ns {:name 'cljs.core}}
                                                    'cljs.core/..))))))))

(deftest test-resolve-dotdot
  (is (= '{:name cljs.core/.., :ns cljs.core}
         (ana/no-warn
           (env/with-compiler-env
             cenv
             (select-keys (ana/resolve-var {:ns {:name 'cljs.core}} '..)
                          [:name :ns]))))))

(deftest test-cljs-428
  (letfn [(check-docs [docs] (is (= 1 (count (re-seq #"\*/" docs)))))]
    (check-docs (with-out-str (env/ensure (comp/emit-comment
                                            "/* multiline comments */"
                                            nil))))
    (check-docs
      (with-out-str
...
(comment
  (env/with-compiler-env cenv (emit (analyze aenv '(defn foo ([a]) ([a b]))))))
...
(defmacro capture-warnings [& body] `(capture-warnings* (fn [] ~@body)))
...
(deftest no-warn-on-emit-invoke-protocol-method
  (let [define-foo #(assoc-in %
    ...
    (binding [ana/*cljs-static-fns* true]
      (are [form]
           (empty? (capture-warnings
                     (env/with-compiler-env
                       (atom cenv-with-foo)
                       (with-out-str (emit (analyze aenv-with-foo form))))))
           '(cljs.user/foo nil)
           '(cljs.user/foo 0)
           '(cljs.user/foo (inc 0))
           '(cljs.user/foo "")
             ...
             (env/with-compiler-env
               ...
               (with-out-str
                 (emit (comp/with-core-cljs
                         opts
                         (fn []
                           (analyze aenv test-cljs-1925-code nil opts)))))))))))
  (let [opts {:static-fns true}
        cenv (env/default-compiler-env opts)]
    (is
      (= []
...
(deftest test-optimized-invoke-emit
  (let [out-file (io/file "target/invoke_test.js")]
    (comp-api/with-core-cljs
      (comp-api/compile-file (io/file "src/test/cljs/cljs/invoke_test.cljs")
                             out-file
                             {:static-fns true}))
    (let [content (slurp out-file)]
      ;; test for fn( not fn.call(, omitting arguments in test because they are
      ;; not relevant
      ;; should emit variadic invokes
      (is
        (str/includes?
          content
          "cljs.invoke_test.variadic_fn.cljs$core$IFn$_invoke$arity$variadic("))
      ;; should emit optimized invokes
      (is (str/includes?
            content
            "cljs.invoke_test.multi_fn.cljs$core$IFn$_invoke$arity$1("))
      ...
      (is
        (str/includes?
          content
          (str
            "new cljs.core.Keyword(null,\"dont-bind-this\",\"dont-bind-this\","
      ...
      ;; (hofinvoke (inv-arg0))
      (is (re-find
            #"(?m)^.*var .*=.*inv_arg0.cljs.core.IFn._invoke.arity.0 \?.*$"
            content))
      ...
      (is (re-find #"(?m)^.*var fexpr.*=.*cljs.core.complement\(funexpr1\);$"
                   content))
      (is (re-find
            #"(?m)^.*var .*=.*inv_arg1.cljs.core.IFn._invoke.arity.0 \?.*$"
            content))
      ;; CLJS-1871: A declare hinted with :arglists meta should result in static
      ;; dispatch
...
(comment (binding [ana/*cljs-static-fns* true]
           (env/with-compiler-env
             cenv
             (emit (analyze aenv
                            '(defn incme
  ...
  (binding [ana/*cljs-static-fns* true]
    (env/with-compiler-env
      ...
      (emit
        (analyze
          aenv
          '(defn foo [x] (if ^boolean (goog.array/isEmpty x) true false)))))))
