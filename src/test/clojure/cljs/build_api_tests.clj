...
(deftest test-cljs-dependents-for-macro-namespaces
  (env/with-compiler-env
    ...
    (swap! env/*compiler* assoc
      :cljs.analyzer/namespaces
      {'example.core {:require-macros {'example.macros 'example.macros,
                                       'mac 'example.macros},
                      :name 'example.core},
       'example.util {:require-macros {'example.macros 'example.macros,
                                       'mac 'example.macros},
                      :name 'example.util},
       'example.helpers {:require-macros {'example.macros-again
                                            'example.macros-again,
                                          'mac 'example.macros-again},
                         :name 'example.helpers},
       'example.fun {:require-macros nil, :name 'example.fun}})
    (is (= (set (build/cljs-dependents-for-macro-namespaces ['example.macros]))
           #{'example.core 'example.util}))
    (is (= (set (build/cljs-dependents-for-macro-namespaces
                  ['example.macros-again]))
           #{'example.helpers}))
    (is (= (set (build/cljs-dependents-for-macro-namespaces
                  ['example.macros 'example.macros-again]))
...
(binding [ana/*cljs-ns* 'cljs.user
          ana/*analyze-deps* false]
  (env/with-compiler-env test-cenv
                         (ana/no-warn (ana/analyze test-env '(ns foo.core)))))
...
(deftest cljs-1500-test-modules
  (let [out (io/file (test/tmp-dir) "cljs-1500-out")
    ...
    (is (re-find #"Loading modules A and B"
                 (slurp (-> modules
                            :cljs-base
                            :output-to))))
    (is (re-find #"Module A loaded"
                 (slurp (-> modules
                            :module-a
                            :output-to))))
    (is (re-find #"Module B loaded"
                 (slurp (-> modules
                            :module-b
                            :output-to))))))
...
(deftest cljs-1883-test-foreign-libs-use-relative-path
  (let [out (io/file (test/tmp-dir) "cljs-1883-out")
        ...
        opts {:foreign-libs [{:file (str (io/file root "thirdparty" "add.js")),
                              :provides ["thirdparty.add"]}],
              :output-dir (str out),
              :main 'foreign-libs.core,
              :target :nodejs}]
    (test/delete-out-files out)
    (build/build (build/inputs (io/file root "foreign_libs")
                               (io/file root "thirdparty"))
                 opts)
    (let [foreign-lib-file (io/file out
                                    (-> opts
                                        :foreign-libs
                                        first
                                        :file))]
...
(deftest cljs-1537-circular-deps
  (let [out (.getPath (io/file (test/tmp-dir) "cljs-1537-test-out"))
    ...
    (try
      (build/build
        (build/inputs (io/file root "circular_deps" "a.cljs")
                      (io/file root "circular_deps" "b.cljs"))
        {:main 'circular-deps.a, :optimizations :none, :output-to out})
      (is false)
      (catch Throwable e
        (let [cause-message (.getMessage (.getCause e))]
          (is
            (or
              (re-find
                #"Circular dependency detected, circular-deps.a -> circular-deps.b -> circular-deps.a"
                cause-message)
              (re-find
                #"Circular dependency detected, circular-deps.b -> circular-deps.a -> circular-deps.b"
                cause-message))))))))

(defn loader-test-project
  [output-dir]
   ...
   :opts {:output-dir output-dir,
          ...
          :foreign-libs [{:file "src/test/cljs_build/loader_test/foreignA.js",
                         ...
                         {:file "src/test/cljs_build/loader_test/foreignB.js",
                          :provides ["foreign.b"],
                          :requires ["foreign.a"]}],
          :modules {:foo {:output-to (str (io/file output-dir "foo.js")),
                          :entries #{'loader-test.foo}},
                    :bar {:output-to (str (io/file output-dir "bar.js")),
...
(deftest cljs-2077-test-loader
  (let [out (.getPath (io/file (test/tmp-dir) "loader-test-out"))]
    ...
    (let [{:keys [inputs opts]} (loader-test-project out)
      ...
      (is (.exists loader))
      (is (not (nil? (re-find #"[\\/]loader_test[\\/]foo\.js"
                              (slurp loader))))))
    (test/delete-out-files out)
    (let [{:keys [inputs opts]} (merge-with merge
                                            (loader-test-project out)
    ...
    (testing "CLJS-2309 foreign libs order preserved"
             ...
             (let [{:keys [inputs opts]} (merge-with merge
                                                     ...
                                                     {:opts {:optimizations
                                                               :advanced}})]
               (build/build (build/inputs inputs) opts)
               (is (not (nil? (re-find #"foreignA[\s\S]+foreignB"
                                       (slurp (io/file out "foo.js"))))))))))
...
(deftest test-npm-deps-simple
  ...
  (let [out (.getPath (io/file (test/tmp-dir) "npm-deps-test-out"))
        {:keys [inputs opts]}
          {:inputs (str (io/file "src" "test" "cljs_build")),
    ...
    (test/delete-out-files out)
    (build/build (build/inputs (io/file inputs "npm_deps_test/core.cljs"))
                 opts
                 cenv)
...
(deftest test-npm-deps
  ...
  (let [cenv (env/default-compiler-env)
    ...
    (testing "mix of symbol & string-based requires"
             (build/build (build/inputs (io/file
                                          inputs
                                          "npm_deps_test/string_requires.cljs"))
                          opts
                          cenv)
    ...
    (testing
      "builds with string requires are idempotent"
      (build/build (build/inputs (io/file inputs
                                          "npm_deps_test/string_requires.cljs"))
                   opts
                   cenv)
      (is (not (nil? (re-find
...
(deftest test-preloads
  (let [out (.getPath (io/file (test/tmp-dir) "preloads-test-out"))
        ...
        cenv (env/default-compiler-env)]
    (test/delete-out-files out)
    (build/build (build/inputs (io/file inputs "preloads_test/core.cljs"))
                 opts
                 cenv)
    (is (.exists (io/file out "preloads_test/preload.cljs")))
    (is (contains? (get-in @cenv
                           [::ana/namespaces 'preloads-test.preload :defs])
                   'preload-var))))

(deftest test-libs-cljs-2152
  (let [out (.getPath (io/file (test/tmp-dir) "libs-test-out"))
        {:keys [inputs opts]} {:inputs (str
                                         (io/file "src" "test" "cljs_build")),
    ...
    (let [ws (atom [])
      ...
      (ana/with-warning-handlers
        ...
        (build/build (build/inputs
                       (io/file inputs "emit_node_requires_test/core.cljs"))
                     opts
                     cenv))
      ...
      (is
        (true?
          (boolean
            (re-find
              #"emit_node_requires_test\.core\.node\$module\$react_dom\$server = require\('react-dom/server'\);"
              (slurp (io/file out "emit_node_requires_test/core.js"))))))
      (is
        (true?
          (boolean
            (re-find
              #"emit_node_requires_test\.core\.node\$module\$react_dom\$server\.renderToString"
              (slurp (io/file out "emit_node_requires_test/core.js"))))))
      (is (empty? @ws))))
  (testing
    "Node native modules, CLJS-2218"
    (let [ws (atom [])
          out (.getPath (io/file (test/tmp-dir) "emit-node-requires-test-out"))
          {:keys [inputs opts]}
            {:inputs (str (io/file "src" "test" "cljs_build")),
      ...
      (ana/with-warning-handlers
        ...
        (build/build (build/inputs
                       (io/file inputs
                                "emit_node_requires_test/native_modules.cljs"))
                     opts
                     cenv))
      ...
      (is
        (true?
          (boolean
            (re-find
              #"emit_node_requires_test\.native_modules\.node\$module\$path\.isAbsolute"
              (slurp (io/file out
                              "emit_node_requires_test/native_modules.js"))))))
      (is (empty? @ws))))
  (.delete (io/file "package.json"))
  (test/delete-node-modules))
...
(deftest test-emit-global-requires-cljs-2214
  (testing
    "simplest case, require"
    (let [ws (atom [])
          out (.getPath (io/file (test/tmp-dir)
                                 "emit-global-requires-test-out"))
          {:keys [inputs opts]}
            {:inputs (str (io/file "src" "test" "cljs_build")),
             :opts {:main 'emit-node-requires-test.core,
                    :output-dir out,
                    :optimizations :none,
                    ;; Doesn't matter what :file is used here, as long at it
                    ;; exists
                    :foreign-libs
                      [{:file "src/test/cljs_build/thirdparty/add.js",
                       ...
                       {:file "src/test/cljs_build/thirdparty/add.js",
                        ...
                        ;; Key str because contains multiple /, value shouldn't
                        ;; matter
                        :global-exports {"@material-ui/core/styles"
                                           "MaterialUIStyles",
                                         "@material-ui/core/styles/a"
                                           "MaterialUIStyles.a"}}]}}
          cenv (env/default-compiler-env)]
      ...
      (ana/with-warning-handlers
        [(collecting-warning-handler ws)]
        (build/build (build/inputs
                       (io/file inputs "emit_global_requires_test/core.cljs"))
                     opts
                     cenv))
      ...
      (is
        (true?
          (boolean
            (re-find
              #"emit_global_requires_test\.core\.global\$module\$react_dom\$server = goog\.global\[\"ReactDOMServer\"\];"
              (slurp (io/file out "emit_global_requires_test/core.js"))))))
      (is
        (true?
          (boolean
            (re-find
              #"emit_global_requires_test\.core\.global\$module\$react_dom\$server\.renderToString"
              (slurp (io/file out "emit_global_requires_test/core.js"))))))
      (testing
        "global exports using string key"
        (is
          (true?
            (boolean
              (re-find
                #"emit_global_requires_test\.core\.global\$module\$_CIRCA_material_ui\$core\$styles = goog\.global\[\"MaterialUIStyles\"\];"
                (slurp (io/file out "emit_global_requires_test/core.js"))))))
        (is
          (true?
            (boolean
              (re-find
                #"emit_global_requires_test\.core\.global\$module\$_CIRCA_material_ui\$core\$styles\.createMuiTheme"
                (slurp (io/file out "emit_global_requires_test/core.js")))))))
      (testing
        "global exports points to a sub property"
        (is
          (true?
            (boolean
              (re-find
                #"emit_global_requires_test\.core\.global\$module\$_CIRCA_material_ui\$core\$styles\$a = goog\.global\[\"MaterialUIStyles\"\]\[\"a\"\];"
                (slurp (io/file out "emit_global_requires_test/core.js"))))))
        (is
          (true?
            (boolean
              (re-find
                #"emit_global_requires_test\.core\.global\$module\$_CIRCA_material_ui\$core\$styles\$a\.foo"
                (slurp (io/file out "emit_global_requires_test/core.js")))))))
      (is (empty? @ws)))))
...
(deftest test-data-readers-records
  (let [out (.getPath (io/file (test/tmp-dir) "data-readers-test-records-out"))
    ...
    (is (true? (boolean
                 (re-find
                   #"data_readers_test.records.map__GT_Foo\("
...
(deftest test-node-modules-cljs-2246
  ...
  (spit (io/file "package.json")
        (json/json-str {:dependencies {:left-pad "1.1.3"},
                        :devDependencies {"@cljs-oss/module-deps" "*"}}))
  (apply sh/sh (cond->> ["npm" "install"] util/windows? (into ["cmd" "/c"])))
  (let [ws (atom [])
    ...
    (ana/with-warning-handlers
      [(collecting-warning-handler ws)]
      (build/build (build/inputs (io/file inputs
                                          "node_modules_opt_test/core.cljs"))
                   opts
                   cenv))
    (is (.exists (io/file out "node_modules/left-pad/index.js")))
    (is (contains? (:js-module-index @cenv) "left-pad"))
...
(deftest test-deps-api-cljs-2255
  (let [out (.getPath (io/file (test/tmp-dir) "cljs-2255-test-out"))]
    ...
    (test/delete-node-modules)
    (spit (io/file "package.json") "{}")
    (build/install-node-deps! {:react "15.6.1", :react-dom "15.6.1"}
                              {:output-dir out})
                       ...
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/react/react.js")),
                           :provides ["react" "react/react.js" "react/react"]}))
                     modules)))
      (is
        (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/react/lib/React.js")),
      ...
      (is (true?
            (some (fn [module]
                    (= module
                       {:module-type :es6,
                        :file (.getAbsolutePath
                                (io/file "node_modules/react-dom/server.js")),
...
(deftest test-cljs-2296
  (let [out (.getPath (io/file (test/tmp-dir) "cljs-2296-test-out"))
    ...
    (test/delete-out-files out)
    (build/build (build/inputs (io/file inputs
                                        "foreign_libs_dir_test/core.cljs"))
                 opts)
    (is (.exists
          (io/file out "src/test/cljs_build/foreign-libs-dir/vendor/lib.js")))
    (is
      (re-find
        #"goog\.provide\(\"module\$[A-Za-z0-9$_]+?src\$test\$cljs_build\$foreign_libs_dir\$vendor\$lib\"\)"
        (slurp (io/file
                 out
                 "src/test/cljs_build/foreign-libs-dir/vendor/lib.js"))))))
  ...
  (let [out "cljs-2334-out"
    ...
    (let [foreign-lib-file (io/file out
                                    (-> opts
                                        ...
                                        first
                                        :file))
          index-js (slurp (io/file "cljs-2334-out" "node_modules"
                                   "left-pad" "index.js"))]
      (is (.exists foreign-lib-file))
      ...
      ;; so we assert it's of the form
      ;; module$path$to$node_modules$left_pad$index
      (is
        (re-find
          #"module\$.*\$node_modules\$left_pad\$index\[\"default\"\]\(42,5,0\)"
  ...
  (let [dir (io/file "src" "test" "cljs_build" "code-split")
        ...
        opts
          {:output-dir (str out),
           ...
           :optimizations :none,
           :modules
             {:a {:entries '#{code.split.a}, :output-to (io/file out "a.js")},
              :b {:entries '#{code.split.b}, :output-to (io/file out "b.js")},
              :c {:entries '#{code.split.c}, :output-to (io/file out "c.js")}}}]
    (test/delete-out-files out)
...
(deftest test-cljs-2592
  ...
  (let [cenv (env/default-compiler-env)
        ...
        opts {:main 'package-json-resolution-test.core,
              ...
              :optimizations :none,
              :install-deps true,
              :npm-deps {:iterall "1.2.2", :graphql "0.13.1"},
              :package-json-resolution :nodejs,
              :closure-warnings {:check-types :off, :non-standard-jsdoc :off}}]
    (test/delete-out-files out)
    (build/build (build/inputs dir) opts cenv)
    (testing
      "processes the iterall index.js"
      (let [index-js (io/file out "node_modules/iterall/index.js")]
        (is (.exists index-js))
        (is (contains? (:js-module-index @cenv) "iterall"))
        (is (re-find
              #"goog\.provide\(\"module\$.*\$node_modules\$iterall\$index\"\)"
              (slurp index-js)))))
    (testing
      "processes the graphql index.js"
      (let [index-js (io/file out "node_modules/graphql/index.js")
            execution-index-js
              (io/file out "node_modules/graphql/execution/index.js")
            ast-from-value-js
              (io/file out "node_modules/grapqhl/utilities/astFromValue.js")]
        (is (.exists index-js))
        (is (contains? (:js-module-index @cenv) "graphql"))
        (is (re-find
              #"goog\.provide\(\"module\$.*\$node_modules\$graphql\$index\"\)"
              (slurp index-js)))))
    (testing
      "processes a nested index.js in graphql"
      (let [nested-index-js (io/file out
                                     "node_modules/graphql/execution/index.js")]
        (is (.exists nested-index-js))
        (is (contains? (:js-module-index @cenv) "graphql/execution"))
        (is
          (re-find
            #"goog\.provide\(\"module\$.*\$node_modules\$graphql\$execution\$index\"\)"
            (slurp nested-index-js)))))
    (testing
      "processes cross-package imports"
      (let [ast-from-value-js
              (io/file out "node_modules/graphql/utilities/astFromValue.js")]
        (is (.exists ast-from-value-js))
        (is (re-find
              #"goog.require\(\"module\$.*\$node_modules\$iterall\$index\"\);"
              (slurp ast-from-value-js)))))
    (testing
      "adds dependencies to cljs_deps.js"
      (let [deps-js (io/file out "cljs_deps.js")]
        (is (re-find
              #"goog\.addDependency\(\"..\/node_modules\/iterall\/index.js\""
              (slurp deps-js)))
        (is (re-find
              #"goog\.addDependency\(\"..\/node_modules\/graphql\/index.js\""
              (slurp deps-js)))
        (is
          (re-find
            #"goog\.addDependency\(\"..\/node_modules\/graphql\/execution/index.js\""
            (slurp deps-js)))))
    (testing
      "adds the right module names to the core.cljs build output"
      (let [core-js (io/file out "package_json_resolution_test/core.js")]
        ...
        (is
          (re-find
            #"module\$.+\$node_modules\$iterall\$index\[\"default\"\]\.isCollection"
            (slurp core-js)))
        (is (re-find
              #"goog\.require\('module\$.*\$node_modules\$graphql\$index'\);"
              (slurp core-js)))
        (is (re-find #"module\$.+\$node_modules\$graphql\$index\[\"default\"\]"
                     (slurp core-js))))))
  (.delete (io/file "package.json"))
  (test/delete-node-modules))
      ...
      (doseq [[name {:keys [output-to]}] (get-in project [:opts :modules])]
        (when-not (= :cljs-base name)
          (let [f (io/file (get mf output-to))
                sha (string/lower-case (util/content-sha (slurp (io/file f))
                                                         7))]
            (is (true? (.exists f)))
            (is (string/includes? (.getPath f) sha))))))))
