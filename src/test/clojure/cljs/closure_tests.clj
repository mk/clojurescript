...
(deftest test-make-preamble
  (testing "no options" (is (= "" (make-preamble {}))))
  (testing
    "nodejs"
    ...
    (testing
      "with no hashbang"
      (is (= "" (make-preamble {:target :nodejs, :hashbang false})))
  ...
  (testing "optimizations none"
           ...
           (is
             (thrown? AssertionError
                      (check-source-map {:source-map "target/build/app.js.map",
                                         :optimizations :none})))))
...
(deftest test-cljs-1882-constants-table-is-sorted
  (let [out (.getPath (io/file (test/tmp-dir) "cljs-1882-out"))
    ...
    (let [compiler (closure/make-closure-compiler)
      ...
      (doseq [file ["cljs/core/constants.js" "module_test/modules/a.js"
                    "cljs/core.js"]]
        (.add module (closure/js-source-file nil (io/file out file))))
      (.sortInputsByDeps module compiler)
      (is (= (->> (.getInputs module)
                  (map #(string/replace (.getName %)
                                        (str (string/replace out #"[\\\/]" "/")
                                             "/")
                                        "")))
             ["cljs/core.js" "cljs/core/constants.js"
...
(deftest test-lib-rel-path-cljs-2152
  ...
  (let [ijs {:provides ["tabby"],
             :url (io/as-url (io/file "src/test/cljs/js_libs/tabby.js")),
             :lib-path (.getAbsolutePath (io/file
                                           "src/test/cljs/js_libs/tabby.js"))}]
    (is (= (closure/lib-rel-path ijs) "tabby.js"))))
                       ...
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/left-pad/index.js")),
                           :provides ["left-pad/index.js" "left-pad/index"
                                      "left-pad"]}))
                     modules))))
  (test/delete-node-modules)
  (spit (io/file "package.json") "{}")
    ...
    (is (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           ...
                           :provides ["react/react.js" "react/react" "react"]}))
                     modules)))
    (is (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
    ...
    (is (true?
          (some (fn [module]
                  (= module
                     {:module-type :es6,
                      :file (.getAbsolutePath
                              (io/file "node_modules/node-fetch/lib/index.js")),
                      :provides ["node-fetch/lib/index.js"
                                 "node-fetch/lib/index" "node-fetch/lib"]}))
                modules))))
  (test/delete-node-modules)
  (spit (io/file "package.json") "{}")
  (closure/maybe-install-node-deps! {:npm-deps {"@comandeer/css-filter"
                                                  "1.0.1"}})
  (let [modules (closure/index-node-modules-dir)]
    (is
      (true?
          ...
          (fn [module]
            (=
              ...
              {:file
                 (.getAbsolutePath
                   (io/file
                     "node_modules/@comandeer/css-filter/dist/css-filter.umd.js")),
               :module-type :es6,
    ...
    (is
      (true?
        (some (fn [module]
                (= module
                   {:file (.getAbsolutePath
                            (io/file "node_modules/jss-extend/lib/index.js")),
                    :module-type :es6,
                    :provides ["jss-extend/lib/index.js" "jss-extend/lib/index"
                               "jss-extend" "jss-extend/lib"]}))
              modules))))
...
(deftest test-index-node-modules-module-deps-js
  ...
  (let [opts {:npm-deps {:left-pad "1.1.3"}}
    ...
    (is (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/left-pad/index.js")),
                           :provides ["left-pad" "left-pad/index.js"
                                      "left-pad/index"]}))
                     (closure/index-node-modules ["left-pad"] opts))))
    (test/delete-node-modules)
    (spit (io/file "package.json") "{}")
    (test/delete-out-files out)
    (let [opts {:npm-deps {:react "15.6.1", :react-dom "15.6.1"}}
          _ (closure/maybe-install-node-deps! opts)
          modules (closure/index-node-modules ["react" "react-dom"
                                               "react-dom/server"]
                                              opts)]
      (is
        (true? (some (fn [module]
    ...
    (spit (io/file "package.json") "{}")
    (test/delete-out-files out)
    (let [opts {:npm-deps {:node-fetch "1.7.1"}, :target :nodejs}]
      (closure/maybe-install-node-deps! opts)
      (is
        (true?
          (some (fn [module]
          ...
          (some (fn [module]
                  (= module
                     {:file (.getAbsolutePath
                              (io/file "node_modules/jss-extend/lib/index.js")),
                      :module-type :es6,
                      :provides ["jss-extend" "jss-extend/lib/index.js"
                                 "jss-extend/lib/index" "jss-extend/lib"]}))
...
(deftest test-cljs-2315
  (spit (io/file "package.json")
        (json/json-str {:devDependencies {"@cljs-oss/module-deps" "*"}}))
  ...
  (let [file (io/file (test/tmp-dir) "cljs-2315-inputs.js")
    ...
    (is (= node-inputs
           [{:file (.getAbsolutePath
                     (io/file "src/test/cljs_build/json_modules_test/a.js")),
             :module-type :es6}
            {:file (.getAbsolutePath
                     (io/file "src/test/cljs_build/json_modules_test/b.json")),
             :module-type :es6}])))
  (.delete (io/file "package.json"))
  (test/delete-node-modules))
  ...
  (let [opts {:npm-deps {:react "15.6.1",
    ...
    (is (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/tslib/tslib.es6.js")),
                           :provides ["tslib" "tslib/tslib.es6.js"
                                      "tslib/tslib.es6"]}))
...
(deftest test-cljs-2327
  (spit (io/file "package.json") "{}")
  (let [opts {:npm-deps {:react "16.0.0-beta.5", :react-dom "16.0.0-beta.5"}}
        out (util/output-directory opts)]
    (test/delete-node-modules)
    (test/delete-out-files out)
    (closure/maybe-install-node-deps! opts)
    (let [modules (closure/index-node-modules ["react" "react-dom"
                                               "react-dom/server"]
                                              opts)]
      (is
        (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/react/index.js")),
                           :provides ["react" "react/index.js" "react/index"]}))
                     modules)))
      (is
        (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/react-dom/index.js")),
                           :provides ["react-dom" "react-dom/index.js"
                                      "react-dom/index"]}))
                     modules)))
      (is (true?
            (some (fn [module]
      ...
      (is
        (true? (some (fn [module]
                       (= module
                          {:module-type :es6,
                           :file (.getAbsolutePath
                                   (io/file "node_modules/warning/browser.js")),
                           :provides ["warning" "warning/browser.js"
                                      "warning/browser"]}))
                     modules))))
    (test/delete-node-modules)
    (test/delete-out-files out)
    (spit (io/file "package.json") "{}")
    (let [opts {:npm-deps {:react-dom "16.0.0-beta.5", :react "16.0.0-beta.5"},
                :target :nodejs}
          _ (closure/maybe-install-node-deps! opts)
          ...
          (some (fn [module]
                  (= module
                     {:module-type :es6,
                      ...
                      :provides ["bootstrap" "bootstrap/dist/js/bootstrap.js"
                  ...
                  (= module
                     {:module-type :es6,
                      ...
                      :provides ["bootstrap/dist/js/bootstrap.js"
                                 "bootstrap/dist/js/bootstrap" "bootstrap"]}))
                modules))))
  (.delete (io/file "package.json"))
  (test/delete-node-modules))
      ...
      (is (true?
            (some (fn [module]
                    (= module
                       {:module-type :es6,
                        :file (.getAbsolutePath
                                (io/file "node_modules/asap/browser-asap.js")),
                        :provides ["asap/asap" "asap/asap" "asap/asap.js"
                                   "asap/asap" "asap" "asap/browser-asap.js"
                                   "asap/browser-asap"]}))
                  modules))))
    (.delete (io/file "package.json"))
    (test/delete-node-modules)
...
(deftest test-cljs-2580
  (spit (io/file "package.json") "{}")
  (let [opts {:npm-deps {"npm-package-with-main-entry-pointing-to-folder"
                           "1.0.0"},
              :target :nodejs}
        out (util/output-directory opts)]
    (test/delete-node-modules)
      ...
      (is
        (true?
          (some
            (fn [module]
              (=
                ...
                {:file
                   (.getAbsolutePath
                     (io/file
                       "node_modules/npm-package-with-main-entry-pointing-to-folder/folder/index.js")),
                 :module-type :es6,
                 :provides
            ...
            modules))))
    (let [modules (closure/index-node-modules
                    ["npm-package-with-main-entry-pointing-to-folder"]
                    opts)]
      (is
        (true?
...
(deftest test-cljs-2592
  (spit (io/file "package.json") "{}")
  (let [opts {:npm-deps {:iterall "1.2.2", :graphql "0.13.1"},
    ...
    (let [modules (closure/index-node-modules ["iterall" "graphql"] opts)]
      (is (true? (some (fn [module]
                         (= module
                            {:module-type :es6,
                             :file (.getAbsolutePath
                                     (io/file "node_modules/iterall/index.js")),
                             :provides ["iterall" "iterall/index.js"
                                        "iterall/index"]}))
                       modules)))
      (is (true? (some (fn [module]
                         (= module
                            {:module-type :es6,
                             :file (.getAbsolutePath
                                     (io/file "node_modules/graphql/index.js")),
                             :provides ["graphql" "graphql/index.js"
                                        "graphql/index"]}))
                       modules)))
      (is
        (true?
          (some
            (fn [module]
              (= module
                 {:module-type :es6,
                  :file (.getAbsolutePath
                          (io/file "node_modules/graphql/execution/index.js")),
                  :provides ["graphql/execution/index.js"
                             "graphql/execution/index" "graphql/execution"]}))
            modules))))
    (.delete (io/file "package.json"))
    (test/delete-node-modules)
    (test/delete-out-files out)))
