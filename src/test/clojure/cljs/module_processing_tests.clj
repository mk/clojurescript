   ...
   (let [filename (as-> (subs relpath (inc (.lastIndexOf relpath "/"))) $
         ...
         dirname
           (as-> (io/file relpath) $
             ...
             ;; Windows
             (string/replace $ "\\" "$")
             (if code? (string/replace $ ":" "_") (string/replace $ ":" "-")))]
     (str "module"
...
(defmethod closure/js-transforms :jsx [ijs opts] (preprocess-jsx ijs opts))
...
(deftest commonjs-module-processing
  ...
  (let [cenv (env/default-compiler-env)]
    ...
    (with-redefs [cljs.js-deps/load-library (memoize
             ...
             (env/with-compiler-env
               ...
               (closure/process-js-modules
                 {:foreign-libs [{:file "src/test/cljs/es6_hello.js",
                  ...
                  :closure-warnings {:non-standard-jsdoc :off}})))
          "processed modules are added to :libs")
      (is (= {"es6-hello" {:name (absolute-module-path
                                   "src/test/cljs/es6_hello.js"),
                           :module-type :es6}}
             (:js-module-index @cenv))
          "Processed modules are added to :js-module-index")
      (is
  ...
  (let [cenv (env/default-compiler-env)]
    ...
    (with-redefs [cljs.js-deps/load-library (memoize cljs.js-deps/load-library*)
                  cljs.js-deps/load-foreign-library
                    (memoize cljs.js-deps/load-foreign-library*)]
      (env/with-compiler-env
        cenv
        (let [opts (closure/process-js-modules
                     {:foreign-libs [{:file "src/test/cljs/calculator.js",
                                      :provides ["calculator"],
                                      :module-type :commonjs}]})
              compile (fn [form]
                        (with-out-str (comp/emit (ana/analyze (ana/empty-env)
                                                              form))))
              crlf (if util/windows? "\r\n" "\n")
              output (str (absolute-module-path "src/test/cljs/calculator.js"
                                                true)
                          "[\"default\"].add((3),(4));"
                          crlf)]
          (swap! cenv #(assoc %
          ...
          (binding [ana/*cljs-ns* 'cljs.user]
            (is (= (str "goog.provide('my_calculator.core');" crlf
                        ...
                        "goog.require('" (absolute-module-path
                                           "src/test/cljs/calculator.js"
                                           true)
                        "');" crlf)
                   (compile
                     '(ns my-calculator.core
                        (:require [calculator :as calc :refer [subtract add]
                                   :rename {subtract sub}])))))
  ...
  (let [cenv (env/default-compiler-env)]
    ...
    (with-redefs [cljs.js-deps/load-library (memoize cljs.js-deps/load-library*)
      ...
      (is (= {:optimizations :simple,
          ...
          "processed modules are added to :libs"))
    (is
      (= {"React" {:name (absolute-module-path "src/test/cljs/react-min.js"),
                   :module-type :commonjs},
