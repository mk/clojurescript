...
(defn analyze
  ([env form] (env/ensure (a/analyze env form)))
  ([env form name] (env/ensure (a/analyze env form name)))
  ([env form name opts] (env/ensure (a/analyze env form name opts))))

(defn collecting-warning-handler
  [state]
...
(def warning-forms
  {:undeclared-var (let [v (gensym)] `(~v 1 2 3)),
   :fn-arity '(do (defn x [a b] (+ a b)) (x 1 2 3 4)),
...
(defn warn-count
  ...
  (let [counter (atom 0)
        tracker (fn [warning-type env & [extra]]
                  (when (warning-type a/*cljs-warnings*) (swap! counter inc)))]
    (a/with-warning-handlers [tracker] (analyze (a/empty-env) form))
    @counter))

(deftest no-warn
  (is (every? zero?
              (map (fn [[name form]] (a/no-warn (warn-count form)))
                warning-forms))))

(deftest all-warn
  (is (every? #(= 1 %)
              (map (fn [[name form]] (a/all-warn (warn-count form)))
                warning-forms))))
...
(deftest spec-validation
  ...
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:refer-clojure :refer [])))
           (catch Exception e (.getMessage e)))
      "Only [:refer-clojure :exclude (names)] and optionally `:rename {from to}` specs supported"))
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:refer-clojure :rename [1 2])))
           (catch Exception e (.getMessage e)))
      "Only [:refer-clojure :exclude (names)] and optionally `:rename {from to}` specs supported"))
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:use [baz.woz :exclude []])))
           (catch Exception e (.getMessage e)))
  ...
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:use [baz.woz :only])))
           (catch Exception e (.getMessage e)))
      "Only [lib.ns :only (names)] and optionally `:rename {from to}` specs supported in :use / :use-macros"))
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:use [baz.woz :only [1 2 3]])))
           (catch Exception e (.getMessage e)))
      "Only [lib.ns :only (names)] and optionally `:rename {from to}` specs supported in :use / :use-macros"))
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:use [baz.woz :rename [1 2]])))
           (catch Exception e (.getMessage e)))
      "Only [lib.ns :only (names)] and optionally `:rename {from to}` specs supported in :use / :use-macros"))
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:use [foo.bar :rename {baz qux}])))
           (catch Exception e (.getMessage e)))
      "Only [lib.ns :only (names)] and optionally `:rename {from to}` specs supported in :use / :use-macros"))
  (is
    (.startsWith
      (try (analyze ns-env
                    '(ns foo.bar (:use [baz.woz :only [foo] :only [bar]])))
           (catch Exception e (.getMessage e)))
  ...
  (is (.startsWith (try (analyze ns-env
                                 '(ns foo.bar
  ...
  (is (.startsWith (try (analyze ns-env
                                 '(ns foo.bar
                                    (:require [foo.bar :rename {baz qux}])))
                        (catch Exception e (.getMessage e)))
                   "Renamed symbol baz not referred"))
  (is
    (.startsWith
      (try (analyze ns-env '(ns foo.bar (:unless [])))
           (catch Exception e (.getMessage e)))
...
(a/no-warn (e/with-compiler-env test-cenv
                                (binding [a/*analyze-deps* false]
                                  (a/analyze-file
                                    (io/file "src/main/cljs/cljs/core.cljs")))))

(deftest basic-inference
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '1))) 'number))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '"foo")))
         'string))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '\a))) 'string))
  (is (= (e/with-compiler-env test-cenv
                              (:tag (analyze test-env '(make-array 10))))
         'array))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '(js-obj))))
         'object))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '[])))
         'cljs.core/IVector))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '{})))
         'cljs.core/IMap))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '#{})))
  ...
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env ())))
         'cljs.core/IList))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '(fn [x] x))))
         'function)))

(deftest if-inference
  (is (= (a/no-warn (e/with-compiler-env test-cenv
...
(deftest loop-recur-inference
  ...
  (is (= (a/no-warn
           (e/with-compiler-env
             test-cenv
             (:tag (analyze test-env '(loop [x 10] (if (pos? x) (dec x) x))))))
         'number))
  (is (= (a/no-warn
           (e/with-compiler-env
             test-cenv
             (:tag
               (analyze test-env
                        '((fn [p?] (loop [x nil] (if (p? x) x (recur (str x)))))
                           11)))))
         '#{string clj-nil}))
  (is
    (= (a/no-warn (e/with-compiler-env
                    test-cenv
                    (:tag (analyze test-env
                                   '((fn [^string x]
                                       (loop [y x] (if (= "x" y) y (recur 1))))
                                      "a")))))
       '#{number string})))
...
(deftest fn-method-inference
  ...
  (is
    (= 'function
       (:tag (e/with-compiler-env
               test-cenv
               (analyze test-env '(fn ([a] 1) ([a b] "foo") ([a b & r] ())))))))
  (is (nil? (:ret-tag (e/with-compiler-env
                        test-cenv
                        (analyze test-env
                                 '(fn ([a] 1) ([a b] "foo") ([a b & r] ())))))))
  ;; methods should have inferred types
  (is
    (= '(number string cljs.core/IList)
       (map :tag
         (:methods
           (e/with-compiler-env
             test-cenv
...
(deftest top-fn-inference
  (e/with-compiler-env test-cenv
                       (a/analyze-form-seq
                         '[(ns test.cljs-2901)
                           (defn foo ([a] 1) ([a b] "foo") ([a b & r] ()))
                           (foo :one)]))
  (is (= '[number string cljs.core/IList]
         (map :tag
           (get-in @test-cenv
                   [::a/namespaces 'test.cljs-2901 :defs 'foo :methods]))))
  (is (= 'number
         (:tag (e/with-compiler-env
                 test-cenv
                 (a/analyze-form-seq
                   '[(ns test.cljs-2901)
                     (defn foo ([a] 1) ([a b] "foo") ([a b & r] ())) (foo :one)]
                   nil
                   true)))))
  (is (= 'string
         (:tag (e/with-compiler-env
...
(deftest variadic-fn-inference
  (is (= '(cljs.core/IList)
         (map :tag
           (:methods (e/with-compiler-env test-cenv
                                          (analyze test-env
                                                   '(fn ([a b & r] ()))))))))
  (is (= 'cljs.core/IList
         (e/with-compiler-env test-cenv
...
(deftest top-variadic-fn-inference
  (e/with-compiler-env
    test-cenv
    (a/analyze-form-seq '[(ns test.cljs-2901-b) (defn foo ([a b & r] ()))
  ...
  (is
    (= 'cljs.core/IList
       (:tag (e/with-compiler-env
               test-cenv
               (a/analyze-form-seq '[(ns test.cljs-2901-b)
                                     (defn foo ([a b & r] ())) (foo :one :two)]
                                   nil
                                   true)))))
  (is (= 'cljs.core/IList
         (:tag
...
(deftest lib-inference
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '(+ 1 2))))
         'number))
  (is (= (e/with-compiler-env test-cenv
  ...
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '(js-obj))))
         'object))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '(-conj [] 1))))
         'clj))
  (is (= (e/with-compiler-env test-cenv (:tag (analyze test-env '(conj [] 1))))
         'clj))
  (is (= (e/with-compiler-env test-cenv
...
(deftest test-numeric
  (is (= (a/no-warn (cljs.env/with-compiler-env test-cenv
                                                (:tag (analyze test-env
                                                               '(dec x)))))
         'number))
  (is (= (a/no-warn (cljs.env/with-compiler-env test-cenv
                                                (:tag (analyze test-env
                                                               '(int x)))))
         'number))
  (is (= (a/no-warn (cljs.env/with-compiler-env
                      test-cenv
...
(deftest test-defn-error
  (is (.startsWith (try (analyze test-env '(defn foo 123))
                        (catch Exception e (.getMessage e)))
  ...
  (is (= (a/rewrite-cljs-aliases
         ...
         '((:require-macros (bar :refer [quux]) :reload)
            (:require (cljs.spec.alpha :as s :refer [fdef])
                      (cljs.spec.alpha :as clojure.spec.alpha)
                      :reload))))
  (is
    (= (a/rewrite-cljs-aliases '((:refer-clojure :exclude [first])
                                  (:require-macros (bar :refer [quux]) :reload)
       ...
       '((:refer-clojure :exclude [first])
          ...
          (:require (cljs.spec.alpha :as s)
                    (cljs.spec.alpha :as clojure.spec.alpha)
                    :reload))))
  (is (= (a/rewrite-cljs-aliases '((:require-macros (bar :refer [quux]) :reload)
                                    (:require clojure.spec.alpha
                                              :reload)))
         '((:require-macros (bar :refer [quux]) :reload)
            (:require (cljs.spec.alpha :as clojure.spec.alpha)
                      :reload)))))
...
(deftest test-namespace-metadata
  (binding [a/*cljs-ns* a/*cljs-ns*]
    (is (= (do (analyze ns-env '(ns weeble.ns {:foo bar})) (meta a/*cljs-ns*))
           {:foo 'bar}))
    (is (= (do (analyze ns-env '(ns ^{:foo bar} weeble.ns)) (meta a/*cljs-ns*))
           {:foo 'bar}))
    ...
    (is (= (meta (:name (analyze ns-env '(ns ^{:foo bar} weeble.ns))))
           {:foo 'bar}))
    (is (= (meta (:name (analyze ns-env
                                 '(ns ^{:foo bar} weeble.ns {:baz quux}))))
           {:foo 'bar, :baz 'quux}))
    (is (= (meta (:name (analyze ns-env
...
(deftest test-cljs-1105
  ...
  (is (not= (a/gen-constant-id :test-kw) (a/gen-constant-id :test_kw))))

(deftest test-symbols-munge-cljs-1432
  (is (not= (a/gen-constant-id :$) (a/gen-constant-id :.)))
  (is (not= (a/gen-constant-id '$) (a/gen-constant-id '.))))
...
(deftest test-constants
  (is (.startsWith (try (analyze test-env
                                 '(do (def ^:const foo 123) (def foo 246)))
                        (catch Exception e (.getMessage e)))
                   "Can't redefine a constant"))
  (is (.startsWith (try (analyze test-env
                                 '(do (def ^:const foo 123) (set! foo 246)))
                        (catch Exception e (.getMessage e)))
                   "Can't set! a constant")))

(deftest test-cljs-1508-rename
  (binding [a/*cljs-ns* a/*cljs-ns*]
    (let [parsed-ns (e/with-compiler-env
                      test-cenv
                      (analyze test-env
                               '(ns foo.core
                                  (:require [clojure.set :as set :refer
                                             [intersection] :rename
                                             {intersection foo}]))))]
      (is (nil? (-> parsed-ns
                    :uses
                             ...
                             (analyze test-env
                                      '(ns foo.core
                                         (:use [clojure.set :only [intersection]
                                                :rename {intersection foo}])))))
    (is (= (e/with-compiler-env
             (atom {::a/namespaces
                      {'foo.core {:renames '{foo clojure.set/intersection}}}})
             (select-keys (a/resolve-var {:ns {:name 'foo.core}} 'foo)
                          [:name :ns]))
           '{:name clojure.set/intersection, :ns clojure.set}))
    (let [rwhen (e/with-compiler-env
                  (atom (update-in @test-cenv
                                   [::a/namespaces]
                                   merge
                                   {'foo.core {:rename-macros
                                                 '{always cljs.core/when}}}))
    ...
    (let [parsed-ns (e/with-compiler-env
      ...
      (is (= (-> parsed-ns
                 :renames)
             '{core-map cljs.core/map})))
    (is (thrown? Exception
                 (e/with-compiler-env
                   test-cenv
                   (analyze test-env
                            '(ns foo.core
                               (:require [clojure.set :rename
                                          {intersection foo}]))))))))
...
(deftest test-cljs-1702
  (let [ws (atom [])]
    (a/with-warning-handlers
      [(collecting-warning-handler ws)]
      (e/with-compiler-env
        test-cenv
        (a/analyze-form-seq '[(ns test.cljs-1702-a) (def ^:private a 3)
                              (def ^:private b 3) (defn- test-fn-a [a] a)
                              (defn- test-fn-b [a] b)])
        (a/analyze-form-seq
          '[(ns test.cljs-1702-b) (test.cljs-1702-a/test-fn-a 1)
            (#'test.cljs-1702-a/test-fn-b 1) test.cljs-1702-a/a
            @#'test.cljs-1702-a/b]))
...
(deftest test-cljs-1785-js-shadowed-by-local
  (let [ws (atom [])]
    (a/with-warning-handlers [(collecting-warning-handler ws)]
                             (analyze ns-env
                                      '(fn [foo] (let [x js/foo] (println x)))))
    (is (.startsWith (first @ws) "js/foo is shadowed by a local"))))

(deftest test-cljs-2005
  (let [ws (atom [])]
    (try (a/with-warning-handlers [(collecting-warning-handler ws)]
                                  (analyze (a/empty-env)
                                           '(defn myfun ([x] x) ([x] x))))
         (catch Exception _))
    (is (.startsWith (first @ws)
                     "myfun: Can't have 2 overloads with same arity"))))
...
(deftest test-canonicalize-specs
  ...
  (is (= (a/canonicalize-specs '(:require (quote clojure.set)))
         '(:require [clojure.set])))
  (is (= (a/canonicalize-specs '(:refer-clojure :exclude '[map]
                                                :rename '{map core-map}))
...
(deftest test-cljs-1346
  (testing
    "`ns*` special form conformance"
    (let [test-env (a/empty-env)]
      (is (= (-> (a/parse-ns '((require '[clojure.set :as set])))
                 :requires)
    ...
    (binding [a/*cljs-ns* a/*cljs-ns*
      ...
      (let [test-env (a/empty-env)]
        (is (= (-> (analyze test-env '(require-macros '[cljs.test :as test]))
                   :require-macros
                   vals
                   set)
               '#{cljs.test})))
      (let [test-env (a/empty-env)
            parsed (analyze test-env
                            '(require-macros '[cljs.test :as test :refer
                                               [deftest is]]))]
        (is (= (-> parsed
                   :require-macros
                   vals
      ...
      (let [test-env (a/empty-env)
            parsed (analyze test-env
                            '(require '[cljs.test :as test :refer-macros
                                        [deftest is]]))]
        (is (= (-> parsed
                   :requires
                   vals
      ...
      (let [test-env (a/empty-env)
        ...
        (is (= (-> parsed
               ...
               (-> parsed
                   :requires)
               '{Long goog.math.Long, Integer goog.math.Integer})))
      (let [test-env (a/empty-env)
  ...
  (testing "arguments to require should be quoted"
           (binding [a/*cljs-ns* a/*cljs-ns*
             ...
             (is (thrown-with-msg? Exception
                                   #"Arguments to require must be quoted"
                                   (analyze test-env '(require clojure.set))))))
  (testing
    "`:ns` and `:ns*` should throw if not `:top-level`"
    (binding [a/*cljs-ns* a/*cljs-ns*
              a/*cljs-warnings* nil]
      (are [analyzed]
           (thrown-with-msg?
      ...
      (are [analyzed]
           (thrown-with-msg? Exception
                             ...
                             analyzed)
           (analyze test-env '(def foo (require '[clojure.set :as set])))
           (analyze test-env '(fn [] (require '[clojure.set :as set])))
           (analyze test-env '(map #(require '[clojure.set :as set]) [1 2]))))))
...
(deftest test-gen-user-ns
  ...
  (let [s "src/cljs/foo.cljs"
    ...
    (is (= (a/gen-user-ns s)
           (symbol (str "cljs.user.foo" (apply str (take 7 sha)))))))
  (let [a "src/cljs/foo.cljs"
        b "src/cljs/foo.cljc"]
    ;; namespaces should have different names because the filename hash will be
    ;; different
    ...
    (let [nsa (str (a/gen-user-ns a))
      ...
      (is (not= (.substring nsa (- (count nsa) 7))
                (.substring nsb (- (count nsb) 7))))
      (is (= (.substring nsa 0 (- (count nsa) 7))
             (.substring nsb 0 (- (count nsb) 7)))))))

(deftest test-cljs-1536
  (let [parsed (e/with-compiler-env
                 test-cenv
                 (analyze (assoc test-env :def-emits-var true) '(def x 1)))]
    (is (some? (:var-ast parsed))))
  (let [parsed (e/with-compiler-env test-cenv
                                    (analyze (assoc test-env
                                               :def-emits-var true)
                                             '(let [y 1] (def y 2))))]
    (is (some? (-> parsed
                   :body
...
(defn ana'
  [form]
  (e/with-compiler-env analyze-ops-cenv (analyze test-env form)))

(defmacro ana [form] `(ana' '~form))

(defn prs-ana
  [fstr]
  (e/with-compiler-env analyze-ops-cenv
                       (let [[form] (a/forms-seq* (java.io.StringReader. fstr))]
                         (ana' form))))
...
(deftest analyze-ops
  ...
  (is (= [:var 'cljs.core 'cljs.core/inc 'inc]
         (-> (ana inc)
             ((juxt :op :ns :name :form)))))
  (is (= [:var 'cljs.core 'cljs.core/inc 'cljs.core/inc]
         (-> (ana cljs.core/inc)
             ((juxt :op :ns :name :form)))))
  ;; dotted :var
  ...
  (is ((every-pred vector? empty?)
        (-> (ana (let []))
            :bindings)))
  (is (vector? (-> (ana (let [a 1] a))
                   :bindings)))
  (is (vector? (-> (ana (let [a 1 b 2] a))
                   :bindings)))
  (is (= (-> (ana (let [a 1] a))
             :bindings
             first
  ...
  ;local shadow
  (is (= 'alert
         (a/no-warn (-> (ana (let [alert 1] js/alert))
                        :body
                        :env
                        :locals
                        (get 'alert)
  ...
  (is ((every-pred vector? empty?)
        (-> (ana (loop [] (recur)))
            ...
            :ret
            :exprs)))
  (is (vector? (-> (ana (loop [a 1] (recur 1)))
                   :body
                   :ret
                   :exprs)))
  (is (vector? (-> (ana (loop [a 1 b 2] (recur 1 2)))
                   :body
                   :ret
                   :exprs)))
  (is (= (-> (ana (loop [a 1] (recur 1)))
  ...
  (is (= (-> (ana (try))
             :children)
         [:body :catch])) ;; not sure why :catch?
  (is (= (-> (ana (try (catch :default e)))
             :children)
         [:body :catch]))
  (is (= (-> (ana (try (catch :default e) (finally)))
             :children)
         [:body :catch :finally]))
  (is (= (-> (ana (try (finally)))
             :children)
         [:body :catch :finally])) ;; not sure why :catch?
  ;   :name
  (is (symbol? (-> (ana (try (catch :default a)))
                   :name)))
  (is (nil? (-> (ana (try))
                :name)))
  ;   :catch
  (is (keyword? (-> (ana (try (catch :default a)))
                    :catch
                    :op)))
  ;   :finally
  (is (= (-> (ana (try (finally 1)))
             :finally
             :op)
         :do))
  (is (= (-> (ana (try (finally 1)))
  ...
  (is (= (-> (ana (case 1))
         ...
         :local))
  ;;   :nodes
  (is (vector? (-> (ana (case 1))
                   :body
                   :ret
                   :nodes)))
  (is (vector? (-> (ana (case 1 :a 1))
                   :body
                   :ret
                   :nodes)))
  (is (vector? (-> (ana (case 1 (:a :b) 1))
                   :body
                   :ret
                   :nodes)))
  ;;       :tests
  ...
  ;letfn
  (is (= :letfn
         (-> (ana (letfn [(my-inc [a] (inc a))] (my-inc 1)))
             :op)))
  (is (= [:bindings :body]
         (-> (ana (letfn [(my-inc [a] (inc a))] (my-inc 1)))
             :children)))
  ;   :bindings
  (is (vector? (-> (ana (letfn [(my-inc [a] (inc a))] (my-inc 1)))
                   :bindings)))
  (is (vector? (-> (ana (letfn [(my-inc [a] (inc a))] (my-inc 1)))
                   :bindings)))
  (is (= :binding
  ...
  (is (= :map
         (-> (ana #'+)
             :meta
             :op)))
  (is (empty? ; ensure at least these entries are present
              (set/difference #{:name :tag :arglists :line :column :end-line
                                :end-column :ns :file :doc :test :top-fn}
                              (->> (ana #'+)
                                   :meta
                                   :keys
                                   (map :val)))))
  ...
  ;throw
  (is (= :throw
         (-> (ana (throw (js/Error. "bad")))
             :op)))
  (is (= [:exception]
         (-> (ana (throw (js/Error. "bad")))
             :children)))
  ;   :exception
  (is (= [:js-var 'js 'js/Error]
         (-> (ana (throw (js/Error. "bad")))
             :exception
             :class
             ((juxt :op :ns :name)))))
  ...
  (is (= [:const true]
         (-> (ana ^:blah (fn []))
             ...
             first
             juxt-op-val)))
  ;(is (= [:const :foo] (-> (ana '^:foo a) :expr :meta :keys first
  ;juxt-op-val)))
  ;(is (= [:const true] (-> (ana '^:foo a) :expr :meta :vals first
  ;juxt-op-val)))
  ;   :expr
  (is (= :fn
         (-> (ana ^:blah (fn []))
  ...
  (is (binding [a/*cljs-ns* 'cljs.user]
        (= :ns*
           (-> (ana (refer-clojure :exclude '[locking]))
               :op))))
  ;quote
  (is (= :quote
         (-> (ana (quote a))
  ...
  (is (= [:js-var 'js/console 'js]
         (-> (ana js/console)
             ((juxt :op :name :ns)))))
  (is (map? (-> (ana js/console)
                :env)))
  (is (= 'js/-Infinity
         (-> (ana js/-Infinity)
             :form)))
  ;; TODO dotted :js-var (?)
  #_(is (= [:js-var 'js/console 'js]
           (-> (ana js/console)
               ((juxt :op :name :ns)))))
  ;munging
  (is (= [false 'a]
         (-> (ana (let [a (println 1) b (println 2)] [a b]))
             :bindings
             first
             ((juxt #(contains? % :ns) :name)))))
  ;shadowing
  (is (= 'a
         (-> (ana (let [a (println 1) a (println 2)] [a a]))
             :bindings second
             :shadow :name)))
  (is (= 'a
         (-> (ana (let [a (println 1) a (println 2) a (println 3)] [a a a]))
             :bindings
             (nth 2)
             :shadow
             :shadow
             :name)))
  ;ns
  (is (binding [a/*analyze-deps* false]
...
(deftest quote-args-error-test
  (is (.startsWith (try (ana (quote)) (catch Exception e (.getMessage e)))
                   "Wrong number of args to quote"))
  (is (.startsWith (try (ana (quote a b)) (catch Exception e (.getMessage e)))
                   "Wrong number of args to quote"))
  (is (.startsWith (try (ana (quote a b c d))
                        (catch Exception e (.getMessage e)))
                   "Wrong number of args to quote")))

(deftest var-args-error-test
  (is (.startsWith (try (ana (var)) (catch Exception e (.getMessage e)))
                   "Wrong number of args to var"))
  (is (.startsWith (try (ana (var a b)) (catch Exception e (.getMessage e)))
                   "Wrong number of args to var"))
  (is (.startsWith (try (ana (var nil)) (catch Exception e (.getMessage e)))
                   "Argument to var must be symbol")))

(deftest test-has-extern?-basic
  (let [externs (externs/externs-map (closure/load-externs
...
(def externs-cenv
  (atom {::a/externs (externs/externs-map
...
(deftest test-externs-infer
  (is
    (= 'js/Foo
       (-> (binding [a/*cljs-ns* a/*cljs-ns*]
             (e/with-compiler-env externs-cenv (analyze (a/empty-env) 'js/baz)))
           :info
           :ret-tag)))
  (is (= 'js/Foo
         (-> (binding [a/*cljs-ns* a/*cljs-ns*]
...
(deftest test-cljs-1871
  (let [ws (atom [])]
    (try (a/with-warning-handlers
           [(collecting-warning-handler ws)]
           (analyze (ana/empty-env)
                    '(do (declare ^{:arglists '([x y])} foo) (defn foo [x]))))
         (catch Exception _))
    (is (string/includes?
          (first @ws)
          "declared arglists ([x y]) mismatch defined arglists ([x])"))))

(deftest test-cljs-2023
  (let [form (with-meta 'js/goog.DEBUG {:tag 'boolean})]
    (is (= (-> (ana-api/analyze (a/empty-env) form)
               :tag)
           'boolean))))
...
(deftest test-cljs-1992 ;; declare after def should have no effect
  (let [test-cenv (e/default-compiler-env)]
    (e/with-compiler-env test-cenv
                         (a/analyze-form-seq '[(ns test.cljs-1992)
                                               (defn test-fn [a b c] :foo)
                                               (declare test-fn)]))
    (let [def (get-in @test-cenv
                      [::a/namespaces 'test.cljs-1992 :defs 'test-fn])]
      (is (:fn-var def)))))

(deftest test-cljs-2101
  (let [test-cenv (e/default-compiler-env)]
    (e/with-compiler-env test-cenv
                         (a/analyze-form-seq ['(ns test.cljs-2101)
                                              `(do ;; Splice in 32 forms in
                                                   ;; order to consume first
                                                   ;; chunk in chunked sequence
                                                   ~@(range 32)
                                                   (def ~'x32 1)
                                                   ;; The previous def must be
                                                   ;; analyzed for subsequent
                                                   ;; var special to succeed
                                                   (def ~'x33 (var ~'x32)))]))))

(deftest test-cljs-2139
  (let [ws (atom [])]
    (try (a/with-warning-handlers [(collecting-warning-handler ws)]
                                  (analyze (a/empty-env) '(defn foo [] x)))
         (catch Exception _))
  ...
  (binding [ana/*checked-arrays* :warn]
    (let [ws (atom [])]
      (try (a/with-warning-handlers
             [(collecting-warning-handler ws)]
             (e/with-compiler-env test-cenv
                                  (analyze (a/empty-env) '(aget (js-obj) "a"))))
           (catch Exception _))
      (is
        (=
          ["cljs.core/aget, arguments must be an array followed by numeric indices, got [object string] instead (consider goog.object/get for object access)"]
          @ws)))
    (let [ws (atom [])]
      (try (a/with-warning-handlers [(collecting-warning-handler ws)]
                                    (e/with-compiler-env
      ...
      (is
        (=
          ["cljs.core/aget, arguments must be an array followed by numeric indices, got [object string string] instead (consider goog.object/getValueByKeys for object access)"]
          @ws)))
    (let [ws (atom [])]
      (try
        (a/with-warning-handlers
          [(collecting-warning-handler ws)]
          (e/with-compiler-env test-cenv
                               (analyze (a/empty-env) '(aset (js-obj) "a" 2))))
        (catch Exception _))
      (is
        (=
          ["cljs.core/aset, arguments must be an array, followed by numeric indices, followed by a value, got [object string number] instead (consider goog.object/set for object access)"]
          @ws)))
    (let [ws (atom [])]
      (try
        (a/with-warning-handlers
          ...
          (e/with-compiler-env
            test-cenv
            (analyze (a/empty-env)
                     '(let [^objects arr (into-array [1 2 3])] (aget arr 0)))))
        (catch Exception _))
      (is (empty? @ws)))
    (let [ws (atom [])]
      (try (a/with-warning-handlers
  ...
  (let [test-env (assoc-in (a/empty-env) [:ns :name] 'cljs.user)]
    (binding [a/*cljs-ns* a/*cljs-ns*
      ...
      (is
        (thrown-with-msg?
          Exception
          #"Alias str already exists in namespace cljs.user, aliasing clojure.string"
          (analyze test-env
      ...
      (is
        (thrown-with-msg?
          ...
          #"Alias str already exists in namespace cljs.user, aliasing clojure.string"
          (analyze test-env
                   '(do (require-macros '[clojure.string :as str])
...
(deftest test-cljs-2387
  (a/no-warn (e/with-compiler-env
               test-cenv
               (a/analyze-file
                 (io/file "src/test/cljs_build/analyzer_test/no_defs.cljs"))))
  (is (= {} (get-in @test-cenv [::a/namespaces 'analyzer-test.no-defs :defs]))))

(deftest test-cljs-2475
  (is
    (thrown-with-msg? Exception
                      #"recur argument count mismatch, expected: 2 args, got: 1"
                      (analyze test-env '(loop [x 1 y 2] (recur 3))))))

(deftest test-cljs-2476
  (doseq [invalid-try-recur-form
            '[(loop [] (try (recur))) (loop [] (try (catch js/Error t (recur))))
...
(comment
  (binding [a/*cljs-ns* a/*cljs-ns*]
    (a/no-warn
      (e/with-compiler-env
        externs-cenv
        (analyze (a/empty-env) '(let [React (js/require "react")] React)))))
    ...
    (a/no-warn
      (e/with-compiler-env
        ...
        (let [aenv (a/empty-env)
          ...
          (dissoc (a/analyze-symbol (assoc-in aenv [:ns :name] 'foo.core) 'x)
                  :env)
          ;(get-in @externs-cenv [::a/namespaces 'foo.core])
          )))))

(def core-inferred
  ["var setTimeout;" "var process;" "process.hrtime;" "goog.isArrayLike;"
   "Java.type;" "Object.out;" "Object.out.println;" "Object.error;"
   "Object.error.println;"])

(defn infer-test-helper
  [{:keys [forms externs warnings warn js-dependency-index with-core? opts]}]
  (let [test-cenv
          (atom (cond-> (if with-core?
                          (env/default-compiler-env*
                            (closure/add-externs-sources
                              (merge {:infer-externs true} opts)))
                          {::a/externs (externs/externs-map
                                         (closure/load-externs
                                           {:externs (or externs [])}))})
                  js-dependency-index (assoc :js-dependency-index
                                        js-dependency-index)))
        wrap (if with-core? #(comp/with-core-cljs nil %) #(do (%)))]
    (a/with-warning-handlers
      [(collecting-warning-handler (or warnings (atom [])))]
      (binding [a/*analyze-deps* false
                a/*cljs-ns* a/*cljs-ns*]
        (e/with-compiler-env
          test-cenv
          (wrap (fn []
                  (binding [a/*analyze-deps* true
                            a/*cljs-warnings* (assoc a/*cljs-warnings*
                                                :infer-warning
                                                  (if (nil? warn) true warn))]
                    (a/analyze-form-seq forms))
                  (with-out-str (comp/emit-externs
...
(deftest test-basic-infer
  (let [res (infer-test-helper
              {:forms
                 '[(ns foo.core) (defn bar [a] (js/parseInt a))
                   (def c js/React.Component) (js/console.log "Hello world!")
                   (fn [& args]
...
(deftest test-method-infer
  (let [res (infer-test-helper
              {:forms '[(defn foo [^js/React.Component c] (.render c))]})]
    (is (= (unsplit-lines ["var React;" "React.Component;"
                           "React.Component.prototype.render;"])
           res))))

(deftest test-minimal-infer
  (let [res (infer-test-helper {:forms '[(js/console.log (.wozMethod
                                                           (js/baz)))],
                                :externs ["src/test/externs/test.js"]})]
    (is (string/blank? res))))

(deftest test-type-hint-minimal-infer
  (let [res (infer-test-helper {:forms
                                  ''[(defn afun [^js/Foo x] (.wozMethod x))],
                                :externs ["src/test/externs/test.js"]})]
    (is (string/blank? res))))
...
(deftest test-type-hint-infer-unknown-method-in-chain
  (let [ws (atom [])
        res (infer-test-helper
              {:forms
                 '[(defn afun [^js/Foo.Bar x] (let [z (.baz x)] (.wozz z)))],
               :externs ["src/test/externs/test.js"],
               :warnings ws})]
...
(deftest test-type-hint-infer-unknown-method
  (let [ws (atom [])
        res (infer-test-helper {:forms '[(defn baz [^js/Foo a] (.gozMethod a))],
                                :externs ["src/test/externs/test.js"],
                                :warnings ws})]
...
(deftest test-infer-unknown-method-from-externs
  (let [ws (atom [])
        res (infer-test-helper {:forms '[(.gozMethod (js/baz))],
                                :externs ["src/test/externs/test.js"],
                                :warnings ws})]
    (is (= (unsplit-lines ["Foo.prototype.gozMethod;"]) res))
    (is (= 1 (count @ws)))
...
(deftest test-infer-js-require
  (let [ws (atom [])
        res (infer-test-helper
              {:forms '[(ns foo.core) (def React (js/require "react"))
...
(deftest test-set-warn-on-infer
  (let [ws (atom [])
        res (infer-test-helper
              {:forms '[(ns warn-on-infer-test.app) (set! *warn-on-infer* true)
...
(deftest test-cljs-1970-infer-with-cljs-literals
  (let [ws (atom [])
        res (infer-test-helper
              {:forms '[(ns cjls-1970.core) (set! *warn-on-infer* true)
                        (defn foo [] (list)) (defn bar [] (vector))],
               :externs ["src/test/externs/test.js"],
               :warnings ws,
               :with-core? true})]
    (is (zero? (count @ws)))))
...
(deftest test-cljs-1918-infer-with-case-keywords
  (let [ws (atom [])
        res (infer-test-helper {:forms '[(ns cjls-1918.core)
                                         (defn foo
                                           [x]
                                           (cljs.core/case x
                                             :foo 1
...
(deftest test-cljs-2385-infer-priority
  (let [ws (atom [])
        res
          (infer-test-helper {:forms '[(ns cjls-1918.core)
                                       (defn thing
                                         [{:as this}]
...
(deftest test-cljs-2767-deftype-defrecord
  (let [ws (atom [])
        res (infer-test-helper
              {:forms '[(ns cjls-2767.core) (defrecord Foo [])],
               :externs ["src/test/externs/test.js"],
               :warnings ws,
               :with-core? true})]
    (is (empty? @ws))
  ...
  (let [ws (atom [])
        res (infer-test-helper {:forms '[(ns cjls-2767.core) (deftype Foo [])],
                                :externs ["src/test/externs/test.js"],
                                :warnings ws,
                                :with-core? true})]
    (is (empty? @ws))
...
(deftest test-cljs-2790-defrecord-fields
  (let [ws (atom [])
        res (infer-test-helper
              {:forms '[(ns cjls-2790.core) (defrecord Foo [a b])],
               :externs ["src/test/externs/test.js"],
               :warnings ws,
               :with-core? true})]
    (is (empty? @ws))
...
(deftest test-locals-mapped-to-sym
  (testing "analyze should be robust to :locals mapping to symbols"
           (is (= [:local 'a]
                  (-> (analyze (assoc-in test-env [:locals 'a] 'foo) 'a)
                      ((juxt :op :name)))))))

(deftest test-cljs-2814
  (is (= "global$module$react" (a/munge-global-export 'react)))
  (is (= "global$module$_CIRCA_material_ui$core$styles"
         (a/munge-global-export "@material-ui/core/styles")))
  (is (= "node$module$_CIRCA_material_ui$core$styles"
         (ana/munge-node-lib "@material-ui/core/styles"))))

(deftest test-cljs-2819
  (let [ws (atom [])]
    (a/with-warning-handlers [(collecting-warning-handler ws)]
                             (analyze ns-env '(def *foo* 1)))
    (is (string/starts-with? (first @ws)
                             "*foo* not declared dynamic and thus"))))
