...
(env/with-compiler-env st (ana/analyze-file "cljs/core.cljs") (ana/load-core))

(deftest test-doc
  (env/with-compiler-env st (is (string? (:doc (ana-api/resolve {} '->))))))
    ...
    (let [repl-env (rhino/repl-env)
      ...
      (env/with-compiler-env compiler-env
                             (binding [ana/*cljs-ns* 'cljs.user]
                               (repl/-setup repl-env)))
      (let [assoc-info (get-in @compiler-env
                               [:cljs.analyzer/namespaces 'cljs.core :defs
                                'assoc])
            {:keys [file line]} assoc-info]
        (is assoc-info)
        (is (number? line))
        (is file)
        (and file (is (io/resource file))))))
