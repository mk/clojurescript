...
(deftest test-api
  ...
  (testing "Testing string replace-first"
           ...
           (is (= "FOObarfoo"
                  (s/replace-first "foobarfoo" #"foo" s/upper-case))))
...
(defspec
  ...
  (let [ref-impl (fn [s]
        ...
        char-codes->string (fn [xs] (apply (.-fromCharCode js/String) xs))]
    (prop/for-all [s
                   (gen/fmap char-codes->string
                             (gen/not-empty (gen/vector (gen/choose 0x20
                                                                    0x7E))))]
                  (= (ref-impl s) (s/capitalize s)))))

(comment (deftest char-sequence-handling
           (are
             [result f args]
             (let [[^CharSequence s & more] args]
             ...
             "faa"
             s/replace
             ["foo" #"o" (StringBuffer. "a")]
             "baz::quux"
             s/replace-first
             ["baz--quux" #"--" "::"]
             "baz::quux"
             s/replace-first
             ["baz--quux" (StringBuffer. "--") (StringBuffer. "::")]
             "zim-zam"
             s/replace-first
             ["zim zam" #" " (StringBuffer. "-")]
             "Pow"
             s/capitalize
             ["POW"]
             "BOOM"
