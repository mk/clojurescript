(ns cljs.spec.test-test
  ...
  (:require [cljs.test :as test :refer-macros
             [deftest testing is are run-tests]]
...
(s/fdef clojure.core/symbol
  :args (s/alt :separate (s/cat :ns string?
                                :n string?)
               :str string?
               :sym symbol?)
  :ret symbol?)
...
(deftest test-cljs-2142 (is (= `[area] (stest/instrument `[pi area]))))
...
(deftest test-cljs-2391-a (is (= 1 (f-2391))))

(deftest test-cljs-2391-b
  (stest/instrument `f-2391 {:stub #{`f-2391}})
  (is (= 2 (f-2391))))

(deftest test-cljs-2391-c (stest/unstrument `f-2391) (is (= 1 (f-2391))))
...
(defn arities ([a] (inc a)) ([a b] (+ a b)) ([a b c] 0))

(s/fdef arities
  :args (s/or :arity-1 (s/cat :a number?)
              :arity-2 (s/cat :a number?
                              :b number?)
              :arity-3 (s/cat :a string?
                              :b boolean?
                              :c map?))
  :ret number?)
...
(deftest test-cljs-2953
  ...
  (is
    (= ::ret-val
       (stest/with-instrument-disabled (is (nil? @#'stest/*instrument-enabled*))
                                       (fn-2953 "abc"))))
  (is @#'stest/*instrument-enabled*))
