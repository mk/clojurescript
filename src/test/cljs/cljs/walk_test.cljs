...
(deftest t-prewalk-order
  (is (= (let [a (atom [])]
           (w/prewalk (fn [form] (swap! a conj form) form)
                      [1 2 {:a 3} (list 4 [5])])
           @a)
         [[1 2 {:a 3} (list 4 [5])] 1 2 {:a 3} [:a 3] :a 3 (list 4 [5]) 4 [5]
          5])))
...
(deftest t-postwalk-order
  (is (= (let [a (atom [])]
           (w/postwalk (fn [form] (swap! a conj form) form)
                       [1 2 {:a 3} (list 4 [5])])
           @a)
         [1 2 :a 3 [:a 3] {:a 3} 4 5 [5] (list 4 [5])
          [1 2 {:a 3} (list 4 [5])]])))
...
(defmethod get-comparator PersistentTreeMap [o] (.-comp o))

(defmethod get-comparator PersistentTreeSet [o] (get-comparator (.-tree-map o)))

(deftest walk
  "Checks that walk returns the correct result and type of collection"
  (let [colls ['(1 2 3) [1 2 3] #{1 2 3} (sorted-set-by > 1 2 3)
               {:a 1, :b 2, :c 3} (sorted-map-by > 1 10 2 20 3 30) (->Foo 1 2 3)
    ...
    (doseq [c colls]
      (let [walked (w/walk identity identity c)]
        ...
        (if (map? c)
          ...
          (is (= (w/walk inc #(reduce + %) c) (reduce + (map inc c)))))
