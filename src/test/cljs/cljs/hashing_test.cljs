...
(deftest test-hash-null (is (zero? (hash (gobject/get (js-obj) "foo")))))

;; hashing bug in many JS runtimes CLJ-118
(deftest test-clj-118
  (let [g #{(conj #{:2} :alt)} h #{#{:2 :alt}}] (is (= g h)))
  (is (= (hash {:a 1, :b 2}) (hash {:b 2, :a 1})))
  (is (= (hash (hash-map :a 1 :b 2)) (hash (hash-map :b 2 :a 1))))
...
(deftest test-962-empty-literal-hashes
  (testing
    "CLJS-962: empty literals should produce collections with correct hash codes"
    (let [l ()
          v []
          s #{}
          m {}]
...
(deftest test-cljs-1779
  (is (= (hash (keyword 'app "foo")) (hash (keyword "app" "foo")))))

(deftest test-mumur-support
  (testing
    "Testing murmur support"
    ;; int-rotate-left
...
(deftest test-cljs-1818 (is (= (hash true) 1231)) (is (= (hash false) 1237)))
