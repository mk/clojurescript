...
(defn adder ([a] a) ([a b] (+ a b)))

(s/fdef adder
  :args (s/cat :a integer?
               :b (s/? integer?))
  :ret integer?)
...
(deftest test-cljs-1754 (is (boolean? (gen/generate (s/gen boolean?)))))
...
(deftest test-cljs-1757 (is (s/exercise-fn `cljs-1757-x)))
...
(defmethod spec-type-1790 :b [_] (s/spec (constantly true)))
...
(deftest conform-explain
  (let [a (s/and #(> % 5) #(< % 10))
        ...
        s (s/coll-of (s/spec (s/cat :tag keyword?
                                    :val any?))
                     :kind
                     list?)
    ...
    (when-not js/COMPILED
      ;; CLJS-2483: these won't work with both :advanced and :none optimization
      ;; settings
      (are [spec x conformed ed]
           (let [co (s/conform spec x)
                 e (::s/problems (s/explain-data spec x))]
             (when (not= conformed co)
               (println "conform fail\n\texpect=" conformed "\n\tactual=" co))
           ...
           nil
           ::s/invalid
           [{:pred `keyword?, :val nil}]
           keyword?
           "abc"
           ::s/invalid
           [{:pred `keyword?, :val "abc"}]))
    (are
      [spec x conformed ed]
      (let [co (s/conform spec x)
            e (::s/problems (s/explain-data spec x))]
      ...
      42
      ::s/invalid
      [{:pred '(cljs.core/fn [%] (cljs.spec.alpha/int-in-range? 7 42 %)),
        :val 42}]
      irange
      #inst "1938"
      ::s/invalid
      [{:pred '(cljs.core/fn [%]
                 (cljs.spec.alpha/inst-in-range?
                   #inst "1939-01-01T00:00:00.000-00:00"
                   #inst "1946-01-01T00:00:00.000-00:00"
                   %)),
        :val #inst "1938"}]
      irange
      #inst "1942"
      ...
      3.0
      ::s/invalid
      [{:pred '(cljs.core/fn [%] (cljs.core/<= 3.1 %)), :val 3.0}]
      drange
      3.1
      3.1
      nil
      ...
      3.2
      nil
      ;drange Double/POSITIVE_INFINITY ::s/invalid [{:pred '(not (isInfinite
      ;%)), :val Double/POSITIVE_INFINITY}]
      ;; can't use equality-based test for Double/NaN
      ;; drange Double/NaN ::s/invalid {[] {:pred '(not (isNaN %)), :val
      ;; Double/NaN}}
      keyword?
      :k
      :k
      ...
      3
      ::s/invalid
      '[{:pred (cljs.core/fn [%] (cljs.core/> % 5)), :val 3}]
      a
      20
      ::s/invalid
      '[{:pred (cljs.core/fn [%] (cljs.core/< % 10)), :val 20}]
      ...
      'a
      ::s/invalid
      '[{:pred cljs.core/string?, :val a, :path [:s]}
        {:pred cljs.core/keyword?, :val a, :path [:k]}]
      c
      nil
      ::s/invalid
      ...
      [:a]
      ::s/invalid
      '[{:pred cljs.core/string?, :val :a, :path [:a], :in [0]}]
      c
      ["a"]
      ::s/invalid
      '[{:reason "Insufficient input",
      ...
      ::s/invalid
      '[{:reason "Extra input",
         :pred (cljs.spec.alpha/cat :a cljs.core/string?
                                    :b cljs.core/keyword?),
         :val (5)}]
      (s/cat)
      nil
      ...
      ::s/invalid
      '[{:reason "Insufficient input",
         :pred (cljs.spec.alpha/alt :a cljs.core/string?
                                    :b cljs.core/keyword?),
         :val (),
         :via []}]
      either
      ...
      [:k1 :k2 "x"]
      ::s/invalid
      '[{:pred cljs.core/keyword?, :val "x", :via []}]
      star
      ["a"]
      ::s/invalid
      '[{:pred cljs.core/keyword?, :val "a", :via []}]
      plus
      nil
      ::s/invalid
      '[{:reason "Insufficient input",
      ...
      [:k1 :k2 "x"]
      ::s/invalid
      '[{:pred cljs.core/keyword?, :val "x", :in [2]}]
      plus
      ["a"]
      ::s/invalid
      '[{:pred cljs.core/keyword?, :val "a", :via []}]
      ...
      ::s/invalid
      '[{:pred (cljs.core/fn [%]
                 (cljs.core/or (cljs.core/nil? %) (cljs.core/sequential? %))),
         :val :k}]
      opt
      [:k]
      :k
      ...
      ::s/invalid
      '[{:reason "Extra input",
         :pred (cljs.spec.alpha/? cljs.core/keyword?),
         :val (:k2)}]
      opt
      [:k1 :k2 "x"]
      ::s/invalid
      ...
      [:k]
      ::s/invalid
      '[{:pred cljs.spec-test/even-count?, :val [:k]}]
      andre
      [:j :k]
      [:j :k]
      nil
      ...
      '(:a :b)
      ::s/invalid
      '[{:pred cljs.core/vector?, :val (:a :b)}]
      coll
      nil
      ::s/invalid
      '[{:path [], :pred cljs.core/coll?, :val nil, :via [], :in []}]
      coll
      []
      []
      nil
      ...
      '(:a :b)
      nil
      ;;coll [:a "b"] ::s/invalid '[{:pred (coll-checker keyword?), :val [:a
      ;;b]}]
      )))
...
(deftest coll-form
  (are
    ...
    '(cljs.spec.alpha/coll-of (cljs.spec.alpha/tuple (cljs.spec.alpha/tuple
                                                       cljs.core/int?)))
...
(defn check-conform-unform
  [spec vals expected-conforms]
...
(deftest coll-conform-unform
  (check-conform-unform (s/coll-of (s/or :i int?
                                         :s string?))
                        [[1 "x"]]
                        [[[:i 1] [:s "x"]]])
  (check-conform-unform (s/every (s/or :i int?
                                       :s string?))
                        [[1 "x"]]
                        [[1 "x"]])
  (check-conform-unform (s/map-of int?
                                  (s/or :i int?
                                        :s string?))
                        [{10 10, 20 "x"}]
                        [{10 [:i 10], 20 [:s "x"]}])
  (check-conform-unform (s/map-of (s/or :i int?
                                        :s string?)
                                  int?
                                  :conform-keys
                                  true)
                        [{10 10, "x" 20}]
                        [{[:i 10] 10, [:s "x"] 20}])
  (check-conform-unform (s/every-kv int?
                                    (s/or :i int?
                                          :s string?))
                        [{10 10, 20 "x"}]
                        [{10 10, 20 "x"}]))
...
(deftest &-explain-pred
  (are [val expected]
       ...
       'cljs.core/int?
       [0 2]
       '(cljs.spec.alpha/& cljs.core/int? cljs.core/even?)))

(deftest keys-explain-pred
  (is (= 'cljs.core/map?
         (-> (s/explain-data (s/keys :req [::x]) :a)
             ::s/problems
             first
             :pred))))
...
(deftest map-spec-generators
  ...
  (is (= #{[:a] [:a :b] [:a :b :c] [:a :c]}
         (->> (s/exercise (s/keys :req-un [::a] :opt-un [::b ::c]) 100)
              ...
              (into #{}))))
  (is
    (= #{[::a ::b] [::a ::b ::c ::d] [::a ::b ::c ::d ::e] [::a ::b ::c ::e]
         [::a ::c ::d] [::a ::c ::d ::e] [::a ::c ::e]}
       (->> (s/exercise (s/keys :req [::a (or ::b (and ::c (or ::d ::e)))]) 200)
            (map (comp vec sort keys first))
            (into #{}))))
  (is (= #{[:a :b] [:a :b :c :d] [:a :b :c :d :e] [:a :b :c :e] [:a :c :d]
           [:a :c :d :e] [:a :c :e]}
         (->> (s/exercise (s/keys :req-un [::a (or ::b (and ::c (or ::d ::e)))])
                          200)
              (map (comp vec sort keys first))
              (into #{}))))
  (is (every? some?
...
(deftest tuple-explain-pred
  (are [val expected]
       (= expected
          (-> (s/explain-data (s/tuple int?) val)
              ::s/problems
              first
              :pred))
       :a
...
(defn bar-2793 ([x] {:x x}) ([x y] {:x x, :y y}) ([x y & m] {:x x, :y y, :m m}))
...
(s/def ::cljs-2940-foo
  (s/cat :bar (s/nilable ::cljs-2940-foo)))

(deftest describing-evaled-specs
  (let [sp #{1 2}] (is (= (s/describe sp) (s/form sp) sp)))
