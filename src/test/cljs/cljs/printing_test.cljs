...
(deftest test-print-with-opts
  (testing
    "Testing printing with opts - :more-marker"
    ; CLJS-1016
    (is (= (pr-str-with-opts [[1 2 3]]
                             {:more-marker "<MORE-MARKER>", :print-length 0})
           "[<MORE-MARKER>]"))
    ...
    (is (= (pr-str-with-opts ['(1 2 3)]
                             {:more-marker "\u2026", :print-length 2})
           "(1 2 \u2026)"))
    (is (#{"{:1 1, :2 2, \u2026}" "{:1 1, :3 3, \u2026}" "{:2 2, :1 1, \u2026}"
           "{:2 2, :3 3, \u2026}" "{:3 3, :1 1, \u2026}" "{:3 3, :2 2, \u2026}"}
         (pr-str-with-opts [{:1 1, :2 2, :3 3}]
                           {:more-marker "\u2026", :print-length 2}))))
  (testing
    "Testing printing with opts - :alt-impl"
    ; CLJS-1010
    (is (= (pr-str-with-opts [[1 2 3]]
                             {:alt-impl
                                (fn [obj writer opts]
...
(deftest test-printing
  (testing "Testing pr-str"
           ...
           (is (= (pr-str ["üñîçó∂£" :ทดสอบ/你好 'こんにちは])
                  "[\"üñîçó∂£\" :ทดสอบ/你好 こんにちは]"))
           (is (= (pr-str "escape chars \t \r \n \\ \" \b \f")
                  "\"escape chars \\t \\r \\n \\\\ \\\" \\b \\f\""))
           (is (= (pr-str (PrintMe. 1 2))
                  "#cljs.printing-test.PrintMe{:a 1, :b 2}"))
           ...
           (is (= (pr-str (rest (conj cljs.core.PersistentQueue.EMPTY 1 2 3)))
                  "(2 3)"))
  ...
  (testing "Testing print-str" (is (= (print-str "asdf") "asdf")))
  (testing "Testing println-str" (is (= (println-str "asdf") "asdf\n")))
