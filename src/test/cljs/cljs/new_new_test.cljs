...
(deftest test-405 (is (= 2 (-bar (baz inc) 1))))
  ...
  (let [fred (Person. "Fred" "Mertz")
    ...
    (testing "Testing records"
             ...
             (is (= (assoc fred :wife :ethel)
                    (map->Person
                      {:firstname "Fred", :lastname "Mertz", :wife :ethel})))
...
(deftest test-515 (is (== 1 (.-x (->PositionalFactoryTest 1)))))
