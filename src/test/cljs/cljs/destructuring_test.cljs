  ...
  (testing "Testing destructuring"
           ...
           (testing "namespaced keys"
                    (let [{:keys [:a :b]} {:a 1, :b 2}]
                      (testing "basic" (is (= 1 a)) (is (= 2 b))))
                    (let [{:keys [:a/b :c/d]} {:a/b 1, :c/d 2}]
                      (testing "keyword syntax" (is (= 1 b)) (is (= 2 d))))
                    (let [{:keys [a/b c/d]} {:a/b 1, :c/d 2}]
                      (testing "symbol syntax" (is (= 1 b)) (is (= 2 d))))
                    (let [{:syms [a/b c/d]} {'a/b 1, 'c/d 2}]
                      (testing ":syms" (is (= 1 b)) (is (= 2 d))))
                    (let [{:keys [::s/x ::s/y]} {:clojure.string/x 1,
                                                 :clojure.string/y 2}]
                      (testing ":keys" (is (= x 1)) (is (= y 2)))))))

(deftest keywords-in-destructuring
  (let [m {:a 1, :b 2}]
    (let [{:keys [:a :b]} m] (is (= [1 2] [a b])))
    (let [{:keys [:a :b :c], :or {c 3}} m] (is (= [1 2 3] [a b c])))))

(deftest namespaced-keywords-in-destructuring
  (let [m {:a/b 1, :c/d 2}]
    (let [{:keys [:a/b :c/d]} m] (is (= [1 2] [b d])))
    (let [{:keys [:a/b :c/d :e/f], :or {f 3}} m] (is (= [1 2 3] [b d f])))))
...
(deftest resolve-keyword-ns-alias-in-destructuring
  (let [{:keys [::s/x ::s/y ::s/z], :or {z 3}} {:clojure.string/x 1,
                                                :clojure.string/y 2}]
    (is (= [1 2 3] [x y z]))))
...
(deftest test-protocol-method-destructuring
  (testing "Testing protocol method destructuring"
           ...
           (let [t (DestructuringWithLocals. 1)]
             (testing "with locals" (is (= [2 3 1] (-find-first t [2 3])))))))

(defn destructure-1216 ([kvs] kvs) ([k v & args] [k v args]))

(defn foo-1216 ([a] (foo-1216 a 10)) ([a b & [c]] [a b c]))
...
(deftest test-cljs-1216
  (testing "varargs regression"
           ...
           (is (= [1 2 [3 4]] (destructure-1216 1 2 3 4)))
           (is (= [1 2 [3 4]] (apply destructure-1216 [1 2 3 4])))
           (is (= (destructure-1216 1 2 3 4)
                  [1 2 [3 4]]
...
(defrecord CLJS-1600-Foo []
  CLJS-1600-IFoo
    (foo-fn [_ {:keys [a b], :as args}] args))

(deftest test-cljs-1600
  (let [foo (reify
              CLJS-1600-IFoo
                (foo-fn [_ {:keys [a b], :as args}] args))]
    (is (= (foo-fn (->CLJS-1600-Foo) {:a 1, :b 2}) {:a 1, :b 2}))
    (is (= (foo-fn foo {:a 1, :b 2}) {:a 1, :b 2})))
  ;; test that the destructuring works
  (let [foo (reify
              CLJS-1600-IFoo
                (foo-fn [_ {:keys [a b], :as args}] {:a a, :b b}))]
    (is (= (foo-fn foo {:a 1, :b 2}) {:a 1, :b 2})))
  (let [foo (reify
              CLJS-1600-IFoo
                (foo-fn [_ {:keys [a b c], :or {c 3}}] {:c c}))]
    (is (= (foo-fn foo {:a 1, :b 2}) {:c 3}))))