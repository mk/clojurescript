...
(defn iter->set
  ...
  (loop [entries #{}
         iter iter]
    (if (.hasNext iter) (recur (conj entries (.next iter)) iter) entries)))
...
(deftest test-cljs-1817
  (let [cljscore-hash hash]
    (with-redefs [hash (fn [x] (if (or (#{:a :b} x) 0) cljscore-hash))]
...
(deftest test-cljs-1976
  ...
  (let [bad-record-1 (->T :eavt 17592186192276 nil)
    ...
    (testing "Transient assoc of hash-colliding keys with different hash values"
             (is (= :ok
                    (try (hash-map bad-record-1 nil bad-record-2 nil)
                         :ok
                         (catch :default _ :error))))
             (is (= :ok
                    (try (hash-map bad-date-1 nil bad-date-2 nil)
                         :ok
                         (catch :default _ :error)))))
    (testing
      "Non-transient assoc of hash-colliding keys with different hash values"
      (is (= :ok
             (try (assoc (hash-map bad-record-1 nil) bad-record-2 nil)
                  :ok
                  (catch :default _ :error))))
      (is (= :ok
             (try (assoc (hash-map bad-date-1 nil) bad-date-2 nil)
                  :ok
                  (catch :default _ :error)))))))
...
(deftest test-cljs-2496
  (testing
    "A seq or iterator over a PAM/PHM should be composed of instances of IMapEntry"
