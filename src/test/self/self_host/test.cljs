...
(ns self-host.test
  (:require [cljs.test :as test :refer-macros
             [run-tests deftest testing is async]]
            [cljs.js :as cljs]
...
(defn latch
  [m f]
  (let [r (atom 0)]
    (add-watch r :latch (fn [_ _ o n] (when (== n m) (f))))
    r))

(defn inc! [r] (swap! r inc))
  ...
  (if (contains? libs name)
    (let [path (str "src/test/self/" (cljs/ns->relpath name)
      ...
      (.readFile
        ...
        "utf-8"
        (fn [err src]
          (cb (if-not err {:lang :clj, :source src} (.error js/console err))))))
    (cb nil)))
...
(defn var-ast
  ...
  ([st sym] (var-ast st (symbol (namespace sym)) (symbol (name sym))))
  ([st ns-sym sym] (get-in st [:cljs.analyzer/namespaces ns-sym :defs sym])))

(defn file->lang
  "Converts a file path to a :lang keyword by inspecting the file
   extension."
  [file-path]
  (if (string/ends-with? file-path ".js") :js :clj))

(defn str-evals-to
  "Checks that a string evaluates to an expected value."
  ([st l expected s] (str-evals-to st l expected nil))
  ([st l expected s opts]
   (cljs/eval-str st
                  s
                  nil
                  (merge {:context :expr, :eval node-eval} opts)
                  (fn [{:keys [error value]}]
                    (is (nil? error))
                    (is (= expected value))
                    (inc! l)))))
  ...
  (async done
         (let [l (latch 4 done)]
           (cljs/require {}
                         ...
                         :reload-all
                         {:load (fn [_ cb]
                                  (cb {:lang :clj, :source "(ns load1.core)"})),
                          :eval (constantly nil)}
                         (fn [{:keys [error value]}]
                           (is (nil? error))
                           (is value)
           ...
           (cljs/require
             ...
             :reload-all
             {:macros-ns true,
              :load (fn [_ cb] (cb {:lang :clj, :source "(ns load2.core)"})),
              :eval (constantly nil)}
             (fn [{:keys [error value]}]
               (is (nil? error))
               (is value)
           ...
           (cljs/require {}
                         'load3.core
                         :reload-all
                         {:load (fn [_ cb] (cb {:lang :js, :source ""})),
                          :eval (constantly nil)}
                         (fn [{:keys [error value]}]
                           (is (nil? error))
                           (is value)
  ...
  (async
    ...
    (let [l (latch 3 done)]
      ...
      (cljs/analyze-str st
                        ...
                        nil
                        {:context :expr}
                        (fn [{:keys [error value]}] (is (nil? error)) (inc! l)))
      (cljs/analyze-str st
                        "(fn [] (let [x 7 y] (prn y)))"
                        nil
                        {:context :expr}
    ...
    (let [l (latch 7 done)]
      ...
      (cljs/compile-str st
                        ...
                        (fn [{:keys [error value]}]
                          ...
                          (inc! l)))
      (cljs/compile-str st
                        "(if cljs.core.first 1 2)"
                        nil
                        {:context :expr}
                        (fn [{:keys [error value]}]
                          (is (nil? error))
                          (is (= "(cljs.core.truth_(cljs.core.first)?(1):(2))"
                                 value))
                          (inc! l)))
      (cljs/compile-str st
                        "(.toString \"a\")"
  ...
  (async
    ...
    (let [l (latch 9 done)]
      ...
      (cljs/eval-str
        ...
        "(do (defn foo [a b] (+ a b)) (foo 1 2))"
        nil
        {:eval node-eval, :context :expr, :def-emits-var true, :static-fns true}
        (fn [{:keys [error value]}]
          (is (nil? error))
          (is (== 3 value))
          (inc! l)))
      (cljs/eval-str st
                     "(def foo (let [x 1] (let [x (inc x)] x)))"
                     nil
                     {:eval node-eval, :context :statement, :def-emits-var true}
                     (fn [{:keys [error value]}] (is (nil? error)) (inc! l)))
      (cljs/eval-str
        st
        "(with-out-str (doseq [x [1 2]] (println x)))"
        nil
        {:eval node-eval, :context :expr}
        (fn [{:keys [error value]}] (is (= "1\n2\n" value)) (inc! l))))))
    ...
    (let [l (latch 4 done)]
      (cljs/analyze-str
        ...
        {:context :expr,
         ...
         :analyze-deps false,
         :load (fn [_ cb] (cb {:lang :clj, :source "(ns analyze-me.core)"}))}
        (fn [{:keys [error]}] (is (nil? error)) (inc! l)))
      (cljs/eval st
                 '(ns analyze-deps-e.core
                    (:require [analyze-me.core :refer [abc]]))
                 {:context :expr,
                  :eval cljs.js/js-eval,
                  :analyze-deps false,
                  :load (fn [_ cb]
                          (cb {:lang :clj, :source "(ns analyze-me.core)"}))}
                 (fn [{:keys [error]}] (is (nil? error)) (inc! l)))
      (cljs/compile-str
        st
    ...
    (let [l (latch 4 done)]
      (cljs/analyze-str
        ...
        {:context :expr,
         :eval cljs.js/js-eval,
         :load-macros false,
         :load (fn [_ _] (throw (ex-info "unexpected" {})))}
        (fn [{:keys [error]}] (is (nil? error)) (inc! l)))
      (cljs/eval st
           ...
           :load
             (fn [{:keys [macros]} cb]
               (if macros
                 ...
                 (cb {:lang :clj,
                      :source "(ns foo.core (:require-macros foo.core))"})))}
          (fn [{:keys [value error]}]
            (is (nil? error))
            (cljs/eval-str st
          ...
          (fn [{:keys [value error]}]
            ...
            (cljs/eval-str st
                           ...
                           (fn [{:keys [error value]}]
                             ...
                             (inc! l))))))
      (let [st (cljs/empty-state)]
        ;; Rely on implicit macro inference for renames (ns loads its own
        ;; macros)
        (cljs/eval-str
          st
          ...
          (fn [{:keys [value error]}]
            ...
            (cljs/eval-str st
                           ...
                           (fn [{:keys [error value]}]
                             ...
                             (inc! l))))))
      (let [st (cljs/empty-state)]
        ;; Rely on implicit macro loading (ns loads its own macros), with an
        ;; alias
        ;; CLJS-1657
        (cljs/eval-str
          st
  ...
  (async
    ...
    (let [l (latch 2 done)]
      (cljs/eval-str
        ...
        (fn [{:keys [value error]}] (is (nil? error)) (inc! l)))
      (cljs/eval-str
        st
...
(deftest test-CLJS-1330
  (async
    ...
    (cljs/eval-str st
                   ...
                   nil
                   {:eval node-eval}
                   (fn [{:keys [error value]}] (is (= "1" value)) (done)))))
  ...
  (async
    ...
    (let [l (latch 3 done)]
      (cljs/eval-str
        ...
        (fn [{:keys [error value]}]
          (is (nil? value))
          (is (= "if-let requires exactly 2 forms in binding vector at line 1 "
      ...
      (cljs/eval-str
        ...
        (fn [{:keys [error value]}]
          (is (nil? value))
          (is (= "if-let requires 1 or 2 forms after binding vector at line 1 "
                 (ex-message (ex-cause error))))
          (inc! l)))
      (cljs/eval-str
        st
        ...
        (fn [{:keys [error value]}]
          (is (nil? value))
          (is (= "if-let requires a vector for its binding at line 1 "
                 (ex-message (ex-cause error))))
          (inc! l))))))
...
(deftest test-CLJS-1584
  (async
    ...
    (cljs/eval-str
      ...
      nil
      {:eval node-eval, :context :expr}
      (fn [{:keys [error value]}] (is (nil? error)) (is (= 2 value)) (done)))))
  ...
  (async done
         (cljs/eval-str
           ...
           {:ns 'cljs.user,
            :context :expr,
            :eval cljs.js/js-eval,
            :load (fn [_ cb] (cb {:lang :clj, :source "(ns aliased.core)"}))}
           (fn [{:keys [error value]}]
             (is (nil? error))
             (cljs.js/eval-str
               st
               "::alias/bar"
               nil
               {:ns 'alias-load.core, :context :expr, :eval cljs.js/js-eval}
               (fn [{:keys [error value]}]
                 (is (nil? error))
...
(deftest test-CLJS-1589
  (async
    ...
    (cljs/eval-str
      ...
      nil
      {:eval node-eval, :context :expr}
      (fn [{:keys [error value]}] (is (nil? error)) (is (= :x value)) (done)))))
    ...
    (let [st (cljs/empty-state)
           ...
           (fn [{:keys [macros]} cb]
             (if macros
               ...
               (cb
                 {:lang :clj,
                  :source
                    "(ns bar.core (:refer-macros bar.core)) (defn sub [a b] (- a b))"})))}
        (fn [_] (inc! l)))
      ...
      (testing
        ...
        (str-evals-to st
                      l
                      13
                      "(do (defmulti bar (fn [x y] [x y])) 13)"
                      {:ns 'foo.core})
        (str-evals-to st
                      l
                      17
                      "(do (deftype FnLikeB [a] IFn (-invoke [_] a)) 17)"
                      {:ns 'foo.core})
        (str-evals-to st
                      l
                      [10 4]
                      "(let [{:keys [a b] :or {b 4}} {:a 10}] [a b])"
                      {:ns 'foo.core})
        (str-evals-to st
                      l
                      [[nil]]
                      "(js->clj (make-array nil 1 1))"
                      {:ns 'foo.core})
        (str-evals-to
          st
          l
          [1 1 1 1 1]
          "(let [an-array (int-array 5 0)] (js->clj (amap an-array idx ret (+ 1 (aget an-array idx)))))"
          {:ns 'foo.core})))))
           ...
           (cljs/eval-str
             ...
             (fn [{:keys [value error]}]
               ...
               (cljs/eval-str
                 ...
                 nil
                 {:eval node-eval, :context :expr}
                 (fn [{:keys [value error]}] (is (= value [6 6])) (done))))))))
    ...
    (async
      ...
      (cljs/eval
        ...
        {:eval node-eval,
         ...
         :load (fn [_ cb]
                 (cb {:lang :clj,
                      :source "(ns foo.core1854) (def ^:const x 1)"}))}
        (fn [{:keys [value error]}]
          (is (nil? error))
          (cljs/eval
            st
            'foo.core1854/x
            {:eval node-eval, :context :expr}
            (fn [{:keys [value error]}] (is (nil? error)) (is (= value 1))))
          (cljs/eval
            st
            ...
            {:eval node-eval,
             ...
             :load (fn [_ cb]
                     (cb {:lang :clj,
                          :source "(ns foo.core1854) (def ^:const x 2)"}))}
            (fn [{:keys [value error]}]
              (is (nil? error))
              (cljs/eval
                st
                'foo.core1854/x
                {:eval node-eval, :context :expr}
                (fn [{:keys [value error]}] (is (nil? error)) (is (= value 2))))
              (cljs/eval
                st
                   ...
                   (fn [{:keys [name]} cb]
                     (case name
                       bar.core1854
                         (cb {:lang :clj,
                              :source
                                "(ns bar.core1854 (:require [foo.core1854]))"})
                       foo.core1854
                         (cb {:lang :clj,
                              :source "(ns foo.core1854) (def ^:const x 3)"})))}
                (fn [{:keys [value error]}]
                  (is (nil? error))
                  (cljs/eval st
                             'foo.core1854/x
...
(deftest test-cljs-1874
  (async done
         (let [st (cljs/empty-state)
           ...
           (cljs/eval
             st
             '(ns foo.core (:require-macros [bar.core]))
             {:load (fn [_ cb]
                      (cb {:lang :clj,
                           :source
                             "(ns bar.core) (defmacro add [a b] `(+ ~a ~b))"}))}
             (fn [_]
  ...
  (async done
         (let [st (cljs/empty-state)
           ...
           (cljs/eval-str
             ...
             nil
             {:context :expr, :eval node-eval}
             (fn [{:keys [error], :as m}] (is (nil? error)) (inc! l))))))
  ...
  (async
    ...
    (let [st (cljs/empty-state)
      ...
      (cljs/compile-str
        ...
        nil
        {:context :expr, :target :nodejs, :eval node-eval}
        (fn [{:keys [error value], :as m}]
          (is (nil? error))
          (is (some? (re-find
                       #"foo\.core\.node\$module\$path = require\('path'\);"
                       value)))
          (inc! l)))
      (cljs/eval-str
        (atom @st)
        ...
        (cljs/eval
          ...
          (fn [{:keys [error value], :as m}]
            ...
            (cljs/eval
              ...
              {:context :expr, :ns 'foo.core, :target :nodejs, :eval node-eval}
              (fn [{:keys [error value]}]
                (is (nil? error))
                (is (= value "bar"))
                (inc! l)))))))))
...
(deftest test-global-exports-cljs-2243
  (async
    ...
    subtract: function (a, b) {
      ...
      (swap! st assoc
        :js-dependency-index
        {"calculator" {:global-exports '{calculator Calculator}}})
      (cljs/compile-str
        (atom @st)
        "(ns foo.core (:require [calculator]))"
        nil
        {:context :expr, :load calculator-load, :eval node-eval}
        (fn [{:keys [error value], :as m}]
          (is (nil? error))
          (is
            (some?
              (re-find
                #"foo\.core\.global\$module\$calculator = goog.global\[\"Calculator\"\];"
                value)))
          (inc! l)))
      (cljs/eval-str
        (atom @st)
      ...
      (cljs/analyze-str (atom @st)
                        "(ns foo.core (:require [calculator]))"
                        nil
                        {:context :expr, :load calculator-load}
                        (fn [{:keys [error value], :as m}]
                          (is (nil? error))
      ...
      (let [st (atom @st)]
        (cljs/eval st
                   ...
                   (fn [{:keys [error value], :as m}]
                     ...
                     (cljs/eval st
                                '(calculator/add 1 2)
                                {:context :expr, :ns 'foo.core, :eval node-eval}
                                (fn [{:keys [error value]}]
                                  (is (nil? error))
                                  (is (= value 3))
                                  (inc! l)))))))))
  ...
  (async
    ...
    (let [st (cljs/empty-state)
      ...
      (cljs/eval-str
        ...
        "(ns foo.core (:require [path]))"
        nil
        {:context :expr, :target :nodejs, :def-emits-var true, :eval identity}
        (fn [{{:keys [source]} :value}]
          (is (some?
                (re-find
                  #"foo\.core\.node\$module\$path = require\('path'\);\snull;"
                  source)))
          (inc! l)))
      (let
        [calculator-load
        ...
        (cljs/eval-str
          ...
          (fn [{{:keys [source]} :value}]
            (is
              (some?
                (re-find
                  #"foo\.core\.global\$module\$calculator = goog.global\[\"Calculator\"\];\snull;"
                  source)))
            (inc! l)))))))
...
(deftest test-cljs-2814
  ...
  (is (= "global$module$_CIRCA_material_ui$core$styles"
         (ana/munge-global-export "@material-ui/core/styles")))
...
(deftest test-cljs-2815
  (async
    ...
    (let [st (cljs/empty-state)
          l (latch 1 done)]
      (let [x-load (fn [_ cb] (cb {:lang :js, :source "global.X = {};"}))]
        (swap! st assoc
          :js-dependency-index
          {"@material-ui/core/styles" {:global-exports
                                         {"@material-ui/core/styles" "X"}},
           "@material-ui/core/styles/a"
             {:global-exports {"@material-ui/core/styles/a" "X.a"}}})
        (cljs/eval-str
          (atom @st)
          "(ns foo.core
...
(mui-styles-a/foo)"
          nil
          {:context :expr, :def-emits-var true, :load x-load, :eval identity}
          (fn [{{:keys [source]} :value}]
            (testing
              ...
              (is
                (some?
                  (re-find
                    #"foo\.core\.global\$module\$_CIRCA_material_ui\$core\$styles = goog.global\[\"X\"\];\s"
                    source))))
            (testing
              ...
              (is
                (some?
                  (re-find
                    #"foo\.core\.global\$module\$_CIRCA_material_ui\$core\$styles\$a = goog.global\[\"X\"\]\[\"a\"\];\s"
                    source))))
            (inc! l)))))))
      ...
      (cljs/eval
        ...
        (fn [{:keys [error]}]
          ...
          (cljs/eval-str st
                         "(pr-str (cake))"
                         nil
                         {:ns 'bar.core2261a, :eval node-eval, :context :expr}
                         (fn [{:keys [error value]}]
                           (is (nil? error))
      ...
      (cljs/eval
        ...
        (fn [{:keys [error]}]
          ...
          (cljs/eval-str st
                         "(pr-str (cake))"
                         nil
                         {:ns 'bar.core2261b, :eval node-eval, :context :expr}
                         (fn [{:keys [error value]}]
                           (is (nil? error))
      ...
      (cljs.js/eval-str st
                        ...
                        {:eval node-eval,
                         :load (fn [{:keys [name macros]} cb]
                                 (cb (when (and (= name 'cljs.x) (not macros))
                                       {:lang :clj, :source "(ns cljs.x)"})))}
                        (fn [{:keys [error]}] (is (nil? error)) (inc! l))))))
...
(deftest test-cljs-2356
  (async
    ...
    (let [st (cljs/empty-state)
          load
            (fn [{:keys [name macros]} cb]
              (cb (cond (= name 'circular.a)
                          {:lang :clj,
      ...
      (binding [ana/*cljs-dep-set* (with-meta #{} {:dep-path []})]
        (cljs.js/compile-str
          st
          "(ns circular.a (:require circular.b))"
          nil
          {:load load}
          (fn [{:keys [error value], :as m}]
            (is (some? error))
            (is
              (=
                "Circular dependency detected circular.a -> circular.b -> circular.a"
                (.-message error)))
            (inc! l))))
      (binding [ana/*cljs-dep-set* (with-meta #{} {:dep-path []})]
        (cljs.js/eval-str
        ...
        (cljs.js/eval-str
          ...
          (fn [{:keys [error value], :as m}]
            ...
            (cljs.js/eval-str
              ...
              nil
              {:load load, :eval node-eval, :def-emits-var true, :ns 'foo.core}
              (fn [{:keys [error value], :as m}]
                (is (nil? error))
                (inc! l)))))))))
        ...
        {:context :expr,
         ...
         :load
           (fn [_ cb]
             (cb {:lang :clj,
                  :source
                    "(ns bar-2367.core) (def b (def a 4)) (defmacro c [] b)"}))}
        (fn [{:keys [error value]}]
          (is (nil? error))
          (cljs.js/eval st
                        '(bar-2367.core/c)
...
(deftest test-mapping-stacktrace
  (async
    ...
    (let [l (latch 1 done)]
      (testing
        "it should correctly map from canonical representation (smoke test)."
        (let [st (cljs/empty-state)]
          (cljs/eval-str
            st
            ...
            (fn [{:keys [ns value error file]}]
              (let [sms (:source-maps @st)]
                (is (= [{:function "broken-first",
                         ...
                         :line 2,
                         :column 7}]
                       (st/mapped-stacktrace [{:file "foo/bar.js",
...
(deftest test-mapping-stacktrace-with-underscore
  (async
    ...
    (let [l (latch 1 done)]
      (testing
        "it should correctly map when file names contain underscore"
        (let [st (cljs/empty-state)]
          (cljs/eval-str
            st
            ...
            {:source-map true,
             ...
             :load
               (fn [{:keys [name]} cb]
                 (cb
                   (when (= name 'foo.with-underscore)
                     {:lang :clj,
                      :source
                        "(ns foo.with-underscore)\n(defn broken-first [] (ffirst 0))",
            ...
            (fn [{:keys [ns value error file]}]
              (let [sms (:source-maps @st)]
                (is (= [{:function "broken-first",
                         ...
                         :line 2,
                         :column 7}]
                       (st/mapped-stacktrace [{:file "foo/with_underscore.js",
                                               :function "broken-first",
                                               :line 2,
                                               :column 0}]
                                             sms)))
...
(deftest test-append-source-map-with-nil-name
  (async
    ...
    (let [l (latch 1 done)]
      (testing
        "it should correctly use cljs-{js/Date as number} when name to cljs.js/eval-str is nil"
        (let [st (cljs/empty-state)]
          (cljs/eval-str
            st
            "(ns cljs.user (:require foo.bar :reload))"
            ...
            (fn [{:keys [ns value error file]}]
              (let [cljs-timestamp? #(let [[c t] (string/split % "-")]
                                      (and (= "cljs" c)
                                           (not (js/isNaN (js/parseInt t)))))
                    sms (:source-maps @st)]
                (is (some cljs-timestamp? (keys sms)))
                (inc! l)))))))))

(defn -main [& args] (run-tests))
