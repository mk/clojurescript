...
(defn ^ZipFile zip-file
  [jar-path]
  (try (cond (instance? File jar-path) (ZipFile. ^File jar-path)
  ...
  (when-let [zf (zip-file jar-path)]
    (with-open [z zf]
      (doall (map #(.getName ^ZipEntry %)
               (enumeration-seq (.entries ^ZipFile z)))))))
...
(defn find-js-jar
  ...
  [jar-path lib-path]
  (map io/resource
    (filter #(and (.endsWith ^String % ".js") (.startsWith ^String % lib-path))
...
(defn find-js-fs
  ...
  (let [file (io/file path)]
    (when (.exists file)
      (map to-url
        (filter #(.endsWith ^String (.getName ^File %) ".js")
      ...
      (fn [files jar-or-dir]
        (let [name (.toLowerCase (.getName ^File jar-or-dir))
          ...
          (->> (when (.exists ^File jar-or-dir)
                 (cond (.isDirectory ^File jar-or-dir)
                         (find-js-fs
                           (str (.getAbsolutePath ^File jar-or-dir) "/" path))
                ...
                (reduce
                  (fn [index' provide]
                    (if (:foreign dep)
                      ...
                      (let [file (when-let [f (or (:source-file dep)
                                                  (:file dep))]
                                   (str f))
                            ext (when file
                                  (subs file
                                        (inc (string/last-index-of file "."))))]
                        (update-in
                          index'
                          [provide]
                          (fn [d] (if (and (= ext "cljc") (some? d)) d dep))))))
                  index
                  provides)
                index)]
        (if (:foreign dep)
          (if-let [file (get-file dep index')]
            ...
            (throw (Exception. (str "No :file provided for :foreign-libs spec "
                                    (pr-str dep)))))
     ...
     (let [file (get state ns-name)]
       (if (or (:visited file) (nil? file))
         ...
         (let [state (assoc-in state [ns-name :visited] true)
               deps (:requires file)
               state (reduce #(dependency-order-visit %1 %2 (conj seen ns-name))
                       state
                       deps)]
...
(defn- unpack-string [m] (or (::original m) m))
  ...
  (let [state (build-index (map pack-string coll))]
    (map unpack-string
      (distinct-by :provides
                   (:order (reduce dependency-order-visit
                             (assoc state :order [])
                             (keys state)))))))
  ...
  ([lib-spec cp-only?]
   (let [find-func (if cp-only? io/resource find-url)]
     (cond-> (assoc lib-spec :foreign true)
       (:file lib-spec) (assoc :url (find-func (:file lib-spec)))
       (:file-min lib-spec) (assoc :url-min
                              (find-func (:file-min lib-spec)))))))
...
(defn library-dependencies
  ...
  (concat (mapcat load-library ups-libs) ;upstream deps
...
(comment ;; load one library
         ...
         (library-dependencies {:foreign-libs
                                  [{:file "http://example.com/remote.js",
                                    :provides ["my.example"]}]})
...
;    (filter
;      (fn [res]
;        (re-find #"(\/google-closure-library-0.0*|\/google-closure-library\/)"
;        (.getPath ^URL res)))
;      (enumeration-seq (.getResources (.getContextClassLoader
;      (Thread/currentThread)) path)))))
