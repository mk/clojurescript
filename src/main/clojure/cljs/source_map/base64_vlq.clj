...
(defn to-vlq-signed
  [v]
  (if (neg? v) (inc (bit-shift-left (- v) 1)) (+ (bit-shift-left v 1) 0)))

(defn from-vlq-signed
  [v]
      ...
      (let [digit (base64/decode (.charAt s i))]
        (let [i (inc i)
          ...
          (if continuation?
            ...
            (lazy-seq (cons (from-vlq-signed result)
                            (let [s (.substring s i)]
...
(comment
  ...
  ;; the above is gline 0, gcol 0, file 0, line 16, col 1, no name if this was
  ;; the first segment read
