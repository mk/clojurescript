...
(ns cljs.closure
  ...
  (:import
    ...
    [java.io File BufferedInputStream BufferedReader Writer InputStreamReader
     IOException StringWriter ByteArrayInputStream]
    [java.net URL]
    [java.util.logging Level]
    [java.util List Random]
    [java.util.concurrent TimeUnit LinkedBlockingDeque Executors CountDownLatch]
    [com.google.javascript.jscomp CompilerOptions CompilationLevel CompilerInput
     CompilerInput$ModuleType DependencyOptions CompilerOptions$LanguageMode
     SourceMap$Format SourceMap$DetailLevel ClosureCodingConvention SourceFile
     Result JSError CheckLevel DiagnosticGroups CommandLineRunner
     AnonymousFunctionNamingPolicy JSModule SourceMap VariableMap]
    [com.google.javascript.jscomp.deps ModuleLoader$ResolutionMode ModuleNames]
    [com.google.javascript.rhino Node]
    [java.nio.file Path Paths Files StandardWatchEventKinds WatchKey WatchEvent
     FileVisitor FileVisitResult]
...
(def ^:private GITLIBS-CACHE-DIR
  (delay
    (.getCanonicalPath (let [env (System/getenv "GITLIBS")]
                         (if (string/blank? env)
...
(defn random-char [] (nth name-chars (.nextInt (Random.) (count name-chars))))
...
(defn- sym->var
  ...
  (let [ns (namespace sym)
        _ (when (nil? ns)
            (throw (ex-info (str kw " symbol " sym " is not fully qualified")
                            (merge ex-data {kw sym}))))
        var-ns (symbol ns)]
    (when (not (find-ns var-ns))
      (try (locking ana/load-mutex (require var-ns))
           (catch Throwable t
             (throw (ex-info (str "Cannot require namespace referred by " kw
                                  " value " sym)
                             (merge ex-data {kw sym})
                             t)))))
    (find-var sym)))
...
(defmethod js-source-file String
  [^String name ^String source]
  (SourceFile/fromCode name source))

(defmethod js-source-file File
  [_ ^File source]
  (SourceFile/fromPath (.toPath source) StandardCharsets/UTF_8))

(defmethod js-source-file URL
  [_ ^URL source]
  (js-source-file _ (io/file (.getPath source))))

(defmethod js-source-file BufferedInputStream
  [^String name ^BufferedInputStream source]
  (SourceFile/fromInputStream name source))

(def check-level
  {:error CheckLevel/ERROR, :warning CheckLevel/WARNING, :off CheckLevel/OFF})
...
(def known-opts
  ...
  #{:anon-fn-naming-policy :asset-path :cache-analysis :closure-defines
    :closure-extra-annotations :closure-warnings :compiler-stats :dump-core
    :elide-asserts :externs :foreign-libs :hashbang :language-in :language-out
    :libs :main :modules :source-map-path :source-map-asset-path :optimizations
    :optimize-constants :output-dir :output-to :output-wrapper :parallel-build
    :preamble :pretty-print :print-input-delimiter :pseudo-names
    :recompile-dependents :source-map :source-map-inline :source-map-timestamp
    :static-fns :target :verbose :warnings :emit-constants :ups-externs
    :ups-foreign-libs :ups-libs :warning-handlers :preloads :browser-repl
    :cache-analysis-format :infer-externs :closure-generate-exports :npm-deps
    :fn-invoke-direct :checked-arrays :closure-module-roots :rewrite-polyfills
    :use-only-custom-externs :watch :watch-error-fn :watch-fn :install-deps
    :process-shim :rename-prefix :rename-prefix-namespace
    :closure-variable-map-in :closure-property-map-in :closure-variable-map-out
    :closure-property-map-out :stable-names :ignore-js-module-exts :opts-cache
    :aot-cache :elide-strict :fingerprint :spec-skip-macros})
...
(defn to-charset
  ...
  (cond
    ...
    :else (throw (ex-info (str "Invalid :closure-output-charset "
                               charset
                               " given, only "
...
(defn ^CompilerOptions$LanguageMode lang-key->lang-mode
  [key]
  (case (keyword (string/replace (name key) #"^es" "ecmascript"))
    :no-transpile CompilerOptions$LanguageMode/NO_TRANSPILE ;; same mode as
                                                            ;; input (for
                                                            ;; language-out
                                                            ;; only)
    ...
    :ecmascript6 CompilerOptions$LanguageMode/ECMASCRIPT_2015 ;; (deprecated and
                                                              ;; remapped)
    :ecmascript6-strict CompilerOptions$LanguageMode/ECMASCRIPT_2015 ;; (deprecated
                                                                     ;; and
                                                                     ;; remapped)
...
(defn set-options
      ...
      (set! (.anonymousFunctionNaming compiler-options)
            (case policy
              ...
              (throw (IllegalArgumentException.
                       (str "Invalid :anon-fn-naming-policy value "
                            policy
                            " - only :off, :unmapped, :mapped permitted")))))))
  ...
  (when (contains? opts :closure-generate-exports)
    (. compiler-options (setGenerateExports (:closure-generate-exports opts))))
  (when (contains? opts :rewrite-polyfills)
    (. compiler-options (setRewritePolyfills (:rewrite-polyfills opts))))
  ...
  (. compiler-options
     (setOutputCharset (to-charset (:closure-output-charset opts "UTF-8"))) ;; only
                                                                            ;; works
                                                                            ;; >
                                                                            ;; 20160125
                                                                            ;; Closure
...
(defn ^CompilerOptions make-options
  ...
  (let [level (case (:optimizations opts)
    ...
    (doseq [[key val] (:closure-defines opts)]
      (let [key (name key)]
        (cond (string? val) (.setDefineToStringLiteral compiler-options key val)
              (number? val) (.setDefineToDoubleLiteral compiler-options key val)
              (or (true? val) (false? val))
                (.setDefineToBooleanLiteral compiler-options key val)
              :else (println "value for"
                             key
    ...
    (when (:source-map opts)
      (if (:modules opts)
        ...
        (set! (.sourceMapOutputPath compiler-options)
              (str (io/file (util/output-directory opts) "cljs_modules.map")))
        (set! (.sourceMapOutputPath compiler-options) (:source-map opts)))
      (set! (.sourceMapDetailLevel compiler-options) SourceMap$DetailLevel/ALL)
...
(defn load-externs
  ...
  [{:keys [externs use-only-custom-externs target ups-externs infer-externs],
    :as opts}]
  (let [validate (fn validate [p us]
                   (if (empty? us)
                     ...
                     us))
        filter-cp-js (fn [paths] (for [p paths u (deps/find-js-classpath p)] u))
        filter-js (fn [paths] (for [p paths u (deps/find-js-resources p)] u))
        add-target (fn [ext]
                     (cons (io/resource "cljs/externs.js")
                           (if (= :nodejs target)
                             (cons (io/resource "cljs/nodejs_externs.js")
                                   (or ext []))
                             ext)))
        load-js (fn [ext] (map #(js-source-file (.getFile %) (slurp %)) ext))]
    (let [js-sources (-> externs
                         filter-js
                         add-target
                         load-js)
          ups-sources (-> ups-externs
                          filter-cp-js
                          load-js)
          all-sources (vec (concat js-sources ups-sources))]
      (cond-> (if use-only-custom-externs
                all-sources
...
(defn report-failure
  ...
  (let [errors (.errors result)
    ...
    (binding [*out* *err*]
      (doseq [next (seq errors)] (println "ERROR:" (.toString ^JSError next)))
      (doseq [next (seq warnings)]
        (println "WARNING:" (.toString ^JSError next)))
      (when (seq errors) (throw (Exception. "Closure compilation failed"))))))
...
(extend-protocol deps/IJavaScript
  String
    ...
    (-provides [this]
      (let [{:keys [provides]} (deps/parse-js-ns (string/split-lines this))]
        (cond-> provides (empty? provides) (conj (util/content-sha this 7)))))
  ...
  clojure.lang.IPersistentMap
    ...
    (-url ([this] (deps/-url this nil))
          ([this opts]
           (let [[url file] (if-let [url-min (and (#{:advanced :simple}
                                                   (:optimizations opts))
                                                  (:url-min this))]
                              [url-min (:file-min this)]
                              [(:url this) (:file this)])]
             (or url (deps/to-url file)))))
    (-relative-path ([this] (deps/-relative-path this nil))
                    ([this opts]
                     (let [file (if-let [file-min (and (#{:advanced :simple}
                                                        (:optimizations opts))
                                                       (:file-min this))]
                                  file-min
                                  (:file this))
...
(defrecord JavaScriptFile [foreign ^URL url ^URL source-url provides requires
                           lines source-map]
  deps/IJavaScript
    ...
    (-source [this] (deps/-source this nil))
    (-source [this opts] (with-open [reader (io/reader url)] (slurp reader)))
  ISourceMap
...
(defn map->javascript-file
  [m]
  (merge (javascript-file (:foreign m)
                          ...
                          (:lines m)
                          (:source-map m))
         (when-let [source-file (:source-file m)] {:source-file source-file})
...
(defn compilable-input-paths
  ...
  (reify
    ...
    cljs.closure/Compilable
      (-compile [_ opts] (mapcat #(cljs.closure/-compile % opts) paths))
      (-find-sources [_ opts]
        (mapcat #(cljs.closure/-find-sources % opts) paths))))

(defn compile-form-seq
  "Compile a sequence of forms to a JavaScript source string."
  ([forms]
   (compile-form-seq forms (when env/*compiler* (:options @env/*compiler*))))
  ([forms opts]
   (comp/with-core-cljs
     opts
     (fn []
       (with-out-str (binding [ana/*cljs-ns* 'cljs.user]
                       (doseq [form forms]
...
(defn- aot-cache?
  ...
  shared AOT cache."
  (and (:aot-cache opts) @USER-HOME-WRITABLE))
  ...
  (let [out-file (when output-file
    ...
    (when (or (nil? out-file)
      ...
      (if (not (aot-cache? opts))
        (-compile (jar-file-to-disk jar-file (util/output-directory opts) opts)
                  opts)
        (let [cache-path (ana/cache-base-path (util/path jar-file) opts)]
          (when (comp/requires-compilation? jar-file
                                            (:output-file cacheable)
                                            opts)
    ...
    (compile-file (io/file (util/output-directory opts)
                           (last (string/split (.getPath ^URL jar-file)
                                               #"\.jar!/")))
                  opts)))
...
(extend-protocol Compilable
  File
    (-compile [this opts]
      (if (.isDirectory this) (compile-dir this opts) (compile-file this opts)))
    (-find-sources [this _]
      (if (.isDirectory this)
  ...
  clojure.lang.Symbol
    ...
    (-find-sources [this opts] (-find-sources (util/ns->source this) opts))
  ...
  clojure.lang.IPersistentSet
    ...
    (-find-sources [this opts] (into [] (mapcat #(-find-sources % opts)) this)))
...
(comment
  ...
  (-find-sources (io/resource "clojure/set.cljs")
                 {:output-file "clojure/set.js"})
  ;; compile a project
  ...
  (-compile '[(ns test.app
                (:require [goog.array :as array])) (defn plus-one [x] (inc x))]
            {})
...
(defn js-dependencies
  ...
  (loop [requires requires
         ...
         deps #{}]
    (if (seq requires)
      (let [node (or (get (@env/*compiler* :js-dependency-index)
                          (first requires))
...
(comment ;; find dependencies
         ...
         (js-dependencies {:libs ["closure/library/third_party/closure"]}
                          ["goog.dom.query"]))
            ...
            (if-let [js-res (io/resource relpath)]
              ...
              (let [ijs (get-in @compiler-env [:js-dependency-index (str ns)])
                ...
                (if-let [js-res (and relpath
                                     ;; try to parse URL, otherwise just return
                                     ;; local
                                     ;; resource
...
(defn add-dependencies
  ...
  (let [inputs (set inputs)
        ...
        required-cljs (clojure.set/difference (cljs-dependencies opts requires)
                                              inputs)
        required-js (js-dependencies opts
                                     (into (set (mapcat deps/-requires
                                                  required-cljs))
                                           requires))]
    (cons (javascript-file nil (io/resource "goog/base.js") ["goog"] nil)
          (deps/dependency-order
            (concat
              (map (fn [{:keys [type foreign url file provides requires],
                         :as js-map}]
                     ;; ignore :seed inputs, only for REPL - David
                     (if (not= :seed type)
                       (let [url (or url (io/resource file))]
                         (merge (javascript-file foreign url provides requires)
                                js-map))
                       js-map))
                required-js)
              (when (-> @env/*compiler*
...
(comment (alter-var-root #'env/*compiler*
                         (constantly (env/default-compiler-env)))
         ;; only get cljs deps
...
(defn compile-task
  [^LinkedBlockingDeque deque input-set compiled opts failed]
    ...
    (when (and ns-info (not @failed))
      (let [{:keys [requires]} ns-info
        ...
        (if (every? #(not (contains? input-set' %)) requires)
          (do
            (try (swap! compiled conj
                   (-compile (or (:source-file ns-info) (:source-forms ns-info))
                             ; - ns-info -> ns -> cljs file relpath -> js
                             ; relpath
                             (merge opts
   ...
   (if (:parallel-build opts)
     ...
     (util/measure
       ...
       (binding [comp/*inputs* (zipmap (map :ns inputs) inputs)]
         (doall (for [ns-info inputs]
                  ; TODO: compile-file calls parse-ns unnecessarily to get
                  ; ns-info
...
(defn remove-goog-base [inputs] (remove #(= (:provides %) ["goog"]) inputs))
...
(defn add-js-sources
  ...
  (let [requires (set (mapcat deps/-requires inputs))
    ...
    (concat
      (map (fn [{:keys [foreign url file provides requires], :as js-map}]
             (let [url (or url (io/resource file))]
               (merge (javascript-file foreign url provides requires) js-map)))
        required-js)
      (when (-> @env/*compiler*
                :options
                :emit-constants)
...
(defn add-preloads
  ...
  (if-not (:preloads opts)
    ...
    (let [pred (fn [x]
          ...
          preloads (remove nil?
                     (map (fn [preload]
                            (try (comp/find-source preload)
                                 (catch Throwable t
                                   (util/debug-prn "WARNING: preload namespace"
                                                   preload
                                                   "does not exist"))))
                       (:preloads opts)))]
...
(defmethod javascript-name URL
  [^URL url]
  (if url (.getPath url) "cljs/user.js"))

(defmethod javascript-name String
  [s]
  (if-let [name (first (deps/-provides s))]
    name
    "cljs/user.js"))

(defmethod javascript-name JavaScriptFile
  [js]
...
(defmethod js-source-file JavaScriptFile
  [_ js]
...
(comment (ensure-cljs-base-module
           {:cljs-base {:output-to "out/modules/base.js"},
            :core {:output-to "out/modules/core.js", :entries '#{cljs.core}},
            :landing {:output-to "out/modules/reader.js",
                      :entries '#{cljs.reader},
                      :depends-on #{:core}}}))
...
(defn- const-expr-form
  ...
  [compiler-state sym]
  (let [const-expr (get-in compiler-state
                           [::ana/namespaces (symbol (namespace sym)) :defs
                            (symbol (name sym)) :const-expr])]
    (some-> const-expr
            ana/const-expr->constant-value)))
    ...
    (let [module-uris (module-graph/modules->module-uris modules inputs opts)
      ...
      (swap! env/*compiler* ana/add-consts
        {'cljs.core/MODULE_INFOS
           (merge (const-expr-form @env/*compiler* 'cljs.core/MODULE_INFOS)
                  module-infos),
         'cljs.core/MODULE_URIS (merge (const-expr-form @env/*compiler*
                                                        'cljs.core/MODULE_URIS)
                                       module-uris)})
      (-compile (:source-file loader)
                (merge opts
                       {:cache-key (util/content-sha (pr-str module-uris)),
                        :output-file (comp/rename-to-js (util/ns->relpath
                                                          (:ns loader)))}))))
  inputs)
  ...
  (let [sources
          (map (fn [js]
                 (cond (instance? JavaScriptFile js) js
                       ...
                       (string? js) (merge (map->javascript-file
        ...
        modules
          (reduce
            (fn [ret [name {:keys [entries depends-on], :as module-desc}]]
              ...
              (when (:verbose opts) (util/debug-prn "Building module" name))
              (let [js-module (JSModule. (clojure.core/name name))
                      ...
                      (reduce
                        (fn [ret entry-sym]
                          (if-let [entries
                            ...
                            (throw
                              (IllegalArgumentException.
                                (str "Could not find matching namespace for "
                                     entry-sym)))))
                        []
                        entries)
                    foreign-deps (atom [])]
                ...
                (doseq [dep depends-on]
                  (if-let [parent-module (get-in (into {} ret)
                                                 [dep :closure-module])]
            ...
            (module-graph/sort-modules
              (ensure-cljs-base-module
                (module-graph/expand-modules (:modules opts) sources)
                opts)))]
    modules))
  ...
  (let [modules
          (build-modules
            ...
            {:optimizations :advanced,
             :output-dir "out",
             :cache-analysis true,
             :modules {:core {:output-to "out/modules/core.js",
                              :entries '#{cljs.core}},
                       :landing {:output-to "out/modules/reader.js",
                                 :entries '#{cljs.reader},
                                 :depends-on #{:core}}}})]
                 ...
                 (remove nil?
                   (map (fn [source]
                          (if-let [^URL source-url (:source-url source)]
                            (.getPath source-url)
                            (if-let [^URL url (:url source)] (.getPath url))))
                     sources))))]
      (if sources
          ...
          (recur
            ...
            (if-let [url (:url source)]
              (let [path (.getPath ^URL url)]
                (if-let [compiled (get-in @env/*compiler*
                                          [::comp/compiled-cljs path])]
                  (if-let [source-url (:source-url source)]
                    (assoc merged
                      (.getPath ^URL source-url)
                        (sm/merge-source-maps (:source-map compiled)
...
(defn optimize-modules
  ...
  #_{:pre [(and (contains? opts :modules) (not (contains? opts :output-to)))]}
  (assert (= (count (:modules opts))
             (count (into #{} (map (comp :output-to second) (:modules opts)))))
  ...
  (let [closure-compiler (make-closure-compiler)
        ...
        _ (when (or ana/*verbose* (:verbose opts))
            (util/debug-prn "Applying optimizations"
                            (:optimizations opts)
                            "to"
                            (count sources)
                            "sources"))
        ^Result result
          (.compileModules closure-compiler externs inputs compiler-options)
    ...
    (if (.success result)
      (do (write-variable-maps result opts)
          (vec
            (for [[name {:keys [output-to closure-module], :as module}] modules]
              [name
               (merge (assoc module
                        :source (do (when source-map (.reset source-map))
                                    (.toSource closure-compiler
                                               ^JSModule closure-module)))
                      (when source-map
                        (let [sw (StringWriter.)
          ...
          (let [source (.toSource closure-compiler)]
            (when-let [name (:source-map opts)]
              (let [name' (str name ".closure")
                    sw (StringWriter.)
                    sm-json-str
                      (do (.appendTo (.getSourceMap closure-compiler) sw name')
                          (.toString sw))]
...
(comment ;; optimize JavaScript strings
...
(defn ^String path-relative-to
  ...
  (let [base-path (util/path-seq (.getCanonicalPath base))
        input-path (util/path-seq (.getCanonicalPath (io/file (deps/-url
                                                                input))))
...
(comment (path-relative-to (io/file "out/goog/base.js")
                           {:url (deps/to-url "out/cljs/core.js")})
         (add-dep-string {}
                         {:url (deps/to-url "out/cljs/core.js"),
...
(defn ^File fingerprint-out-file
  ...
  (let [dir (.getParent out-file)
    ...
    (io/file
      dir
      (str name "-" (string/lower-case (util/content-sha content 7)) "." ext))))

(defn output-one-file
  [{:keys [output-to fingerprint], :as opts} js]
  (let [js (elide-strict js opts)]
    (cond
      (nil? output-to) js
      (or (string? output-to) (util/file? output-to))
...
(defn output-main-file
  ...
  (let [module (get (:modules opts) (:module-name opts))
        asset-path (or (:asset-path opts) (util/output-directory opts))
            ...
            (str
              (when (or (not module) (= :cljs-base (:module-name opts)))
                (str
                  ...
                  asset-path
                  "\";\n"
                  "load((new java.io.File(new java.io.File(\""
                  asset-path
                  "\",\"goog\"), \"base.js\")).getPath());\n"
                  "load((new java.io.File(new java.io.File(\""
                  asset-path
                  "\",\"goog\"), \"deps.js\")).getPath());\n"
                  "load((new java.io.File(new java.io.File(new java.io.File(\""
                  asset-path
                  "\",\"goog\"),\"bootstrap\"),\""
                  (name (:target opts))
                  ".js\")).getPath());\n"
        ...
        (str
          (when (or (not module) (= :cljs-base (:module-name opts)))
            (str
              ...
              "if(typeof goog == \"undefined\") document.write('<script src=\""
              asset-path
              "/goog/base.js\"></script>');\n"
          ...
          (apply str
            (map (fn [entry]
                   (when-not (= "goog" entry)
                     (str "document.write('<script>goog.require(\""
                          (comp/munge entry)
                          "\");</script>');\n")))
              (if-let [entries (when module (:entries module))]
                entries
...
(defn fingerprinted-modules
  [modules fingerprint-info]
  (into
    {}
    (map (fn [[module-name module-info]]
           (let [module-info' (assoc module-info
                                :output-to (get-in fingerprint-info
                                                   [module-name
...
(defn output-modules
  ...
  (let [fingerprint-info (atom {})]
    (doseq [[name {:keys [output-to source foreign-deps], :as module-desc}]
              modules]
      ...
      (let [fdeps-str (when-not (empty? foreign-deps)
            ...
            js (as-> source source
                 (if (= name :cljs-base) (add-header opts source) source)
                 (if fdeps-str (str fdeps-str "\n" source) source)
                 (elide-strict source opts)
                 (if sm-name
                   (add-source-map-link (assoc opts
                                          :output-to output-to
                                          :source-map sm-name)
                                        source)
                   source))
...
(defn lib-rel-path
  [{:keys [lib-path url provides], :as ijs}]
  (if (nil? lib-path)
...
(defn ^String rel-output-path
  ...
  ([js] (rel-output-path js (when env/*compiler* (:options @env/*compiler*))))
  ([js opts]
   (let [url (deps/-url js opts)]
     (cond url (cond (deps/-closure-lib? js) (lib-rel-path js)
...
(defn make-convert-js-module-options
  [opts]
  (-> opts
      (select-keys [:closure-warnings :closure-extra-annotations :pretty-print
                    :language-in :language-out :closure-module-roots
                    :rewrite-polyfills])
      (assoc-in [:closure-warnings :non-standard-jsdoc] :off)
      (set-options (CompilerOptions.))))

(defn module-type->keyword
  [^CompilerInput$ModuleType module-type]
...
(defn add-converted-source
  ...
  (let [processed-file (if-let [min (and (#{:advanced :simple}
                                          (:optimizations opts))
                                         file-min)]
                         min
                         file)
        ...
        ;; getJsModuleType returns NONE for ES6 files, but getLoadsFlags module
        ;; returns es6 for those
        module-type (or (some-> (.get (.getLoadFlags input) "module")
                                keyword)
    ...
    (assoc ijs
      ...
      :source
        ...
        (str
          (apply str (map (fn [n] (str "goog.provide(\"" n "\");\n")) provides))
          (->> (.getRequires input)
...
(comment
  ...
  (= (package-json-entries {:package-json-resolution :webpack})
     ["browser" "module" "main"])
  (= (package-json-entries {:package-json-resolution ["foo" "bar" "baz"]})
     ["foo" "bar" "baz"])
  (= (package-json-entries {:target :nodejs}) ["main"])
  (= (package-json-entries {:target :nodejs, :package-json-resolution :nodejs})
     ["main"])
  (= (package-json-entries {:target :nodejs, :package-json-resolution :webpack})
     ["browser" "module" "main"])
  (= (package-json-entries {:target :nodejs,
                            :package-json-resolution ["foo" "bar"]})
     ["foo" "bar"]))
...
(defn convert-js-modules
  ...
  (let [^List externs '()
        ...
        ^CompilerOptions options
          (doto (make-convert-js-module-options opts)
    ...
    ;; Based on language-in setting, this could also handle ES7/8/TypeScript
    ;; transpilation.
    (.transpileAndDontCheck closure-compiler)
    (map (partial add-converted-source closure-compiler inputs-by-name opts)
      js-modules)))
...
(defmulti js-transforms
  ...
  (fn [ijs opts] (:preprocess ijs)))

(defmethod js-transforms :default
  [ijs opts]
...
(defn write-javascript
  ...
  (let [out-dir (io/file (util/output-directory opts))
        ...
        js-module?
          (and res out-dir (.startsWith (util/path res) (util/path out-dir))) ;; We
                                                                              ;; already
                                                                              ;; Closure
                                                                              ;; processed
                                                                              ;; it
        ...
        ijs (merge
              {:requires (deps/-requires js),
               ...
               :group (:group js)}
              (when (not js-module?)
                {:url (deps/to-url out-file), :out-file (.toString out-file)}))]
    (when (and (not js-module?)
      ...
      (when res (.setLastModified ^File out-file (util/last-modified res))))
  ...
  (if (write-js? js)
    ...
    (let [source-url (:source-url js)
      ...
      (when (and out-file
        ...
        (do (when (or ana/*verbose* (:verbose opts))
...
(comment (write-javascript {} "goog.provide('demo');\nalert('hello');\n")
...
(defn output-unoptimized
  ...
  (let [disk-sources (doall (remove #(= (:group %) :goog)
    ...
    (cond
      modules (let [modules' (module-graph/expand-modules modules sources)]
                (output-deps)
                (doall
                  (map (fn [[module-name _]]
                         (output-main-file (merge opts
                                                  {:module-name module-name,
                                                   :modules modules'})))
                    modules)))
      (and main (not= :none (:target opts))) (do (output-deps)
...
(defn get-upstream-deps*
  ...
  ([classloader]
   (let [upstream-deps (map #(read-string (slurp %))
                         (enumeration-seq (. classloader
                                             (getResources "deps.cljs"))))]
     (apply merge-with
       (fn [a b]
         (if (map? a) (merge-with #(into #{%1} #{%2}) a b) (concat a b)))
       upstream-deps))))
...
(defn add-wrapper
  [{:keys [output-wrapper], :as opts} js]
  (if output-wrapper
    (cond (fn? output-wrapper) (output-wrapper js)
          (string? output-wrapper) (format output-wrapper js)
          :else (str ";(function(){\n" js "\n})();\n"))
    js))

(defn add-source-map-link
  [{:keys [source-map output-to], :as opts} js]
  (if source-map
    (if (= output-to :print)
...
(defn absolute-parent [path] (.getParent (.getAbsoluteFile (io/file path))))
...
(defn check-output-to
  [{:keys [output-to], :as opts}]
  (when (contains? opts :output-to)
    (assert (or (string? output-to) (= :print output-to))
            (format ":output-to %s must specify a file or be :print"
                    (pr-str output-to))))
  true)

(defn check-output-dir
...
(defn check-source-map
  ...
  (when (and (contains? opts :source-map)
    ...
    (assert
      (and (or (contains? opts :output-to) (contains? opts :modules))
  ...
  (when main
    ...
    (when (symbol? main)
      (assert (not (string/starts-with? (str main) "'"))
              (format ":main must be an unquoted symbol, got %s instead"
                      main)))))

(defn check-preloads
  [{:keys [preloads optimizations], :as opts}]
...
(defn check-cache-analysis-format
  [{:keys [cache-analysis cache-analysis-format], :as opts}]
  (assert
    (not (and cache-analysis
...
(defn expand-libs
  ...
  (letfn [(prep-path [p root]
                     (subs (string/replace (subs p 0 (- (count p) 3)) root "")
                           1))
            (path->provides [p]
          ...
          (expand-lib*
            ...
            (if-not file
              ...
              (let [root (.getAbsolutePath (io/file file))
                ...
                (if (.isDirectory dir)
                  (into []
                        (comp (filter #(.endsWith (.getName ^File %) ".js"))
...
(defn compute-upstream-npm-deps
  ([]
   (compute-upstream-npm-deps (when env/*compiler* (:options @env/*compiler*))))
  ([{:keys [npm-deps]}]
   (let [{ups-npm-deps :npm-deps} (get-upstream-deps)]
     (reduce (fn [m [dep v]]
               (cond-> m
...
(defn ensure-module-opts
  [opts]
  (update opts :modules #(ensure-cljs-base-module % opts)))

(defn shim-process?
  [{:keys [target process-shim], :as opts}]
  (if (= :nodejs target) (true? process-shim) (not (false? process-shim))))

(defn normalize-closure-defines
  [defines]
  (into {}
        (map (fn [[k v]] [(if (symbol? k) (str (comp/munge k)) k) v]) defines)))
  ...
  (let [opts
          (cond-> opts
            (shim-process? opts)
              (-> (update-in [:preloads] (fnil conj []) 'process.env)
                  (cond-> (not= :none optimizations)
                            (update-in [:closure-defines 'process.env/NODE_ENV]
                                       (fnil str "production"))))
            (or (:closure-defines opts) (shim-process? opts))
              (update :closure-defines normalize-closure-defines)
            (:browser-repl opts) (update-in [:preloads]
                                            (fnil conj [])
                                            'clojure.browser.repl.preload)
            (and (contains? opts :modules) (not (contains? opts :stable-names)))
    ...
    (cond-> (-> opts
                (assoc :optimizations optimizations
                       ...
                       :cache-analysis-format (:cache-analysis-format opts
                                                                      :transit))
                (update-in [:preamble] #(into (or % []) ["cljs/imul.js"])))
      (:target opts) (assoc-in [:closure-defines
                                (str (comp/munge 'cljs.core/*target*))]
                       (name (:target opts)))
      (= optimizations :none) (assoc :cache-analysis
                                (:cache-analysis opts true) :source-map
                                (:source-map opts true))
      (:aot-cache opts) (assoc :cache-analysis true)
      (= optimizations :advanced) (cond-> (not (false? (:static-fns opts)))
      ...
      (nil? (:closure-module-roots opts)) (assoc :closure-module-roots [])
      (nil? (:opts-cache opts)) (assoc :opts-cache "cljsc_opts.edn")
      (not (contains? opts :aot-cache)) (assoc :aot-cache false)
      (not (contains? opts :npm-deps)) (assoc :npm-deps false)
      (contains? opts :modules) (ensure-module-opts)
      (:stable-names opts)
        (as-> opts
          (let [out-dir (if (true? (:stable-names opts))
            ...
            (merge
              ...
              opts)))
      (nil? (:ignore-js-module-exts opts)) (assoc :ignore-js-module-exts
                                             [".css"]))))
...
(defn- pipe
  [^Process proc in ^Writer out]
  ;; we really do want system-default encoding here
  (with-open [^java.io.Reader in (-> in
                                     InputStreamReader.
                                     BufferedReader.)]
    (loop [buf (char-array 1024)]
      (when (alive? proc)
        (try (let [len (.read in buf)]
    ...
    (when-not (empty? npm-deps)
      (let [pkg-json (io/file "package.json")]
        ...
        (let [proc
              ...
              _ (do (.start (Thread. (bound-fn [] (pipe proc is iw))))
...
(defn node-module-deps
  "EXPERIMENTAL: return the foreign libs entries as computed by running
   ...
   installed."
  ([entry]
   (node-module-deps entry (when env/*compiler* (:options @env/*compiler*))))
     ...
     (if (zero? err)
       (into []
             (map (fn [{:strs [file provides]}]
                    file
                    (merge {:file file,
                            ;; Just tag everything es6 here,
                            ;; add-converted-source will
                            ;; ask the real type, CJS/ES6, from Closure.
                            :module-type :es6}
                           (when provides {:provides provides}))))
             (next (json/read-str (str iw))))
       (do (when-not (.isAlive proc) (binding [*out* *err*] (println (str ew))))
           [])))))
...
(defn node-inputs
  "EXPERIMENTAL: return the foreign libs entries as computed by running
   ...
   installed."
  ([entries]
   (node-inputs entries (when env/*compiler* (:options @env/*compiler*))))
  ([entries opts]
...
(defn index-node-modules
  ...
  ([modules opts]
   (let [node-modules (io/file "node_modules")]
     (if (and (not (empty? modules))
              (.exists node-modules)
              (.isDirectory node-modules))
       (let [modules (into #{} (map name) modules)
             deps-file (io/file (util/output-directory opts)
                   ...
                   (when-not (package-json? path)
                     (let [pkg-json-main
                                     ...
                                     (some
                                       (fn [candidate]
                                         (when (= candidate
                                           ...
                                           name))
                                       (cond-> [main-path]
                                         (not (or (string/ends-with? main-path
                                                                     ".js")
                                                  (string/ends-with? main-path
                                                                     ".json")))
                                           (into [(str main-path ".js")
                       ...
                       {:provides
                          (let [module-rel-name
                                  (-> (subs path
                                            (.lastIndexOf path "node_modules"))
                                      (string/replace \\ \/)
                                      (string/replace #"node_modules[\\\/]" ""))
                                provides
                                  (cond-> [module-rel-name
                                           (string/replace module-rel-name
                                                           #"\.js(on)?$"
                                                           "")]
                                ...
                                index-replaced (string/replace
                                                 ...
                                                 "")]
                            (cond-> provides
                              (and (boolean (re-find #"[\\\/]index\.js(on)?$"
                                                     module-rel-name))
...
(defn index-node-modules-dir
  ([] (index-node-modules-dir (when env/*compiler* (:options @env/*compiler*))))
  ([opts]
...
(defn preprocess-js
  ...
  (cond
    ...
    (symbol? preprocess)
      (let [preprocess-var
              (sym->var preprocess :preprocess {:file (:file js-module)})]
        (try (preprocess-var js-module opts)
             (catch Throwable t
               (throw (ex-info (str "Error running preprocessing function "
                                    preprocess)
                               {:file (:file js-module), :preprocess preprocess}
                               t)))))
    :else
      (do (ana/warning :unsupported-preprocess-value @env/*compiler* js-module)
          js-module)))
...
(defn process-js-modules
  ...
  [opts]
  (let [;; Modules from both :foreign-libs (compiler options) and
        ;; :ups-foreign-libs (deps.cljs)
        ;; are processed together, so that files from both sources can depend on
        ;; each other.
        ;; e.g. commonjs module in :foreign-libs can depend on commonjs module
        ;; from :ups-foreign-libs.
        js-modules (filter :module-type
                     (concat (:foreign-libs opts) (:ups-foreign-libs opts)))]
    (if (seq js-modules)
      (util/measure
        (:compiler-stats opts)
        "Process JS modules"
        (let [_ (when-let [unsupported (first (filter (complement #{:es6
                                                                    :commonjs})
                                                (map :module-type js-modules)))]
                  (ana/warning :unsupported-js-module-type
                               @env/*compiler*
                               unsupported))
              ;; Load all modules - add :source so preprocessing and conversion
              ;; can access it
              js-modules
                (into
                  []
                  (comp
                    ...
                    (map (fn [js]
                           (if (:preprocess js) (preprocess-js js opts) js)))
                    (map (fn [js]
          ...
          (reduce
            (fn [new-opts {:keys [file module-type], :as ijs}]
              (let [ijs (write-javascript opts ijs)
                    module-name (-> (deps/load-library (:out-file ijs))
                                    first
                                    :provides
                                    first)]
...
(defn get-data-readers*
  ...
  ([classloader]
   (let [data-reader-urls (enumeration-seq
                            (. classloader (getResources "data_readers.cljc")))]
...
(defn load-data-readers!
  ...
  (let [data-readers (get-data-readers)
    ...
    (swap! compiler update-in
      ...
      (get-data-readers))
    (doseq [ns nses]
      (try (locking ana/load-mutex (require ns)) (catch Throwable _)))))

(defn add-externs-sources
  [opts]
  (cond-> opts
    (:infer-externs opts) (assoc :externs-sources
...
(defn handle-js-modules
  ...
  (let [top-level
          (reduce (fn [acc m]
            ...
            {}
            ;; if :npm-deps option is false, node_modules/ dir shouldn't be
            ;; indexed
            (if (not (false? npm-deps)) (index-node-modules-dir)))
        ...
        opts (update
               ...
               (fn [libs]
                 (into
                   (if (= target :nodejs) [] (index-node-modules node-required))
                   (into expanded-libs
    ...
    (swap! compiler-env (fn [cenv]
                          (-> cenv
                              ;; we need to also track the whole top level -
                              ;; this is to support
                              ;; cljs.analyze/analyze-deps, particularly in REPL
                              ;; contexts - David
                              (merge {:js-dependency-index
                                        (deps/js-dependency-index opts)})
                              (update-in [:options] merge opts)
                              (update-in [:node-module-index]
                                         (fnil into #{})
                                         (if (= target :nodejs)
...
(defn output-bootstrap
  ...
  (when (and (#{:nodejs :nashorn :graaljs} target)
    ...
    (let [target-str (name target)
      ...
      (util/mkdirs outfile)
      (spit outfile
            (slurp (io/resource (str "cljs/bootstrap_" target-str ".js")))))))
  ...
  ([source opts compiler-env]
   (env/with-compiler-env
     ...
     (let [orig-opts opts
           ...
           ;; we want to warn about NPM dep conflicts before installing the
           ;; modules
           _ (when (:install-deps opts)
               (check-npm-deps opts)
               (swap! compiler-env update-in
                 [:npm-deps-installed?]
                 (fn [installed?]
                   (if-not installed?
           ...
           checked-arrays (or (:checked-arrays opts) ana/*checked-arrays*)
       ...
       (swap! compiler-env #(-> %
                                ...
                                ;; Save the current js-dependency index once we
                                ;; have computed opts
                                ;; or the analyzer won't be able to find
                                ;; upstream dependencies - Antonio
                                (assoc :js-dependency-index
                                         (deps/js-dependency-index opts))
                                ;; Save list of sources for
                                ;; cljs.analyzer/locate-src - Juho Teperi
                                (assoc :sources sources)))
       (binding [comp/*recompiled*
                   (when-not (false? (:recompile-dependents opts)) (atom #{}))
                 ...
                 ana/*cljs-warnings*
                   (let [warnings (opts :warnings true)]
                     (merge ana/*cljs-warnings*
                            (if (or (true? warnings) (false? warnings))
                              (zipmap [:unprovided :undeclared-var
                                       :undeclared-ns :undeclared-ns-form]
                                      (repeat warnings))
                              warnings)))
                 ana/*verbose* (:verbose opts)]
         (when ana/*verbose*
           (util/debug-prn "Options passed to ClojureScript compiler:"
                           (pr-str opts)))
         (let
           [one-file? (and (:main opts)
            ...
            source (if one-file?
                     (let [main (:main opts)
                       ...
                       (assert uri
                               (str "No file for namespace " main " exists"))
                       uri)
                     source)
            compile-opts
              (if one-file? (assoc opts :output-file (:output-to opts)) opts)
            _ (load-data-readers! compiler-env)
            ;; reset :js-module-index so that ana/parse-ns called by
            ;; -find-sources
            ;; can find the missing JS modules
            js-sources (env/with-compiler-env
                         (dissoc @compiler-env :js-module-index)
                         (-> (if source
                                 (-find-sources source opts)
                                 (-find-sources (reduce into
                                                  #{}
                                                  (map (comp :entries val)
                                                    (:modules opts)))
                                                opts))
            ...
            _ (spit (io/file (util/output-directory opts) (:opts-cache opts))
                    (pr-str orig-opts))
...
(comment ;; testing modules
         (build "samples/hello/src"
                {:optimizations :advanced,
                 ...
                 :source-map true,
                 :modules {:hello {:output-to "samples/hello/out/hello.js",
                                   :entries '#{cljs.reader hello.core}}}})
         (require '[cljs.externs :as externs])
         (externs/parse-externs (js-source-file
                                  "cljs/externs.js"
                                  (io/file "src/main/cljs/cljs/externs.js"))))

(defn ^File target-file-for-cljs-ns
  [ns-sym output-dir]
  (util/to-target-file (util/output-directory {:output-dir output-dir})
                       {:ns ns-sym}))
...
(defn cljs-dependents-for-macro-namespaces
  ...
  (map :name
    (let [namespaces-set (set namespaces)]
      (filter (fn [x]
                (not-empty (set/intersection namespaces-set
                                             (-> x
                                                 :require-macros
                                                 vals
                                                 set))))
...
(defn watch
  ...
  ([source opts compiler-env quit]
   (let [opts (cond-> opts
                (= (:verbose opts :not-found) :not-found) (assoc :verbose true))
               ...
               (try
                 (let [start (System/nanoTime)]
                   ...
                   (println "... done. Elapsed"
                            (/ (unchecked-subtract (System/nanoTime) start) 1e9)
                            "seconds")
                   (flush))
                 ...
                 (Files/walkFileTree
                   ...
                   (reify
                     FileVisitor
                       ...
                       (postVisitDirectory [_ dir exc] FileVisitResult/CONTINUE)
             ...
             (when
               ...
               (when-let [clj-files
                 ...
                 (let [^Path dir (.watchable key)
                   ...
                   (doseq [ns (cljs-dependents-for-macro-namespaces compiler-env
                                                                    nses)]
...
(comment (watch "samples/hello/src"
                {:optimizations :none,
...
(defn ^File src-file->target-file
  ([src]
   (src-file->target-file src (when env/*compiler* (:options @env/*compiler*))))
  ([src opts]
   (util/to-target-file (when (:output-dir opts) (util/output-directory opts))
                        (ana/parse-ns src))))

(defn ^String src-file->goog-require
  ([src] (src-file->goog-require src {:wrap true}))
  ([src {:keys [wrap all-provides macros-ns], :as options}]
   (let [goog-ns (case (util/ext src)
                   ("cljs" "cljc")
                     (let [ns-str (str (comp/munge (:ns (ana/parse-ns src))))]
                       (cond-> ns-str
                   ...
                   (throw (IllegalArgumentException.
                            (str "Can't create goog.require expression for "
                                 src))))]
  ...
  (let [copts (select-keys opts [:optimizations :output-dir])]
    ...
    (build
      '[(ns clojure.browser.repl.client
        ...
        (defn start
          [url]
          (event/listen js/window "load" (fn [] (repl/start-evaluator url))))]
      copts
      (env/default-compiler-env copts))))

(defn create-client-js-file
  [opts file-path]
...
(defn aot-cache-core
  ...
  (let [base-path (io/file "src" "main" "cljs" "cljs")
    ...
    (create-client-js-file {:optimizations :simple, :output-dir "aot_out"}
                           (io/file "resources" "brepl_client.js"))
    (doseq [f (file-seq (io/file "aot_out")) :when (.isFile f)] (.delete f))))

(comment
  (time (do (aot-cache-core) nil))
  (time (do (ana/analyze-file "cljs/core.cljs") nil))
  (println
    (build '[(ns hello.core) (defn ^{:export greet} greet [n] (str "Hola " n))
  ...
  (build "samples/hello/src"
         {:optimizations :advanced, :output-to "samples/hello/hello.js"})
