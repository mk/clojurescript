...
(defn close-socket [s] (.close (:in s)) (.close (:out s)) (.close (:socket s)))

(defn write
  [^BufferedWriter out ^String js]
  (.write out js)
  (.write out (int 0)) ;; terminator
  (.flush out))

(defn ^String read-response
  [^BufferedReader in]
  (let [sb (StringBuilder.)]
    (loop [sb sb
      ...
      (case c
        ...
        0 (str sb)
        (do (.append sb (char c)) (recur sb (.read in)))))))
...
(defn node-eval
  ...
  (let [tname (thread-name)
    ...
    (let [result (.take ^LinkedBlockingQueue (.get results tname))]
      (condp = (:status result)
        "success" {:status :success, :value (:value result)}
        "exception" {:status :exception, :value (:value result)}))))
...
(defn platform-path [v] (str "path.join.apply(null, " (seq->js-array v) ")"))
...
(defn- event-loop
  ...
  (while
    (alive? proc)
    (try (let [res (read-response in)]
           (try (let [{:keys [type repl value], :or {repl "main"}, :as event}
                        (json/read-str res :key-fn keyword)]
                  (case type
                    "result" (.offer (.get results repl) event)
...
(defn- build-process
  ...
  (let [xs (cond-> [(get opts :node-command "node")]
             (:debug-port repl-env) (conj (str "--inspect="
                                               (:debug-port repl-env))))
        proc (-> (ProcessBuilder. (into-array xs))
                 (.redirectInput input-src))]
   ...
   (locking
     ...
     (when-not @socket
       (let [output-dir (io/file (util/output-directory opts))
             ...
             _ (spit of
                     (string/replace (slurp (io/resource
                                              "cljs/repl/node_repl.js"))
                                     "var PORT = 5001;"
             ...
             [fc & cs] (rest (util/path-seq path)) ;; remove leading empty
                                                   ;; string
         ...
         (loop [r nil]
           (when-not (= r "ready")
             (Thread/sleep 50)
             (try (reset! socket (create-socket host port)) (catch Exception e))
             (if @socket (recur (read-response (:in @socket))) (recur nil))))
         (.start (Thread. (bound-fn [] (event-loop proc (:in @socket)))))
         ;; compile cljs.core & its dependencies, goog/base.js must be available
         ;; for bootstrap to load, use new closure/compile as it can handle
         ;; resources in JARs
         (let [core-js (closure/compile
                         core
                         (assoc opts
                           :output-file (closure/src-file->target-file
                                          core
                  ...
                  (fn [name]
                    (js/CLOSURE_IMPORT_SCRIPT
                      (if (some? goog/debugLoader_)
                        ...
                        (unchecked-get (.. js/goog -dependencies_ -nameToPath)
                                       name))))))
         ;; load cljs.core, setup printing
         (repl/evaluate-form repl-env
                             env
                             "<cljs repl>"
                             '(do (.require js/goog "cljs.core")
...
(defrecord NodeEnv [host port path socket proc state]
  repl/IReplEnvOptions
    (-repl-options [this] {:output-dir ".cljs_node_repl", :target :nodejs})
  repl/IParseError
    (-parse-error [_ err _] (assoc err :value nil))
  repl/IJavaScriptEnv
...
(defn repl-env*
  ...
  (let [{:keys [host port path debug-port]}
          (merge {:host "localhost", :port (+ 49000 (rand-int 10000))} options)]
    (assoc (NodeEnv. host port path (atom nil) (atom nil) (atom {:listeners 0}))
