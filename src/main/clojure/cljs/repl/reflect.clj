...
(defn- dissoc-unless
  "Dissoc all keys from map that do not appear in key-set.
    ...
    => {:foo 1}"
  [m key-set]
  {:pre [(map? m) (set? key-set)]}
  (reduce (fn [coll key] (if (contains? key-set key) coll (dissoc coll key)))
    m
    (keys m)))
...
(defn- get-meta
  ...
  (let [ns (symbol (namespace sym))
    ...
    (if-let [sym-meta (get (:defs (get @analyzer/namespaces ns)) n)]
      (-> (dissoc-unless sym-meta #{:name :method-params :doc :line :file})
          (update-in [:name] str)
...
(defn macroexpand
  ...
  (let [mform (analyzer/macroexpand-1 {} form)]
    (if (identical? form mform) mform (macroexpand mform))))
...
(defn- compile-and-return
  ...
  (let [ast (analyzer/analyze {:ns {:name 'cljs.user}} form)
        js (try (compiler/emit-str ast) (catch Exception e (println e)))]
...
(defmethod handle-reflect-query "var"
  [[_ sym] req conn opts]
  (let [sym (read-url-string sym)] (compile-and-return conn (get-meta sym))))
