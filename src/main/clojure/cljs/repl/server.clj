...
(defn dispatch-on
  ...
  ([method pred handler] (dispatch-on method {:pred pred, :handler handler}))
  ([method {:as m}]
   (swap! handlers (fn [old] (update-in old [method] #(conj (vec %) m))))))
...
(defn parse-file-parts
  ...
  (let [ref-idx (str/index-of file "#")
    ...
    (merge {:ref ref}
           (if q-idx
             {:path (subs file 0 q-idx), :query-str (subs file (inc q-idx))}
             {:path file}))))
...
(defn ^bytes gzip
  ...
  (let [baos (ByteArrayOutputStream. (count bytes))]
    (try (let [gzos (GZIPOutputStream. baos)]
           (try (.write gzos bytes) (finally (.close gzos))))
         (finally (.close baos)))
    (.toByteArray baos)))
...
(defn send-and-close
  ...
  ([conn status form] (send-and-close conn status form "text/html"))
  ...
  ([conn status form content-type encoding gzip?]
   (let [byte-form (cond-> (.getBytes form encoding) gzip? gzip)
         ...
         headers (map #(.getBytes (str % "\r\n"))
                   (cond-> [(status-line status) "Server: ClojureScript REPL"
     ...
     (with-open [os (.getOutputStream conn)]
       (doseq [header headers] (.write os header 0 (count header)))
...
(defn send-404
  ...
  (send-and-close conn
                  404
                  (str "<html><body>" "<h2>Page not found</h2>"
                       "No page " path
...
(defn- dispatch-request
  ...
  (if-let [handlers ((:method request) @handlers)]
    (if-let [handler (some (fn [{:keys [pred handler]}]
                             (when (pred request conn opts) handler))
                           handlers)]
...
(defn- server-loop
  ...
  (when-let [conn (try (.accept server-socket) (catch Throwable _))]
    ...
    (.start (Thread. ((ns-resolve 'clojure.core 'binding-conveyor-fn)
...
(defn stop
  []
  (when-let [sock (:socket @state)] (when-not (.isClosed sock) (.close sock))))
