...
(defn send-for-eval
  ...
  ([conn form return-value-fn]
   ...
   (server/send-and-close conn
                          200
                          (json/write-str {"repl" (thread-name), "form" form})
                          "application/json")))

(defn- return-value
  "Called by the server when a return value is received."
  [val]
  (when-let [f (:return-value-fn @browser-state)] (f val)))

(defn repl-client-js [] (slurp (:client-js @browser-state)))
...
(defn send-repl-client-page
  ...
  (server/send-and-close
    ...
    (str
      ...
      "</script>"
      "<script type=\"text/javascript\">
          clojure.browser.repl.client.start(\"http://"
        (-> request
            :headers
            :host)
      "\");
...
(defn default-index
  ...
  (str
    ...
    "  &lt;/body&gt;\n" "&lt;/html&gt;\n"
    "</pre>" "</div></div>"
    "<script src=\"" output-to
    "\"></script>" "</body></html>"))

(defn- path->mime-type
  [ext->mime-type path default]
      ...
      (let [path (if (= "/" path) "/index.html" path)
        ...
        (cond
          ...
          (= path "/index.html")
            (server/send-and-close
              conn
              200 (default-index (or output-to (str output-dir "/main.js")))
              "text/html" "UTF-8")
              ...
              (server/send-and-close
                ...
                200
                  (str
                    ...
                    output-dir
                    "/goog/base.js\"></script>');\n"
                    "document.write('<script src=\"" output-dir
                    "/goog/deps.js\"></script>');\n"
...
(server/dispatch-on :get
                    (fn [{:keys [path]} _ _]
                      (or (= path "/")
                          (path->mime-type ext->mime-type path nil)))
                    send-static)
...
(defmethod handle-post :ready
  ...
  (locking server/lock (.clear server/connq))
  (send-for-eval
    conn
    (binding [ana/*cljs-warnings* (assoc ana/*cljs-warnings*
                                    :undeclared-var false)]
      (cljsc/-compile '[(set! *print-fn* clojure.browser.repl/repl-print)
...
(defn run-in-order
  ...
  (loop [order expecting
    ...
    (if-let [f (get fns order)]
      (do (f) (recur (inc order) (dissoc fns order)))
...
(defmethod handle-post :print
  [{:keys [repl content order]} conn _]
...
(defmethod handle-post :result
  ...
  (constrain-order order
                   (fn [] (return-value content) (server/set-connection conn))))
...
(defn browser-eval
  ...
  (let [return-value (promise)]
    (send-for-eval form (fn [val] (deliver return-value val)))
...
(defn- waiting-to-connect-message
  [url]
...
(defn setup
  [{:keys [working-dir launch-browser server-state], :as repl-env}
    ...
    (when-not (:socket @server-state)
      (binding [browser-state (:browser-state repl-env)
        ...
        (swap! browser-state
          (fn [old]
            (assoc old
              :client-js (cljsc/create-client-js-file
                           {:optimizations :simple, :output-dir working-dir}
...
(defrecord BrowserEnv []
  repl/IJavaScriptEnv
    (-setup [this opts] (setup this opts))
    (-evaluate [this _ _ js]
      (binding [browser-state (:browser-state this)
        ...
        (browser-eval js)))
    (-load [this provides url] (load-javascript this provides url))
    (-tear-down [this]
      ...
      (let [server-state (:server-state this)]
        (when (zero? (:listeners (swap! server-state update :listeners dec)))
          ...
          (when-not (.isShutdown (:es this)) (.shutdownNow (:es this))))))
  repl/IReplEnvOptions
    (-repl-options [this]
      {:browser-repl true,
       :repl-requires '[[clojure.browser.repl] [clojure.browser.repl.preload]],
       :cljs.cli/commands
         {:groups {::repl {:desc "browser REPL options"}},
          :init {["-H" "--host"] {:group ::repl,
                                  :fn
                                    #(assoc-in %1 [:repl-env-options :host] %2),
                                  :arg "address",
                                  :doc "Address to bind"},
                 ["-p" "--port"] {:group ::repl,
                                  :fn #(assoc-in %1
                                        [:repl-env-options :port]
                                        (Integer/parseInt %2)),
                                  :arg "number",
                                  :doc "Port to bind"}}}})
...
(defn repl-env*
  ...
  (merge (BrowserEnv.)
         {:host host,
          ...
          :preloaded-libs [],
          :src "src/",
          :browser-state (atom {:return-value-fn nil, :client-js nil}),
          :ordering (agent {:expecting nil, :fns {}}),
          :es (Executors/newFixedThreadPool 16),
          :server-state (atom {:socket nil, :listeners 0})}
         opts))
...
(defn -main [& args] (apply cli/main repl-env args))

(comment (require '[cljs.repl :as repl])
