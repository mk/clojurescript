...
(ns cljs.repl.rhino
  ...
  (:import [java.io File Reader]
           [org.mozilla.javascript Context ScriptableObject RhinoException
            Undefined]))

(def ^String bootjs
  (str "var global = this;\n" "var CLOSURE_IMPORT_SCRIPT = function(src) {\n"
       "    var ns = \"cljs.repl.rhino\"," "        name = \"load-file\","
...
(defmethod stacktrace :default
  [e]
  (apply str
    (interpose "\n" (map #(str "        " (.toString %)) (.getStackTrace e)))))

(defmethod stacktrace RhinoException
  [^RhinoException e]
...
(defn rhino-eval
  ...
  (try (let [linenum (or line Integer/MIN_VALUE)]
         {:status :success,
       ...
       (catch Throwable ex
         ...
         (let [top-level (-> scope
           ...
           (ScriptableObject/putProperty top-level
                                         "_STAR_e"
...
(defn load-javascript
  [repl-env ns url]
  (try (with-open [reader (io/reader url)]
...
(defn rhino-setup
  ...
  (let [scope (:scope repl-env)
    ...
    (apply closure/output-unoptimized
      (assoc opts :output-to (.getPath repl-deps))
      deps)
    ;; setup back references & output stream
    (ScriptableObject/putProperty scope
                                  "___repl_env"
                                  (Context/javaToJS repl-env scope))
    (ScriptableObject/putProperty scope
                                  "__repl_opts"
                 ...
                 (when (or (not (contains? *loaded-libs* name)) reload)
                   ...
                   (js/CLOSURE_IMPORT_SCRIPT
                     (if (some? goog/debugLoader_)
                       ...
                       (goog.object/get (.. js/goog -dependencies_ -nameToPath)
                                        name))))))))
    ;; set closure-defines
    (rhino-eval repl-env
...
(defrecord RhinoEnv []
  repl/IReplEnvOptions
    (-repl-options [this] {:output-dir ".cljs_rhino_repl", :wrap wrap-fn})
  repl/IParseStacktrace
    (-parse-stacktrace [this frames-str ret opts]
      (st/parse-stacktrace this frames-str (assoc ret :ua-product :rhino) opts))
  repl/IGetError
    (-get-error [this e env opts]
      (let [{:keys [scope]} this
        ...
        {:status :exception,
         ...
         :stacktrace (stacktrace ex)}))
  repl/IJavaScriptEnv
    (-setup [this opts] (rhino-setup this opts))
    (-evaluate [this filename line js] (rhino-eval this filename line js))
...
(defn repl-env*
  ...
  (let [cx (Context/enter)]
    ...
    (merge (RhinoEnv.) {:cx cx, :scope (.initStandardObjects cx)})))
