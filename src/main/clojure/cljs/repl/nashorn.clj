...
(util/compile-if
  (Class/forName "jdk.nashorn.api.scripting.NashornException")
  (do
    ...
    (defn create-engine
      ...
      ([{:keys [code-cache], :or {code-cache true}}]
       (let [args (when code-cache ["-pcc"])
             ...
             factory (get (zipmap (map #(.getEngineName %) factories) factories)
                          "Oracle Nashorn")]
    ...
    (defn eval-str [^ScriptEngine engine ^String s] (.eval engine s))
    (defn eval-resource
    ...
    (defn init-engine
      [engine {:keys [output-dir], :as opts} debug]
      ...
      engine)
    (defn tear-down-engine [engine] (eval-str engine "nashorn_tear_down();"))
    (defn load-js-file
      [engine file]
    ...
    (defn bootstrap-repl
      [engine output-dir opts]
      (env/ensure
        (let [deps-file ".nashorn_repl_deps.js"
              core (io/resource "cljs/core.cljs")
              core-js (closure/compile
                        core
                        (assoc opts
    ...
    ;; Nashorn script stacktraces have a relative path which includes the
    ;; output-dir
    (defn- strip-file-name
      [^String file-name output-dir]
    ...
    (defrecord NashornEnv [engine debug]
      repl/IReplEnvOptions
        (-repl-options [this]
          {:output-dir ".cljs_nashorn_repl", :target :nashorn})
      repl/IJavaScriptEnv
               ...
               (catch ScriptException e
                 (let [^Throwable root-cause (clojure.stacktrace/root-cause e)]
                   {:status :exception,
                    ...
                    :stacktrace (NashornException/getScriptStackString
                                  root-cause)}))
               (catch Throwable e
                 (let [^Throwable root-cause (clojure.stacktrace/root-cause e)]
                   {:status :exception,
                    :value (.getMessage root-cause),
                    :stacktrace (apply str
                                  (interpose "\n"
                                    (map str (.getStackTrace root-cause))))}))))
        (-load [{engine :engine, :as this} ns url] (load-ns engine ns))
        (-tear-down [this] (tear-down-engine engine))
      repl/IParseStacktrace
        (-parse-stacktrace [this frames-str ret opts]
          (st/parse-stacktrace this
                               frames-str
                               (assoc ret :ua-product :nashorn)
                               opts))
      repl/IParseError
        (-parse-error [_ err _]
          (update-in err
    ...
    (defn repl-env*
      ...
      (let [engine (create-engine opts)]
        (merge (NashornEnv. engine debug) opts)))
    (defn repl-env
  ...
  (do
    ...
    (defn repl-env
      "Create a Nashorn repl-env for use with the repl/repl* method in Clojurescript."
      [& {:as opts}]
      (throw (ex-info "Nashorn not available under this Java runtime"
                      {:type :repl-error})))
    (defn -main
      []
      (throw (ex-info "Nashorn not available under this Java runtime"
