...
(defn- js-opt-key? [k] (and (string? k) (string/starts-with? k "js.")))

(defn- form-js-opts
  [opts]
  (for [[k v] opts :when (js-opt-key? k)] `(.option ~k ~v)))
...
(defn create-engine
  ...
  (let [engine (eval `(GraalJSScriptEngine/create
                        nil
...
(defn tear-down-engine [engine] (eval-str engine "graaljs_tear_down();"))
...
(defn- safe-to-string
  ...
  (let [type-str (pr-str (type x))]
    (try
      (if (contains? skip-types type-str) (str #"<" type-str ">") (.toString x))
      (catch StackOverflowError _ (str "#<stackoverflow " type-str ">")))))

(defrecord GraalJSEnv [engine debug]
  repl/IReplEnvOptions
    (-repl-options [this] {:output-dir ".cljs_graaljs_repl", :target :graaljs})
  repl/IJavaScriptEnv
...
(def ^:private default-js-opts {"js.timer-resolution" "1"})

(defn repl-env*
  [{:keys [debug], :as opts}]
  (let [opts (merge default-js-opts opts)
        engine (create-engine opts)]
    (merge (GraalJSEnv. engine debug) opts)))
