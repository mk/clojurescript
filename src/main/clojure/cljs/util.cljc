...
(defn- main-src-directory
  []
  (some (fn [file] (when (= "main" (.getName file)) file))
...
(defn ^String compiled-by-version
  [f]
  (with-open [reader (io/reader f)]
    (let [match (some->> reader
                         line-seq
                         first
...
(defn ns->relpath
  ...
  ([ns ext sep]
   (cond-> (string/replace (munge-path ns) \. sep) ext (str "." (name ext)))))
...
(defn ^File to-target-file
  ...
  ([target-dir {:keys [ns source-file], :as ns-info} ext]
   (let [src-ext (if source-file (cljs.util/ext source-file) "cljs")
         ...
         parents (cond-> (butlast relpath) target-dir (conj target-dir))]
...
(defn ^String filename [^File f] (.getName f))
...
(defn ^String path
  [x]
  (cond (file? x) (.getAbsolutePath ^File x)
        (url? x) (if windows?
                   (let [f (URLDecoder/decode (.getFile x))] (normalize-path f))
                   (.getPath ^URL x))
        (string? x) x
        :else (throw (Exception. (str "Expected file, url, or string. Got "
                                      (pr-str x))))))

(defn ^String ext
  "Given a file, url or string return the file extension."
  [x]
  (let [s (cond (file? x) (filename x)
                (url? x) (path x)
                (string? x) x
...
(defn last-modified
  [src]
  (cond (file? src) (.lastModified ^File src)
        (url? src) (let [conn (.openConnection ^URL src)]
                     (try (.getLastModified conn)
                          (finally (let [ins (.getInputStream conn)]
                                     (when ins (.close ins))))))
...
(defmacro compile-if
  ...
  ([exp then else]
   (if (try (eval exp) (catch Throwable _ false)) `(do ~then) `(do ~else))))

(defmacro compile-when [exp then] `(compile-if ~exp ~then nil))
...
(defn unknown-opts
  ...
  (for [unknown (set/difference passed knowns)]
    [unknown
     (some-> (suggestion 3 (str unknown) (map str knowns))
             (subs 1)
             keyword)]))
  ...
  (loop [index (int 0)
    ...
    (if (== (alength bytes) index)
      ...
      (let [byte (aget bytes index)]
        (.append buffer
                 (aget ^chars hex-digits
                       (bit-and (bit-shift-right byte 4) 0xF)))
        (.append buffer (aget ^chars hex-digits (bit-and byte 0xF)))
        (recur (inc index) buffer)))))

(defn content-sha
  ([^String s] (content-sha s nil))
  ([^String s ^Long n]
