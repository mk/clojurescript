...
(ns cljs.externs
  ...
  (:import [java.util.logging Level]
           [com.google.javascript.jscomp CompilerOptions SourceFile JsAst
            CommandLineRunner]
           [com.google.javascript.rhino Node Token JSTypeExpression]))
...
(defmulti parse-extern-node (fn [^Node node] (.getToken node)))

(defmethod parse-extern-node Token/VAR
  [node]
...
(defmethod parse-extern-node Token/EXPR_RESULT
  [node]
  (when (> (.getChildCount node) 0) (parse-extern-node (.getFirstChild node))))

(defmethod parse-extern-node Token/ASSIGN
  [node]
  (when (> (.getChildCount node) 0)
    (let [ty (get-type node)
...
(defmethod parse-extern-node Token/NAME
  [node]
...
(defmethod parse-extern-node Token/GETPROP
  [node]
...
(defmethod parse-extern-node Token/OBJECTLIT
  [node]
  (when (> (.getChildCount node) 0)
...
(defmethod parse-extern-node Token/STRING_KEY
  [node]
...
(defn parse-externs
  ...
  (let [^CompilerOptions compiler-options (CompilerOptions.)
        closure-compiler
          (doto (let [compiler (com.google.javascript.jscomp.Compiler.)]
...
(defn index-externs
  [externs]
  (reduce (fn [m xs] (cond-> m (seq xs) (update-in xs merge {}))) {} externs))
...
(defn externs-map
  ...
  ([sources defaults]
   (let [sources (if-not (empty? sources)
     ...
     (reduce (fn [externs externs-file]
               (util/map-merge externs
...
(comment
  ...
  (-> (nth (parse-externs
             (closure/js-source-file "goog/string/string.js"
                                     (io/input-stream
                                       (io/resource "goog/string/string.js"))))
           2)
      last
      meta)
  (externs-map [(closure/js-source-file
  ...
  (filter (fn [s]
    ...
    (CommandLineRunner/getDefaultExterns))
  (-> (filter (fn [s] (= "externs.zip//webkit_dom.js" (.getName s)))
              (CommandLineRunner/getDefaultExterns))
      first
      parse-externs
      index-externs
  ...
  (-> (filter (fn [s] (= "externs.zip//webkit_dom.js" (.getName s)))
      ...
      first
      meta)
  (require '[clojure.java.io :as io] '[cljs.closure :as cc])
