...
(ns cljs.core
  (:refer-clojure
    :exclude
    [-> ->> .. amap and areduce alength aclone assert assert-args binding
     bound-fn case comment cond condp declare definline definterface defmethod
     defmulti defn defn- defonce defprotocol defrecord defstruct deftype delay
     destructure doseq dosync dotimes doto extend-protocol extend-type fn for
     future gen-class gen-interface if-let if-not import io! lazy-cat lazy-seq
     let letfn locking loop memfn ns or proxy proxy-super pvalues refer-clojure
     reify sync time when when-first when-let when-not while with-bindings
     with-in-str with-loading-context with-local-vars with-open with-out-str
     with-precision with-redefs satisfies? identical? true? false? number? nil?
     instance? symbol? keyword? string? str get make-array vector list hash-map
     array-map hash-set aget aset + - * / < <= > >= == zero? pos? neg? inc dec
     max min mod byte char short int long float double unchecked-byte
     unchecked-char unchecked-short unchecked-int unchecked-long unchecked-float
     unchecked-double unchecked-add unchecked-add-int unchecked-dec
     unchecked-dec-int unchecked-divide unchecked-divide-int unchecked-inc
     unchecked-inc-int unchecked-multiply unchecked-multiply-int
     unchecked-negate unchecked-negate-int unchecked-subtract
     unchecked-subtract-int unchecked-remainder-int unsigned-bit-shift-right
     bit-and bit-and-not bit-clear bit-flip bit-not bit-or bit-set bit-test
     bit-shift-left bit-shift-right bit-xor defmacro cond-> cond->> as-> some->
     some->> require use refer-clojure if-some when-some test ns-publics
     ns-imports ns-interns ns-unmap var vswap! macroexpand-1 macroexpand some?
     resolve
     #?@(:cljs [alias coercive-not coercive-not= coercive-= coercive-boolean
                truth_ js-arguments js-delete js-in js-debugger exists? divide
                js-mod unsafe-bit-and bit-shift-right-zero-fill mask bitpos
                caching-hash defcurried rfn specify! js-this this-as implements?
                array js-obj simple-benchmark gen-apply-to js-str es6-iterable
                load-file* undefined? specify copy-arguments goog-define
                js-comment js-inline-comment unsafe-cast require-macros
                use-macros gen-apply-to-simple unchecked-get unchecked-set])])
...
#?(:clj (core/defmacro import-macros
          [ns [& vars]]
          (core/let [ns (find-ns ns)
                     ...
                     defs (map
                            (core/fn [sym var]
                              (core/let [{:keys [arglists doc file line]}
                                           (meta sym)]
...
#?(:clj (import-macros clojure.core
                       [-> ->> .. assert comment cond declare defn-
                        extend-protocol fn for if-let if-not letfn memfn when
                        when-first when-let when-not while cond-> cond->> as->
                        some-> some->> if-some when-some]))
...
#?(:cljs (core/defmacro comment "Ignores body, yields nil" [& body]))
...
#?(:cljs
     (core/defmacro declare
       ...
       [& names]
       `(do ~@(map #(core/list 'def (vary-meta % assoc :declared true))
                names))))
...
(core/defmacro doto
  ...
  (core/let [gx (gensym)]
    `(let [~gx ~x]
       ~@(map (core/fn [f] (if (seq? f) `(~(first f) ~gx ~@(next f)) `(~f ~gx)))
           forms)
       ~gx)))

#?(:cljs (core/defn- parse-impls
           [specs]
           (core/loop [ret {}
                       s specs]
             (if (seq s)
...
#?(:cljs (core/defn- emit-extend-protocol
           [p specs]
...
#?(:cljs (core/defn ^{:private true} maybe-destructured
           [params body]
...
#?(:cljs
     ...
     Defines a function"
       ...
       (core/let [name (if (core/symbol? (first sigs)) (first sigs) nil)
                  ...
                  psig
                    (fn* [sig]
                      ...
                      (core/when (not (seq? sig))
                        (throw (js/Error. (core/str "Invalid signature "
                                                    sig
                                                    " should be a list"))))
                                       ...
                                       (js/Error.
                                         (if (seq? (first sigs))
                                           ...
                                           (core/str "Invalid signature "
                                                     sig
                                                     " should be a list")))))
                                 conds (core/when (core/and (next body)
                                                            (map? (first body)))
                                         (first body))
         ...
         (with-meta (if name (list* 'fn* name new-sigs) (cons 'fn* new-sigs))
           (meta &form)))))
     ...
     test, if not, yields else"
       ...
       ([bindings then else & oldform]
        (assert-args if-let
                     ...
                     (empty? oldform)
                     "1 or 2 forms after binding vector"
                     (= 2 (count bindings))
                     "exactly 2 forms in binding vector")
        (core/let [form (bindings 0)
                   tst (bindings 1)]
          `(let [temp# ~tst] (if temp# (let [~form temp#] ~then) ~else))))))
...
#?(:cljs
     ...
     in all of the definitions of the functions, as well as the body."
       {:forms '[(letfn [fnspecs*] exprs*)], :special-form true, :url nil}
       [fnspecs & body]
...
(core/defmacro memfn
  ...
  [name & args]
  (core/let [t (with-meta (gensym "target") (meta name))]
    `(fn [~t ~@args] (. ~t (~name ~@args)))))

#?(:cljs (core/defmacro when
...
#?(:cljs
     ...
     Roughly the same as (when (seq xs) (let [x (first xs)] body)) but xs is evaluated only once"
       ...
       (core/let [[x xs] bindings]
         `(when-let [xs# (seq ~xs)] (let [~x (first xs#)] ~@body)))))
...
#?(:cljs
     ...
     When test is true, evaluates body with binding-form bound to the value of test"
       ...
       (core/let [form (bindings 0)
                  tst (bindings 1)]
         `(let [temp# ~tst] (when temp# (let [~form temp#] ~@body))))))

#?(:cljs (core/defmacro when-not
...
#?(:cljs
     (core/defmacro when-some
      ...
      value of test"
       ...
       (core/let [form (bindings 0)
                  tst (bindings 1)]
         `(let [temp# ~tst] (if (nil? temp#) nil (let [~form temp#] ~@body))))))

(core/defn- ^{:dynamic true} assert-valid-fdecl
  "A good fdecl looks like (([a] ...) ([a b] ...)) near the end of defn."
  [fdecl]
  (core/when (empty? fdecl)
    (throw #?(:clj (IllegalArgumentException. "Parameter declaration missing")
  ...
  (core/let [argdecls
    ...
    (core/when bad-args
      (throw
        #?(:clj (IllegalArgumentException. (core/str "Parameter declaration \""
                                                     (first bad-args)
                                                     "\" should be a vector"))
           :cljs (js/Error. (core/str "Parameter declaration \""
                                      (first bad-args)
                                      "\" should be a vector")))))))

(def ^{:private true} sigs
  (core/fn [fdecl]
    ...
    (core/let [asig
                 (core/fn [fdecl]
                   (core/let [arglist (first fdecl)
                              ...
                              arglist
                                (if #?(:clj (clojure.lang.Util/equals
                                              '&form
                                              (first arglist))
                                       :cljs (= '&form (first arglist)))
                                  #?(:clj (clojure.lang.RT/subvec
                                            arglist
                                            2
                     ...
                     (if (map? (first body))
                       (if (next body)
                         (with-meta arglist
                           (conj (if (meta arglist) (meta arglist) {})
                                 (first body)))
                         arglist)
                       arglist)))]
      (if (seq? (first fdecl))
...
(core/defmacro defonce
  ...
  [x init]
  `(when-not (exists? ~x) (def ~x ~init)))

(core/defn destructure
  [bindings]
  (core/let
                ...
                (core/let [gmap (gensym "map__")
                  ...
                  (core/loop [ret
                                (core/->
                                  ...
                                  (conj v)
                                  (conj gmap)
                                  (conj `(if (implements? ISeq ~gmap)
                                           (apply cljs.core/hash-map ~gmap)
                                           ~gmap))
                                  ((core/fn [ret]
                    ...
                    (if (seq bes)
                      (core/let [bb (key (first bes))
                                 bk (val (first bes))
                                 local (if #?(:clj (core/instance?
                                                     clojure.lang.Named
                                                     bb)
                                              :cljs (cljs.core/implements?
                                                      INamed
                                                      bb))
                                         (with-meta (symbol nil (name bb))
                                           (meta bb))
                                         bb)
                                 bv (if (contains? defaults local)
                                      (core/list 'cljs.core/get
                                                 gmap
                                                 bk
                                                 (defaults local))
                                      (core/list 'cljs.core/get gmap bk))]
                        (recur
                          (if (core/or (core/keyword? bb) (core/symbol? bb)) ;(ident?
                                                                             ;bb)
                            (core/-> ret
                                     (conj local bv))
                            (pb ret bb bv))
                      ...
                      ret))))]
           (core/cond
             (core/symbol? b) (core/-> bvec
                                       (conj
                                         (if (namespace b) (symbol (name b)) b))
                                       (conj v))
             (core/keyword? b) (core/-> bvec
                                        (conj (symbol (name b)))
                                        (conj v))
...
(core/defmacro goog-define
  ...
  (core/let [defname (comp/munge (core/str *ns* "/" sym))
             type (core/cond (core/string? default) "string"
                             (core/number? default) "number"
                             (core/or (core/true? default)
                                      (core/false? default))
                               "boolean")]
      ...
      (core/let [vs (take-nth 2 (drop 1 bindings))
        ...
        `(let ~bfs
              (loop* ~(vec (interleave gs gs))
                     (let ~(vec (interleave bs gs)) ~@body)))))))

(def fast-path-protocols
  "protocol fqn -> [partition number, bit]"
  (zipmap
    (map #(symbol "cljs.core" (core/str %))
      '[IFn ICounted IEmptyableCollection ICollection IIndexed ASeq ISeq INext
        ...
        ISeqable ISequential IList IRecord IReversible ISorted IPrintWithWriter
        IWriter IPrintWithWriter IPending IWatchable IEditableCollection
        ITransientCollection ITransientAssociative ITransientMap
        ITransientVector ITransientSet IMultiFn IChunkedSeq IChunkedNext
        IComparable INamed ICloneable IAtom IReset ISwap IIterable])
    (iterate (core/fn [[p b]]
               (if (core/== 2147483648 b)
                 [(core/inc p) 1]
                 [p
                  #?(:clj (core/bit-shift-left b 1)
                     :cljs (core/* 2 b))]))
             [0 1])))
...
(core/defn- string-expr [e] (vary-meta e assoc :tag 'string))
...
(core/defn- bool-expr [e] (vary-meta e assoc :tag 'boolean))

(core/defn- simple-test-expr?
  [env ast]
  (core/and (#{:var :js-var :local :invoke :const :host-field :host-call :js
               :quote}
             (:op ast))
  ...
  ([x & next]
   (core/let [forms (concat [x] next)]
     (if (every? #(simple-test-expr? &env %)
                 (map #(cljs.analyzer/no-warn (cljs.analyzer/analyze &env %))
                   forms))
  ...
  ([x & next]
   (core/let [forms (concat [x] next)]
     (if (every? #(simple-test-expr? &env %)
       ...
       `(let [or# ~x] (if or# or# (or ~@next)))))))

(core/defmacro nil? [x] `(coercive-= ~x nil))

(core/defmacro some? [x] `(not (nil? ~x)))

(core/defmacro coercive-not [x] (bool-expr (core/list 'js* "(!~{})" x)))

(core/defmacro coercive-not=
  [x y]
  (bool-expr (core/list 'js* "(~{} != ~{})" x y)))

(core/defmacro coercive-= [x y] (bool-expr (core/list 'js* "(~{} == ~{})" x y)))

(core/defmacro coercive-boolean
  [x]
  (with-meta (core/list 'js* "~{}" x) {:tag 'boolean}))

;; internal - do not use.
(core/defmacro truth_
  [x]
  (core/assert (core/symbol? x) "x is substituted twice")
  (core/list 'js* "(~{} != null && ~{} !== false)" x x))

(core/defmacro js-arguments [] (core/list 'js* "arguments"))

(core/defmacro js-delete [obj key] (core/list 'js* "delete ~{}[~{}]" obj key))

(core/defmacro js-in [key obj] (core/list 'js* "~{} in ~{}" key obj))

(core/defmacro js-debugger
  "Emit JavaScript \"debugger;\" statement"
  []
  (core/list 'do (core/list 'js* "debugger") nil))
...
(core/defmacro true? [x] (bool-expr (core/list 'js* "~{} === true" x)))

(core/defmacro false? [x] (bool-expr (core/list 'js* "~{} === false" x)))

(core/defmacro string?
  [x]
...
(core/defmacro exists?
  ...
  (if (core/symbol? x)
    (core/let [x (core/cond-> (:name (cljs.analyzer/resolve-var &env x))
                   (= "js" (namespace x)) name)
               segs (string/split (core/str
                                    (string/replace (core/str x) "/" "."))
                                  #"\.")
               n (count segs)
               syms (map #(vary-meta (symbol "js" (string/join "." %))
                                     assoc
...
(core/defmacro identical?
  [a b]
  (bool-expr (core/list 'js* "(~{} === ~{})" a b)))

(core/defmacro instance?
  [c x]
  ...
  (bool-expr (if (clojure.core/symbol? c)
               ...
               `(let [c# ~c x# ~x] (~'js* "(~{} instanceof ~{})" x# c#)))))

(core/defmacro number?
  [x]
  (bool-expr (core/list 'js* "typeof ~{} === 'number'" x)))

(core/defmacro symbol? [x] (bool-expr `(instance? Symbol ~x)))
...
(core/defmacro ^::ana/numeric unchecked-add ([& xs] `(+ ~@xs)))

(core/defmacro ^::ana/numeric unchecked-add-int ([& xs] `(+ ~@xs)))

(core/defmacro ^::ana/numeric unchecked-dec ([x] `(dec ~x)))

(core/defmacro ^::ana/numeric unchecked-dec-int ([x] `(dec ~x)))

(core/defmacro ^::ana/numeric unchecked-divide-int ([& xs] `(/ ~@xs)))

(core/defmacro ^::ana/numeric unchecked-inc ([x] `(inc ~x)))

(core/defmacro ^::ana/numeric unchecked-inc-int ([x] `(inc ~x)))

(core/defmacro ^::ana/numeric unchecked-multiply ([& xs] `(* ~@xs)))

(core/defmacro ^::ana/numeric unchecked-multiply-int ([& xs] `(* ~@xs)))

(core/defmacro ^::ana/numeric unchecked-negate ([x] `(- ~x)))

(core/defmacro ^::ana/numeric unchecked-negate-int ([x] `(- ~x)))

(core/defmacro ^::ana/numeric unchecked-remainder-int ([x n] `(core/mod ~x ~n)))

(core/defmacro ^::ana/numeric unchecked-subtract ([& xs] `(- ~@xs)))

(core/defmacro ^::ana/numeric unchecked-subtract-int ([& xs] `(- ~@xs)))
...
(core/defmacro ^::ana/numeric dec [x] `(- ~x 1))

(core/defmacro ^::ana/numeric inc [x] `(+ ~x 1))

(core/defmacro ^::ana/numeric zero? [x] `(== ~x 0))

(core/defmacro ^::ana/numeric pos? [x] `(> ~x 0))

(core/defmacro ^::ana/numeric neg? [x] `(< ~x 0))

(core/defmacro ^::ana/numeric max
  ([x] x)
  ([x y] `(let [x# ~x y# ~y] (~'js* "((~{} > ~{}) ? ~{} : ~{})" x# y# x# y#)))
...
(core/defmacro ^::ana/numeric js-mod
  [num div]
  (core/list 'js* "(~{} % ~{})" num div))

(core/defmacro ^::ana/numeric bit-not [x] (core/list 'js* "(~ ~{})" x))
...
(core/defmacro ^::ana/numeric bit-clear
  [x n]
  (core/list 'js* "(~{} & ~(1 << ~{}))" x n))

(core/defmacro ^::ana/numeric bit-flip
  [x n]
  (core/list 'js* "(~{} ^ (1 << ~{}))" x n))

(core/defmacro bit-test
  [x n]
  (bool-expr (core/list 'js* "((~{} & (1 << ~{})) != 0)" x n)))

(core/defmacro ^::ana/numeric bit-shift-left
  [x n]
  (core/list 'js* "(~{} << ~{})" x n))

(core/defmacro ^::ana/numeric bit-shift-right
  [x n]
  (core/list 'js* "(~{} >> ~{})" x n))

(core/defmacro ^::ana/numeric bit-shift-right-zero-fill
  [x n]
  (core/list 'js* "(~{} >>> ~{})" x n))

(core/defmacro ^::ana/numeric unsigned-bit-shift-right
  [x n]
  (core/list 'js* "(~{} >>> ~{})" x n))

(core/defmacro ^::ana/numeric bit-set
  [x n]
  (core/list 'js* "(~{} | (1 << ~{}))" x n))
...
;; internal
(core/defmacro bitpos
  [hash shift]
  (core/list 'js* "(1 << ~{})" `(mask ~hash ~shift)))

;; internal
(core/defmacro caching-hash
...
(core/defmacro reify
 ...
 (reify options* specs*)
  ...
  == {:k :v}"
  [& impls]
  (core/let [t (with-meta (gensym (core/str "t_"
                                            (string/replace
             ...
             ns (core/-> &env
                         ...
                         :name)
             munge comp/munge]
    `(do (when-not (exists? ~(symbol (core/str ns) (core/str t)))
...
(core/defmacro specify
  ...
  [expr & impls]
  `(cljs.core/specify! (cljs.core/clone ~expr) ~@impls))

(core/defmacro ^:private js-this [] (core/list 'js* "this"))

(core/defmacro this-as
  "Defines a scope where JavaScript's implicit \"this\" is bound to the name provided."
  [name & body]
  `(let [~name (js-this)] ~@body))

(core/defn- to-property [sym] (symbol (core/str "-" sym)))

(core/defn- warn-and-update-protocol
  [p type env]
  (core/when-not (= 'Object p)
    (core/if-let [var (cljs.analyzer/resolve-existing-var (dissoc env :locals)
                                                          p)]
      (do (core/when-not (:protocol-symbol var)
            (cljs.analyzer/warning :invalid-protocol-symbol env {:protocol p}))
          (core/when (core/and (:protocol-deprecated
      ...
      (core/when (:undeclared cljs.analyzer/*cljs-warnings*)
        (cljs.analyzer/warning :undeclared-protocol-symbol
                               env
                               {:protocol p})))))

(core/defn- resolve-var
  [env sym]
...
(core/defmulti ^:private extend-prefix
  (core/fn [tsym sym]
    (core/-> tsym
             meta
             :extend)))
...
(core/defmethod extend-prefix :instance
  [tsym sym]
  `(.. ~tsym ~(to-property sym)))

(core/defmethod extend-prefix :default
  [tsym sym]
  `(.. ~tsym ~'-prototype ~(to-property sym)))

(core/defn- adapt-obj-params
  [type [[this & args :as sig] & body]]
  (core/list (vec args) (list* 'this-as (vary-meta this assoc :tag type) body)))

(core/defn- adapt-ifn-params
  [type [[this & args :as sig] & body]]
  (core/let [self-sym (with-meta 'self__ {:tag type})]
    `(~(vec (cons self-sym args))
      (this-as ~self-sym (let [~this ~self-sym] ~@body)))))

;; for IFn invoke implementations, we need to drop first arg
(core/defn- adapt-ifn-invoke-params
  [type [[this & args :as sig] & body]]
  `(~(vec args) (this-as ~(vary-meta this assoc :tag type) ~@body)))

(core/defn- adapt-proto-params
  [type [[this & args :as sig] & body]]
  (core/let [this' (vary-meta this assoc :tag type)]
    `(~(vec (cons this' args)) (this-as ~this' ~@body))))

(core/defn- add-obj-methods
  [type type-sym sigs]
  (map (core/fn [[f & meths :as form]]
         (core/let [[f meths]
                      (if (vector? (first meths)) [f [(rest form)]] [f meths])]
           `(set! ~(extend-prefix type-sym f)
                  ~(with-meta `(fn ~@(map #(adapt-obj-params type %) meths))
                     (meta form)))))
    sigs))

(core/defn- ifn-invoke-methods
    ...
    (if (vector? (first meths))
      ...
      (core/let [meth meths]
        [`(set! ~(extend-prefix type-sym
                ...
                ~(with-meta `(fn ~@(adapt-proto-params type meth))
                   (meta form)))])
      (map (core/fn [[sig & body :as meth]]
             `(set!
                ~(extend-prefix type-sym (core/str pf "$arity$" (count sig)))
                ~(with-meta `(fn ~(adapt-proto-params type meth)) (meta form))))
        meths))))

(core/defn- proto-assign-impls
  ...
  (core/let [psym (resolve p)
    ...
    (if (= p 'Object)
      ...
      (concat (core/when-not (skip-flag psym)
                [`(set! ~(extend-prefix type-sym pprefix)
  ...
  (core/when-not (= p 'Object)
    (core/let [var (ana/resolve-var (dissoc env :locals) p)
      ...
      (core/when (= decmeths ::not-found)
        (ana/warning :protocol-invalid-method
                     env
                     {:protocol p, :fname fname, :no-such-method true}))
      (core/when (namespace method-name)
        (core/let [method-var (ana/resolve-var (dissoc env :locals)
                                               method-name
        ...
        (core/when (seq sigs)
          (core/let [sig (first sigs)
            ...
            (core/when (contains? seen c)
              (ana/warning :protocol-duped-method
                           env
                           {:protocol p, :fname fname}))
            (core/when (some '#{&} sig)
              (ana/warning :protocol-impl-with-variadic-method
                           env
                           {:protocol p, :name fname}))
            (core/when (core/and (not= decmeths ::not-found)
          ...
          (core/when (seq methods)
            (core/let [[fname :as method] (first methods)]
              (core/when (contains? seen fname)
                (ana/warning :extend-type-invalid-method-shape
                             env
...
(core/defmacro extend-type
  ...
  (core/let [env &env
    ...
    (core/when (core/and (:extending-base-js-type cljs.analyzer/*cljs-warnings*)
      ...
      (cljs.analyzer/warning :extending-base-js-type
                             env
                             {:current-symbol type-sym,
                              :suggested-symbol (js-base-type type-sym)}))
    `(do ~@(mapcat #(assign-impls env resolve type-sym type %) impl-map))))

(core/defn- prepare-protocol-masks
  [env impls]
  (core/let [resolve (partial resolve-var env)
             impl-map (->impl-map impls)
             fpp-pbs (seq (keep fast-path-protocols
    ...
    (if fpp-pbs
      (core/let [fpps (into #{}
                            (filter (partial contains? fast-path-protocols)
                              (map resolve (keys impl-map))))
                 parts
                   (core/as-> (group-by first fpp-pbs) parts
                     (into {}
                           (map (juxt key (comp (partial map peek) val)) parts))
                     (into {}
                           (map (juxt key
                                      (comp (partial reduce core/bit-or) val))
                             parts)))]
        [fpps
         (reduce (core/fn [ps p] (update-in ps [p] (core/fnil identity 0)))
           parts
           (range fast-path-protocol-partitions-count))]))))

(core/defn- annotate-specs
  [annots v [f sigs]]
  (conj
    v
    (vary-meta (cons f (map #(cons (second %) (nnext %)) sigs)) merge annots)))
       ...
       (if (seq specs)
         (core/let [p (first specs)
                    ret (core/->
                          (conj ret p)
                          (into
                            (reduce (partial annotate-specs annots)
                              []
...
(core/defn- build-positional-factory
  ...
  (core/let [fn-name (with-meta (symbol (core/str '-> rsym))
             ...
             field-values (if (core/-> rsym
                                       meta
                                       :internal-ctor)
                            (conj fields nil nil nil)
                            fields)]
    `(defn ~fn-name ~docstring [~@fields] (new ~rname ~@field-values))))
    ...
    (core/let [gs (gensym)
               ...
               impls
                 (concat
                   impls
                   ['IRecord 'ICloneable
                    `(~'-clone [this#] (new ~tagname ~@fields)) 'IHash
                    `(~'-hash
                      [this#]
                      (caching-hash this#
                                    (fn [coll#]
                                      (bit-xor ~(hash (core/-> rname
                                                               comp/munge
                                                               core/str))
                                               (hash-unordered-coll coll#)))
                                    ~'__hash)) 'IEquiv
                    (core/let [this (gensym 'this)
                               other (gensym 'other)]
                      `(~'-equiv
                        [~this ~other]
                        (and
                    ...
                    'IMeta `(~'-meta [this#] ~'__meta) 'IWithMeta
                    `(~'-with-meta
                      [this# ~gs]
                      (new ~tagname ~@(replace {'__meta gs} fields))) 'ILookup
                    `(~'-lookup [this# k#] (-lookup this# k# nil))
                    `(~'-lookup
                      [this# ~ksym else#]
                      (case ~ksym
                        ~@(mapcat (core/fn [f] [(keyword f) f]) base-fields)
                          (cljs.core/get ~'__extmap ~ksym else#))) 'ICounted
                    `(~'-count
                      [this#]
                      (+ ~(count base-fields) (count ~'__extmap))) 'ICollection
                    `(~'-conj
                      [this# entry#]
                      (if (vector? entry#)
                        (-assoc this# (-nth entry# 0) (-nth entry# 1))
                        (reduce -conj this# entry#))) 'IAssociative
                    `(~'-assoc
                      [this# k# ~gs]
                      (condp keyword-identical? k#
                        ~@(mapcat (core/fn [fld] [(keyword fld)
                                                  (list* `new
                                                         tagname
                                                         (replace {fld gs,
                                                                   '__hash nil}
                    ...
                    `(~'-seq
                      [this#]
                      (seq (concat [~@(map #(core/list 'cljs.core/MapEntry.
                                                       (keyword %)
                                                       %
                                                       nil)
                                        base-fields)]
                    ...
                    `(~'-iterator
                      ...
                      (RecordIter.
                        ...
                        (if ~'__extmap (-iterator ~'__extmap) (core/nil-iter))))
                    'IPrintWithWriter
                    `(~'-pr-writer
                      [this# writer# opts#]
                      (let [pr-pair# (fn [keyval#]
                        ...
                        (pr-sequential-writer
                          ...
                          "}"
                          opts#
                          (concat [~@(map #(core/list `vector (keyword %) %)
                                       base-fields)]
                                  ~'__extmap)))) 'IKVReduce
                    `(~'-kv-reduce
                      [this# f# init#]
...
(core/defn- build-map-factory
  ...
  (core/let [fn-name (with-meta (symbol (core/str 'map-> rsym))
             ...
             docstring (core/str "Factory function for "
                                 rname
    ...
    `(defn ~fn-name
       ~docstring
       [~ms]
       (let [extmap# (cond->> (dissoc ~ms ~@ks) (record? ~ms) (into {}))]
...
(core/defmacro defrecord
  ...
  (core/let [rsym (vary-meta rsym assoc :internal-ctor true)
    ...
    `(let []
       ...
       (set! (.-cljs$lang$ctorPrSeq ~r)
             (fn [this#] (cljs.core/list ~(core/str r))))
       (set! (.-cljs$lang$ctorPrWriter ~r)
...
(core/defmacro defprotocol
  ...
  (core/let [p (:name (cljs.analyzer/resolve-var (dissoc &env :locals) psym))
             [doc methods] (if (core/string? (first doc+methods))
                             ...
                             [nil doc+methods])
             psym (vary-meta psym assoc :doc doc :protocol-symbol true)
             ns-name (core/-> &env
                              :ns
                              :name)
             ...
             _ (core/doseq [[mname & arities] methods]
                 (core/when (some #{0} (map count (filter vector? arities)))
                   (throw #?(:clj (Exception. (core/str "Invalid protocol, "
                                                        psym
                                                        " defines method "
                                                        mname
                                                        " with arity 0"))
                             :cljs (js/Error. (core/str "Invalid protocol, "
                                                        psym
                                                        " defines method "
                                                        mname
                                                        " with arity 0"))))))
             expand-sig
               (core/fn [fname slot sig]
                 (core/let [sig (core/if-not (every? core/symbol? sig)
                                  (mapv (core/fn [arg]
                                          (core/cond (core/symbol? arg) arg
                                                     (core/and (map? arg)
                                                               (core/some?
                                                                 (:as arg)))
                                                       (:as arg)
                 ...
                 (core/let [doc (core/as-> (last sigs) doc
                            ...
                            amp (core/when (some #{'&} (apply concat sigs))
                                  (cljs.analyzer/warning
                                    ...
                                    &env
                                    {:protocol psym, :name fname}))
                            slot (symbol (core/str prefix (munge (name fname))))
                            fname (vary-meta fname assoc :protocol p :doc doc)]
                   `(defn ~fname
                      ~@(map (core/fn [sig]
                               (expand-sig
                                 fname
...
(core/defmacro with-redefs
  ...
  (core/let [names (take-nth 2 bindings)
             ...
             orig-val-syms (map (comp gensym #(core/str % "-orig-val__") name)
                             names)
             temp-val-syms (map (comp gensym #(core/str % "-temp-val__") name)
                             names)
    ...
    `(let [~@(interleave orig-val-syms names) ~@(interleave temp-val-syms vals)]
       ~@(map bind-value binds)
       (try ~@body (finally ~@(map bind-value resets))))))
  ...
  (core/let [gpred (gensym "pred__")
             ...
             emit (core/fn emit [pred expr args]
                    (core/let [[[a b c :as clause] more]
                               ...
                               n (count clause)]
                      (core/cond
                        (= 0 n) `(throw (js/Error. (cljs.core/str
                                                     "No matching clause: "
                                                     ~expr)))
                        (= 1 n) a
                        ...
                        :else `(if-let [p# (~pred ~a ~expr)]
                                 ...
                                 ~(emit pred expr more)))))
             gres (gensym "res__")]
    `(let [~gpred ~pred ~gexpr ~expr] ~(emit gpred gexpr clauses))))

(core/defn- assoc-test
  [m test expr env]
  (if (contains? m test)
    (throw
      #?(:clj (clojure.core/IllegalArgumentException.
                (core/str "Duplicate case test constant '" test
                          "'" (core/when (:line env)
...
(core/defn- const?
  [env x]
  (core/let [m (core/and (core/list? x) (ana/resolve-var env (last x)))]
...
(core/defmacro case
  ...
  (core/let [esym (gensym)
             default (if (odd? (count clauses))
                       (last clauses)
                       `(throw (js/Error. (cljs.core/str "No matching clause: "
                                                         ~esym))))
             env &env
             pairs (reduce
                     (core/fn [m [test expr]]
             ...
             tests (keys pairs)]
    (core/cond
      (every? (some-fn core/number?
                       core/string?
                       #?(:clj core/char?
                          :cljs (core/fnil core/char? :nonchar))
                       #(const? env %))
              tests)
        (core/let [no-default
                     (if (odd? (count clauses)) (butlast clauses) clauses)
                   tests (mapv #(if (seq? %) (vec %) [%])
      ...
      (every? core/keyword? tests)
        (core/let [no-default
                   ...
                   tests (mapv #(if (seq? %) (mapv kw-str %) [(kw-str %)])
                           (take-nth 2 no-default))
                   thens (vec (take-nth 2 (drop 1 no-default)))]
          `(let [~esym ~e
                 ~esym (if (keyword? ~esym)
                         (.-fqn ~(vary-meta esym assoc :tag 'cljs.core/Keyword))
                         nil)]
             (case* ~esym ~tests ~thens ~default)))
      ;; equality
      :else
        `(let [~esym ~e]
           (cond ~@(mapcat (core/fn [[m c]] `((cljs.core/= ~m ~esym) ~c)) pairs)
                   :else
                 ~default)))))

(core/defmacro ^:private when-assert [x] (core/when *assert* x))
...
(core/defmacro assert
  ...
  ([x message]
   (core/when *assert*
     `(when-not ~x
        (throw (js/Error. (cljs.core/str "Assert failed: " ~message
                                         "\n" ~(core/pr-str x))))))))
...
(core/defmacro for
  ...
  (core/let [to-groups
             ...
             emit-bind
               (core/fn emit-bind [[[bind expr & mod-pairs] &
                                    [[_ next-expr] :as next-groups]]]
                 (core/let [giter (gensym "iter__")
                            gxs (gensym "s__")
                            do-mod
                              (core/fn do-mod [[[k v :as pair] & etc]]
                                (core/cond
                                  (= k :let) `(let ~v ~(do-mod etc))
                                  (= k :while) `(when ~v ~(do-mod etc))
                                  (= k :when)
                                    `(if ~v ~(do-mod etc) (recur (rest ~gxs)))
                                  (core/keyword? k)
                   ...
                   (if next-groups
                     ...
                     (core/let [gi (gensym "i__")
                                gb (gensym "b__")
                                do-cmod
                                  (core/fn do-cmod [[[k v :as pair] & etc]]
                                    (core/cond
                 ...
                 (core/if-not exprs
                   ...
                   (core/let [k (first exprs)
                     ...
                     (core/cond
                       ...
                       (= k :while)
                         [false
                          `(when ~v ~subform ~@(core/when needrec [recform]))]
                       (= k :when) [false
                                    `(if ~v
                                       (do ~subform
                       ...
                       :else
                         (core/let [chunksym (with-meta (gensym "chunk__")
                                               {:tag 'not-native})
                                    countsym (gensym "count__")
                                    isym (gensym "i__")
                                    recform-chunk `(recur ~seqsym
                                                          ~chunksym
                                                          ~countsym
                                                          (unchecked-inc ~isym))
                                    steppair-chunk (step recform-chunk
                                                         (nnext exprs))
...
(core/defmacro array
  ...
  (core/let [xs-str (core/->> (repeat "~{}")
                              ...
                              (interpose ",")
                              (apply core/str))]
    (vary-meta (list* 'js* (core/str "[" xs-str "]") rest) assoc :tag 'array)))

(core/defmacro make-array
  ([size]
   (vary-meta (if (core/number? size)
                `(array ~@(take size (repeat nil)))
                `(js/Array. ~size))
              assoc
              :tag
              'array))
  ([type size] `(cljs.core/make-array ~size))
  ([type size & more-sizes]
   (vary-meta `(let [dims# (list ~@more-sizes)
...
(core/defmacro list
  ...
  ([x] `(cljs.core/List. nil ~x nil 1 nil))
  ([x & xs]
...
(core/defmacro vector
  ...
  ([& xs]
   (core/let [cnt (count xs)]
     (if (core/< cnt 32)
       `(cljs.core/PersistentVector. nil
                                     ~cnt
                                     5
                                     (.-EMPTY-NODE cljs.core/PersistentVector)
                                     (array ~@xs)
                                     nil)
       (vary-meta `(.fromArray cljs.core/PersistentVector (array ~@xs) true)
                  assoc
                  :tag
  ...
  ([& kvs]
   (core/let [keys (map first (partition 2 kvs))]
     (if (core/and (every? #(= (:op (cljs.analyzer/unwrap-quote %)) :const)
       ...
       `(cljs.core/PersistentArrayMap. nil
                                       ~(clojure.core// (count kvs) 2)
                                       (array ~@kvs)
                                       nil)
   ...
   (if (core/and (core/<= (count xs) 8)
     ...
     `(cljs.core/PersistentHashSet. nil
                                    (cljs.core/PersistentArrayMap.
                                      nil
                                      ~(count xs)
                                      (array ~@(interleave xs (repeat nil)))
                                      nil)
                                    nil)
     (vary-meta `(.createAsIfByAssoc cljs.core/PersistentHashSet (array ~@xs))
                assoc
                :tag
...
(core/defn- js-obj*
  ...
  (core/let [kvs-str (core/->> (repeat "~{}:~{}")
                               ...
                               (interpose ",")
                               (apply core/str))]
    (vary-meta (list* 'js* (core/str "{" kvs-str "}") (apply concat kvs))
               assoc
               :tag
               'object)))

(core/defmacro js-obj
  [& rest]
  (core/let [sym-or-str? (core/fn [x]
                           (core/or (core/symbol? x) (core/string? x)))
             ...
             expr->local (zipmap (filter (complement sym-or-str?) (keys kvs))
                                 (repeatedly gensym))
             obj (gensym "obj")]
    (if (empty? rest)
      (js-obj* '())
      `(let [~@(apply concat (clojure.set/map-invert expr->local)) ~obj
             ~(js-obj* (filter-on-keys core/string? kvs))]
         ~@(map (core/fn [[k v]] `(goog.object/set ~obj ~k ~v)) sym-pairs)
         ~@(map (core/fn [[k v]] `(goog.object/set ~obj ~v ~(core/get kvs k)))
             expr->local)
         ~obj))))

(core/defmacro alength
  [a]
  (vary-meta (core/list 'js* "~{}.length" a) assoc :tag 'number))
...
(core/defmacro amap
  ...
  `(let [a# ~a
     ...
     (loop [~idx 0]
       (if (< ~idx l#) (do (aset ~ret ~idx ~expr) (recur (inc ~idx))) ~ret))))
...
(core/defmacro areduce
  ...
  `(let [a# ~a
         l# (alength a#)]
     (loop [~idx 0 ~ret ~init] (if (< ~idx l#) (recur (inc ~idx) ~expr) ~ret))))
...
(core/defmacro dotimes
  ...
  (core/let [i (first bindings)
             n (second bindings)]
    `(let [n# ~n] (loop [~i 0] (when (< ~i n#) ~@body (recur (inc ~i)))))))
...
(core/defmacro defmulti
  ...
  [mm-name & options]
  (core/let [docstring (if (core/string? (first options)) (first options) nil)
             options (if (core/string? (first options)) (next options) options)
    ...
    (core/let [options (apply core/hash-map options)
      ...
      `(defonce ~(with-meta mm-name m)
                (let [method-table# (atom {})
                      ...
                      hierarchy# (cljs.core/get
                                   ~options
                                   :hierarchy
                                   ((~'js* "cljs.core.get_global_hierarchy")))]
                  (cljs.core/MultiFn. (cljs.core/symbol ~mm-ns ~(name mm-name))
                                      ~dispatch-fn
                                      ~default
                                      hierarchy#
                                      method-table#
...
(core/defmacro defmethod
  ...
  `(-add-method ~(with-meta multifn {:tag 'cljs.core/MultiFn})
                ~dispatch-val
                (fn ~@fn-tail)))
...
(def ^:private cs
  (into [] (map (comp gensym core/str core/char) (range 97 118))))
...
(core/defmacro js-str [s] (core/list 'js* "''+~{}" s))

(core/defmacro es6-iterable
  [ty]
  `(goog.object/set (.-prototype ~ty)
                    cljs.core/ITER_SYMBOL
                    (fn [] (this-as this# (cljs.core/es6-iterator this#)))))
...
(core/defmacro ns-imports
  ...
  (core/let [ns (second quoted-ns)]
    `(into {}
           [~@(map (core/fn [[ctor qualified-ctor]]
...
(core/defmacro locking [x & forms] `(do ~@forms))
...
(core/defmacro load-file* [f] `(goog/nodeGlobalRequire ~f))
...
(core/defmacro macroexpand-1
  ...
  (core/let [form (second quoted)]
    (if (seq? form) `(quote ~(ana/macroexpand-1 &env form)) form)))
...
(core/defn- variadic-fn?
  [fdecl]
  (core/and (= 1 (count fdecl)) (some '#{&} (ffirst fdecl))))
...
(core/defn- variadic-fn*
  ...
  ([sym [arglist & body :as method] solo]
   (core/let [sig (remove '#{&} arglist)
              restarg (gensym "seq")]
     (core/letfn [(get-delegate [] 'cljs$core$IFn$_invoke$arity$variadic)
                    (get-delegate-prop []
                  ...
                  (param-bind [param]
                    ...
                    (apply-to
                      []
                      (if (core/< 1 (count sig))
                        (core/let [params (repeatedly (core/dec (count sig))
                                                      gensym)]
                          `(fn ([~restarg]
                                (let [~@(mapcat param-bind params)]
                                  (this-as
                                    self#
                                    (. self#
                                       (~(get-delegate) ~@params ~restarg)))))))
                        `(fn ([~restarg]
                              (this-as
                                self#
                                (. self# (~(get-delegate) (seq ~restarg))))))))]
       `(do (set! (. ~sym ~(get-delegate-prop)) (fn (~(vec sig) ~@body)))
            ~@(core/when solo
                `[(set! (. ~sym ~'-cljs$lang$maxFixedArity)
                        ~(core/dec (count sig)))])
            (js-inline-comment " @this {Function} ")
            ;; dissoc :top-fn so this helper gets ignored in cljs.analyzer/parse
            ;; 'set!
            (set! (. ~(vary-meta sym dissoc :top-fn) ~'-cljs$lang$applyTo)
                  ~(apply-to)))))))

(core/defmacro copy-arguments
  [dest]
...
(core/defn- elide-implicit-macro-args
  [arglists]
               ...
               meta
                 (assoc meta
                   :top-fn {:variadic? true,
                            :fixed-arity mfa,
                            :max-fixed-arity mfa,
                            :method-params (core/cond-> [sig]
                                             macro? elide-implicit-macro-args),
                            :arglists (core/cond-> (core/list arglist)
                                        macro? elide-implicit-macro-args),
...
(core/comment (require '[clojure.pprint :as pp])
  ...
  (core/letfn [(dest-args [c]
                 ...
                 (fixed-arity
                   ...
                   (core/let [c (count sig)]
                     [c
                      `(. ~rname
                          (~(symbol (core/str "cljs$core$IFn$_invoke$arity$" c))
                           ~@(dest-args c)))]))
               (fn-method
    ...
    (core/let [rname (symbol (core/str ana/*cljs-ns*) (core/str name))
               ...
               maxfa (apply core/max
                       (concat (map count sigs)
                               [(core/- (count (first (filter varsig?
                                                        arglists)))
                                        2)]))
               macro? (:macro meta)
               ...
               meta
                 (assoc meta
                   :top-fn {:variadic? variadic,
                            :fixed-arity mfa,
                            :max-fixed-arity mfa,
                            :method-params (core/cond-> sigs
                                             macro? elide-implicit-macro-args),
                            :arglists (core/cond-> arglists
                                        macro? elide-implicit-macro-args),
                       ...
                       (let [argseq# (new ^::ana/no-resolve cljs.core/IndexedSeq
                         ...
                         (. ~rname
                            (~'cljs$core$IFn$_invoke$arity$variadic
                             ...
                             argseq#))))
                    (if (:macro meta)
                      `(throw (js/Error. (str "Invalid arity: "
                                              (- (alength (js-arguments)) 2))))
                      `(throw (js/Error. (str "Invalid arity: "
...
(def
  ^{:doc
    ...
    :macro true}
  defn
  (core/fn defn [&form &env name & fdecl]
    ;; Note: Cannot delegate this check to def because of the call to (with-meta
    ;; name ..)
    (if (core/instance? #?(:clj clojure.lang.Symbol
                           :cljs Symbol)
                        name)
      nil
    ...
    (core/let [m (if (core/string? (first fdecl)) {:doc (first fdecl)} {})
               fdecl (if (core/string? (first fdecl)) (next fdecl) fdecl)
               m (if (map? (first fdecl)) (conj m (first fdecl)) m)
               fdecl (if (map? (first fdecl)) (next fdecl) fdecl)
               fdecl (if (vector? (first fdecl)) (core/list fdecl) fdecl)
               ...
               ;                 :cljs (core/instance? Symbol iname)) false
               ;                 true))
               ;      ;; inserts the same fn name to the inline fn if it does
               ;      not have one
               ;      (assoc m
               ;        :inline (cons ifn
      ...
      (core/cond
        ...
        :else (core/list 'def
                         ...
                         ;;must figure out how to convey primitive hints to self
                         ;;calls first
                         (cons `fn fdecl))))))
...
(core/defn defmacro
  ...
  {:arglists '([name doc-string? attr-map? [params*] body]
   ...
   :macro true}
  [&form &env name & args]
  (core/let [prefix (core/loop [p (core/list (vary-meta name assoc :macro true))
                                args args]
...
(core/defmacro resolve
  ...
  [quoted-sym]
  (core/assert (core/and (seq? quoted-sym) (= 'quote (first quoted-sym)))
    ...
    [sym (second quoted-sym)
     ...
     [var meta]
       (try
         ...
         (catch #?@(:clj [Throwable t]
                    :cljs [:default e]) [(ana/resolve-var env sym) nil]))
