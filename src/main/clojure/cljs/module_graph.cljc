            ...
            matches (into #{}
                          (filter (fn [source]
                                    (when (some #(.startsWith ^String % matcher)
                                                (map (comp str comp/munge)
                                                  (:provides source)))
                                      source)))
                          sources)]
        (when-not (empty? matches) matches))
      (when-let [input (some (fn [source]
                               (let [matcher (into #{}
                                                   [(name entry)
    ...
    (fn [ret module-name module]
      (assoc ret
        module-name (update module
                            :entries
                            (fn [es]
                              (into #{} (map (comp str comp/munge)) es)))))
    {}
    modules))
...
(defn add-cljs-base
  ...
  [modules]
  (cond-> modules (not (contains? modules :cljs-base)) (assoc :cljs-base {})))

(defn add-cljs-base-dep
  "Adds :cljs-base to any module in compiler :modules with an empty :depends-on."
  [modules]
  (reduce-kv (fn [ret k {:keys [depends-on], :as module-info}]
               (assoc ret
                 k (cond-> module-info
...
(defn annotate-depths
  ...
  [modules]
  (reduce-kv (fn [ret module-name module]
               (let [module' (assoc module
                               :depth (depth-of module-name modules))]
...
(defn deepest-common-parent
  ...
  (let [common-parents (reduce set/intersection
                         (map #(conj (set (deps-for-module % all-modules)) %)
                           modules))]
    (apply max-key (fn [p] (get-in all-modules [p :depth])) common-parents)))

(defn canonical-name
  "Given an entry use indexed-inputs to return the canonical name. Throws if
   entry cannot be found."
  [entry indexed-inputs]
  (if-let [entry (get indexed-inputs
                      (-> entry
                          comp/munge
                          str))]
    (-> (:provides entry)
        ...
        (doseq [entry entries]
          (let [seen' @seen]
            (if-some [module-name' (get seen' entry)]
              (throw
                (Exception. (str "duplicate entry \"" entry
                                 "\", occurs in " module-name
                                 " and " module-name'
                                 ". entry :provides is "
...
(defn inputs->assigned-modules
  ...
  (let [index (index-inputs inputs)
        ...
        orphans
          {:cljs-base {:entries (->> (reduce-kv (fn [m k _]
                                                  (reduce dissoc
                                                    m
                                                    (get-in m [k :provides])))
                                                index
...
(defn expand-modules
  ...
  [modules inputs]
  (let [order
          (first (reduce (fn [[ret n] {:keys [provides]}]
                           [(merge ret
                                   (zipmap (map (comp str comp/munge) provides)
                                           (repeat n))) (inc n)])
                   [{} 0]
                   inputs))
        modules' (-> modules
                     ...
                     add-cljs-base
                     add-cljs-base-dep)
        assigns (inputs->assigned-modules inputs (annotate-depths modules'))
        um (reduce-kv
             (fn [ret entry module-name]
               (update-in ret [module-name :entries] (fnil conj #{}) entry))
             modules'
             assigns)]
    (reduce-kv
...
(comment (inputs->assigned-modules inputs
                                   (-> modules
                                       add-cljs-base
                                       add-cljs-base-dep
                                       annotate-depths))
         (pprint (expand-modules modules inputs)))

(defn topo-sort
  "Topologically sort a graph using the given edges-key."
  [graph edges-key]
  (letfn [(no-incoming-edges
            [graph edges-key]
            (->> graph
                 (filter (fn [[k v]]
...
(comment (def ms
           (sort-modules (-> {:cljs-base {:output-to "out/module/base.js"},
                              :core {:output-to "out/modules/core.js",
                                     :entries '#{cljs.core}},
...
(defn parent?
  [f0 f1]
  (.startsWith (.getAbsolutePath (io/file f0)) (.getAbsolutePath (io/file f1))))

;; JS modules become Closure libs that exist in the output directory. However
;; in
;; the current indexing pipeline, these will not have an :out-file. Correct
;; these
...
(defn maybe-add-out-file
  ...
  (if-not lib-path
    ijs
    (if (parent? lib-path output-dir) (assoc ijs :out-file lib-path) ijs)))
...
(defn modules->module-uris
  ...
  (assert (#{:advanced :simple :none :whitespace} optimizations)
          "Must supply valid :optimizations in opts map")
  (assert output-dir "Must supply :output-dir in opts map")
  (letfn [(get-uri [rel-path] (cond->> rel-path asset-path (str asset-path)))
    ...
    (let [get-rel-path (partial
                         get-rel-path*
                         (.. (io/file output-dir) getAbsoluteFile getPath))]
      (case optimizations
        :none
          (into
            {}
            (map
              (fn [[module-name {:keys [entries], :as module}]]
                [module-name
                 (into
                   []
                   (comp (mapcat #(find-sources-for-module-entry % inputs))
                         (map (comp get-uri
                                    get-rel-path
                                    (fn [{:keys [out-file], :as ijs}]
                                      (if-not out-file
                                        (throw
                                          (Exception.
                                            (str "No :out-file for IJavaScript "
                                                 (pr-str ijs))))
                                        out-file))
...
(defn module-for
  ...
  (let [modules' (normalize modules)
    ...
    (->> modules'
         (some (fn [[module-name {:keys [entries]} :as me]]
