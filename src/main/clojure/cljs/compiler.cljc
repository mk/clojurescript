...
(def ^:private es5>=
  (into #{}
        (comp (mapcat (fn [lang] [lang
                                  (keyword (string/replace (name lang)
                                                           #"^ecmascript"
                                                           "es"))])))
...
(defn get-first-ns-segment
  ...
  [ns]
  (let [ns (str ns) idx (.indexOf ns ".")] (if (== -1 idx) ns (subs ns 0 idx))))

(defn ^:dynamic find-ns-starts-with
  [needle]
  (reduce-kv (fn [xs ns _]
...
(defn shadow-depth
  ...
  (let [{:keys [name info]} s]
    (loop [d 0
           {:keys [shadow]} info]
      (cond shadow (recur (inc d) shadow)
...
(defn hash-scope
  ...
  #?(:clj (or (:identity s) (System/identityHashCode s))
     :cljs (hash-combine (-hash ^not-native (:name s)) (shadow-depth s))))
...
(defn fn-self-name
  ...
  (let [name (string/replace (str name) ".." "_DOT__DOT_")
    ...
    (symbol (munge (str (string/replace (str ns) "." "$") "$" scoped-name)))))
     ...
     (let [name-var s
       ...
       (if-not (nil? (:fn-self-name info))
         ...
         (let [depth (shadow-depth s)
               ...
               name (cond (true? field) (str "self__." name)
      ...
      (when (:line env)
        (let [{:keys [line column]} env]
          (swap! *source-map-data*
            (fn [m]
              (let [minfo
                      (cond-> {:gcol #?(:clj (.get ^AtomicLong
                                                   *source-map-data-gen-col*)
                                        :cljs (:gen-col m)),
                               :gline (:gen-line m)}
                        (#{:var :local :js-var} (:op ast)) (assoc :name
                                                             (str (-> ast
                                                                      :info
                                                                      :name))))]
  ...
  ([^Object a]
   (cond (nil? a) nil
         ...
         #?(:clj (seq? a)
           ...
           (apply emits a)
         #?(:clj (fn? a)
            :cljs ^boolean (goog/isFunction a))
           (a)
         :else
           (let [^String s (cond-> a (not (string? a)) .toString)]
...
(defn ^:private _emitln
  ...
  (when *source-map-data*
    ...
    (swap! *source-map-data* (fn [{:keys [gen-line], :as m}]
                               (assoc m
...
(defn emitln
  ...
  ([a b c d e] (emits a) (emits b) (emits c) (emits d) (emits e) (_emitln))
  ([a b c d e & xs]
   (emits a)
   (emits b)
   (emits c)
...
(defn ^String emit-str [expr] (with-out-str (emit expr)))

#?(:clj (defmulti emit-constant* class)
   :cljs (defmulti emit-constant* type))
...
(defmethod emit-constant* :default
  [x]
  (throw (ex-info (str "failed compiling constant: "
                       x
                       "; "
                       (pr-str (type x))
                       " is not a valid ClojureScript constant.")
                  {:constant x, :type (type x)})))

(defmethod emit-constant* nil [x] (emits "null"))

#?(:clj (defmethod emit-constant* Long [x] (emits "(" x ")")))

#?(:clj (defmethod emit-constant* Integer [x] (emits x))); reader puts Integers in metadata

#?(:clj (defmethod emit-constant* Double
          [x]
          (let [x (double x)]
            (cond (Double/isNaN x) (emits "NaN")
                  (Double/isInfinite x) (emits
                                          (if (pos? x) "Infinity" "-Infinity"))
                  :else (emits x))))
   :cljs (defmethod emit-constant* js/Number
           [x]
           (cond (js/isNaN x) (emits "NaN")
                 (not (js/isFinite x)) (emits
                                         (if (pos? x) "Infinity" "-Infinity"))
                 :else (emits "(" x ")"))))

#?(:clj (defmethod emit-constant* BigDecimal
          [x]
          (emits (.doubleValue ^BigDecimal x))))

#?(:clj (defmethod emit-constant* clojure.lang.BigInt
          [x]
          (emits (.doubleValue ^clojure.lang.BigInt x))))

(defmethod emit-constant* #?(:clj String
                             :cljs js/String)
  [x]
  (emits (wrap-in-double-quotes (escape-string x))))

(defmethod emit-constant* #?(:clj Boolean
                             :cljs js/Boolean)
  [x]
  (emits (if x "true" "false")))

#?(:clj (defmethod emit-constant* Character
          [x]
          (emits (wrap-in-double-quotes (escape-char x)))))

(defmethod emit-constant* #?(:clj java.util.regex.Pattern
                             :cljs js/RegExp)
  [x]
  (if (= "" (str x))
    (emits "(new RegExp(\"\"))")
...
(defmethod emit-constant* #?(:clj clojure.lang.Keyword
                             :cljs Keyword)
  [x]
  (if-let [value (and (-> @env/*compiler*
                          :options
                          :emit-constants)
                      (-> @env/*compiler*
                          ::ana/constant-table
...
(defmethod emit-constant* #?(:clj clojure.lang.Symbol
                             :cljs Symbol)
  [x]
  (if-let [value (and (-> @env/*compiler*
                          :options
...
(defn emit-constants-comma-sep
  [cs]
  (fn []
    (doall (map-indexed (fn [i m] (if (even? i) (emit-constant m) (emits m)))
                        (comma-sep cs)))))
...
(defmethod emit-constant* #?(:clj java.util.Date
                             :cljs js/Date)
  [^java.util.Date date]
  (emits "new Date(" (.getTime date) ")"))

(defmethod emit-constant* #?(:clj java.util.UUID
                             :cljs UUID)
  [^java.util.UUID uuid]
  (let [uuid-str (.toString uuid)]
    (emits "new cljs.core.UUID(\"" uuid-str "\", " (hash uuid-str) ")")))

(defmethod emit-constant* #?(:clj JSValue
                             :cljs cljs.tagged-literals/JSValue)
  [^JSValue v]
  (let [items (.-val v)]
    (if (map? items)
        ...
        (when-not (= :statement (:context env))
          (let [reserved (cond-> js-reserved
                           (and (es5>= (:language-out options))
                                ;; we can skip munging things like
                                ;; `my.ns.default`
                ...
                js-module (get-in cenv
                                  [:js-namespaces
                                   (or (namespace var-name) (name var-name))])
                info (cond-> info (not= form 'js/-Infinity) (munge reserved))]
            (emit-wrap
              env
              (case (:module-type js-module)
                ;; Closure exports CJS exports through default property
                :commonjs (if (namespace var-name)
                            (emits (munge (namespace var-name) reserved)
                                   "[\"default\"]."
                                   (munge (name var-name) reserved))
                            (emits (munge (name var-name) reserved)
                                   "[\"default\"]"))
                ;; Emit bracket notation for default prop access instead of dot
                ;; notation
                :es6 (if (and (namespace var-name)
...
(defmethod emit* :the-var
  ...
  (let [{:keys [name]} (:info var)]
    (emit-wrap env
               (emits "new cljs.core.Var(function(){return "
                      (munge name)
                      ";},"
                      sym
                      ","
  ...
  (cond
    ...
    (<= (count keys) array-map-threshold)
      (if (distinct-keys? keys)
        (emits "new cljs.core.PersistentArrayMap(null, "
               (count keys)
               ", ["
        ...
        (emits "cljs.core.PersistentArrayMap.createAsIfByAssoc(["
               (comma-sep (interleave keys vals))
               "])"))
    :else (emits "cljs.core.PersistentHashMap.fromArrays(["
                 (comma-sep keys)
                 "],["
                 (comma-sep vals)
                 "])")))
...
(defn emit-vector
  ...
  (if (empty? items)
    ...
    (let [cnt (count items)]
      (if (< cnt 32)
        (emits "new cljs.core.PersistentVector(null, "
               cnt
  ...
  (cond
    ...
    (distinct-constants? items)
      (emits
        ...
        "], null), null)")
    :else (emits "cljs.core.PersistentHashSet.createAsIfByAssoc(["
                 (comma-sep items)
                 "])")))
...
(defn emit-js-object
  ...
  (when-let [items (seq items)]
    (let [[[k v] & r] items]
      ...
      (doseq [[k v] r] (emits ", \"" (name k) "\": " (emit-js-object-val v)))))
  (emits "})"))
...
(defmethod emit* :const
  [{:keys [form env]}]
  (when-not (= :statement (:context env)) (emit-wrap env (emit-constant form))))
...
(defn falsey-constant?
  [expr]
  (let [{:keys [op form const-expr]} (ana/unwrap-quote expr)]
    (or (and (= op :const) (or (false? form) (nil? form)))
...
(defmethod emit* :case
  ...
  (let [gs (gensym "caseval__")]
    ...
    (when default
      (emitln "default:")
      (if (= :expr (:context env)) (emitln gs "=" default) (emitln default)))
    (emitln "}")
...
(def base-types
  #{"null" "*" "...*" "boolean" "Boolean" "string" "String" "number" "Number"
    "array" "Array" "object" "Object" "RegExp" "Date"})

(def mapped-types {"nil" "null"})
...
(defn valid-define-value? [x] (or (string? x) (true? x) (false? x) (number? x)))
  ...
  (when (or init (:def-emits-var env))
    (let [mname (munge name)]
      ...
      (when (:def-emits-var env)
        (emitln "; return (")
        (emits (merge {:op :the-var, :env (assoc env :context :expr)} var-ast))
        (emitln ");})()"))
...
(defmethod emit* :fn
  [{variadic :variadic?,
    :keys [name env methods max-fixed-arity recur-frames loop-lets]}]
  ;;fn statements get erased, serve no purpose and can pollute scope if named
  (when-not (= :statement (:context env))
    (let [loop-locals (->> (concat (mapcat :params
                                     (filter #(and % @(:flag %)) recur-frames))
      ...
      (if (= 1 (count methods))
        ...
        (let [name (or name (gensym))
          ...
          (when variadic
            ...
            (emitln mname
                    ".cljs$lang$applyTo = "
                    (some #(let [[n m] %] (when (:variadic? m) n)) ms)
                    ".cljs$lang$applyTo;"))
          (doseq [[n meth] ms]
...
(defmethod emit* :try
  ...
  (let [context (:context env)]
    (if (or name finally)
      (do (when (= :expr context) (emits "(function (){"))
          (emits "try{" try "}")
...
(defmethod emit* :recur
  ...
  (let [temps (vec (take (count exprs) (repeatedly gensym)))
        params (:params frame)]
    (dotimes [i (count exprs)] (emitln "var " (temps i) " = " (exprs i) ";"))
    (dotimes [i (count exprs)] (emitln (munge (params i)) " = " (temps i) ";"))
    (emitln "continue;")))
...
(defmethod emit* :invoke
  ...
  (let [info (:info f)
        fn? (and ana/*cljs-static-fns* (not (:dynamic info)) (:fn-var info))
        ...
        proto?
          (and
            ...
            tag
            (or
              (and ana/*cljs-static-fns* protocol (= tag 'not-native))
              (and (or ana/*cljs-static-fns* (:protocol-inline env))
                   (or (= protocol tag)
                       ;; ignore new type hints for now - David
                       (and (not (set? tag))
                            (not ('#{any clj clj-or-nil clj-nil number string
                                     boolean function object array js}
                                  tag))
                            (when-let [ps (:protocols
                                            (ana/resolve-existing-var env tag))]
                              (ps protocol)))))))
          ...
          (if fn?
            (let [arity (count args)
              ...
              (cond
                ...
                (and (not variadic?) (= (count mps) 1)) [f nil]
                           ...
                           (assoc
                             :name
                               (symbol
                                 (str (munge info)
                                      ".cljs$core$IFn$_invoke$arity$variadic")))
                           ;; bypass local fn-self-name munging, we're emitting
                           ;; direct
                           ...
                           (->
                             ...
                             (assoc :name (symbol
                                            (str (munge info)
                                                 ...
                                                 arity)))
                             ;; bypass local fn-self-name munging, we're
                             ;; emitting direct
                             ;; shadowing already applied
                             (update-in [:info]
                                        #(-> %
                                             (dissoc :shadow)
                                             (dissoc :fn-self-name)))))) nil]
                      [f nil]))))
            [f nil])]
    (emit-wrap
      env
      (cond opt-not? (emits "(!(" (first args) "))")
            proto? (let [pimpl (str (munge (protocol-prefix protocol))
...
(defn load-libs
  ...
  (let [{:keys [options js-dependency-index]} @env/*compiler*
        ...
        [node-libs libs-to-load]
          (let [libs (remove (set (vals seen)) (filter (set (vals libs)) deps))]
            (if (= :nodejs target)
              (let [{node-libs true, libs-to-load false}
    ...
    (doseq [lib libs-to-load]
      (cond
        #?@(:clj [(ana/foreign-dep? lib) ;; we only load foreign libraries under
                                         ;; optimizations :none
                  ;; under :modules we also elide loads, as the module loader
                  ;; will
            ...
            :cljs [(and (ana/foreign-dep? lib)
                        (not (keyword-identical? optimizations :none))) nil])
          (or (-> libs
                  meta
                  :reload)
        ...
        (emitln "goog.require('" (munge lib) "', 'reload-all');") :else
        (when-not (= lib 'goog) (emitln "goog.require('" (munge lib) "');"))))
    (doseq [lib node-libs]
      (emitln (munge ns-name)
              "."
...
(defmethod emit* :js
  ...
  (if (and code
           #?(:clj (.startsWith ^String (string/trim code) "/*")
...
#?(:clj (defn with-core-cljs
          "Ensure that core.cljs has been loaded."
          ([] (with-core-cljs (when env/*compiler* (:options @env/*compiler*))))
          ([opts] (with-core-cljs opts (fn [])))
          ([opts body]
           {:pre [(or (nil? opts) (map? opts)) (fn? body)]}
           (when-not (get-in @env/*compiler*
                             [::ana/namespaces 'cljs.core :defs])
             (ana/analyze-file "cljs/core.cljs" opts))
           (body))))

#?(:clj (defn url-path [^File f] (.getPath (.toURL (.toURI f)))))

#?(:clj (defn compiled-by-string
          ([]
           (compiled-by-string (when env/*compiler*
...
#?(:clj (defn emit-cached-core
          [src dest cached opts]
          ...
          (when (true? (:source-map opts))
            (spit (io/file (str dest ".map"))
                  (json/write-str
                    (assoc (json/read-str (slurp (io/resource
                                                   "cljs/core.aot.js.map")))
                      "file" (str (io/file (util/output-directory opts)
                                           "cljs"
                                           "core.js"))))))
          (merge (ana/parse-ns src dest nil) {:out-file dest})))

#?(:clj (defn emit-source-map
          [src dest sm-data opts]
            ...
            (if-let [smap (:source-map-asset-path opts)]
              (emitln
                ...
                (if (true? (:source-map-timestamp opts))
                  (str (if-not (string/index-of smap "?") "?" "&")
                       "rel="
            ...
            (spit sm-file
                  (sm/encode
                    ...
                    {:lines (+ (:gen-line sm-data) 2),
                     ...
                     :relpaths {(util/path src) (util/ns->relpath
                                                  (first (:provides opts))
                                                  (:ext opts))}})))))
     ...
     (defn emit-source
       ...
       (with-open [out ^java.io.Writer (io/make-writer dest {})]
         (binding [*out* out
                   ...
                   ana/*checked-arrays* (or ana/*checked-arrays*
                                            (:checked-arrays opts))
                   ana/*cljs-static-fns* (or ana/*cljs-static-fns*
                                             (:static-fns opts))
               ...
               (loop [forms (ana/forms-seq* rdr (util/path src))
                 ...
                 (if (seq forms)
                   (let [env (assoc env :ns (ana/get-namespace ana/*cljs-ns*))
                         {:keys [op], :as ast}
                           (ana/analyze env (first forms) nil opts)]
                     (cond (= op :ns)
                           ...
                           (= :ns* (:op ast))
                             (let [ns-emitted? (some? ns-name)
                               ...
                               (recur (rest forms)
                                      ns-name
                                      (merge deps (:uses ast) (:requires ast))))
                   ...
                   (let [_ (when emitter
                         ...
                         sm-data (when *source-map-data*
                                   (assoc @*source-map-data*
                                     :gen-col (.get ^AtomicLong
                                                    *source-map-data-gen-col*)))
                         ret (merge
           ...
           (ensure
             (with-core-cljs
               ...
               (fn []
                 ...
                 (util/measure
                   (and (or ana/*verbose* (:verbose opts))
                        (:compiler-stats opts))
...
#?(:clj (defn requires-compilation?
          "Return true if the src file requires compilation."
          ([src dest]
           (requires-compilation? src
                                  dest
                                  (when env/*compiler*
                                    (:options @env/*compiler*))))
          ([^File src ^File dest opts]
             ...
             (if (and (= 'cljs.loader ns) (not (contains? opts :cache-key)))
               ...
               (ensure
                 (or
                   ...
                   (and opts
                        (not (and (io/resource "cljs/core.aot.js")
                                  (= 'cljs.core ns)))
                        (not= (ana/build-affecting-options opts)
                              (ana/build-affecting-options (util/build-options
                                                             dest))))
                   (and opts
                        (:source-map opts)
                        (if (= (:optimizations opts) :none)
                          (not (.exists (io/file (str (.getPath dest) ".map"))))
                          (not (get-in @env/*compiler*
                                       [::compiled-cljs
       ...
       ([src dest opts]
        ...
        (binding [ana/*file-defs* (atom #{})
          ...
          (let [nses (get @env/*compiler* ::ana/namespaces)
            ...
            (if (.exists src-file)
              (try (let [{ns :ns, :as ns-info}
                           (ana/parse-ns src-file dest-file opts)
                         opts (if (and (not= (util/ext src) "clj") ;; skip
                                                                   ;; cljs.core
                                                                   ;; macro-ns
                                       (= ns 'cljs.core))
                                (cond-> opts
                     ...
                     (if (or (requires-compilation? src-file dest-file opts)
                       ...
                       (do (util/mkdirs dest-file)
                           ...
                           (let [[ret recompiled?]
                             ...
                             ret))
                       (do ;; populate compilation environment with analysis
                           ;; information
                   ...
                   (catch Exception e
                     (throw (ex-info (str "failed compiling file:" src)
                                     {:file src}
                                     e))))
     ...
     (defn cljs-files-in
       ...
       (map io/file
         (reduce (fn [m x]
                   (if (.endsWith ^String x ".cljs")
                     (cond-> (conj m x)
                       (contains? m (str (subs x 0 (dec (count x))) "c"))
                         (set/difference #{(str (subs x 0 (dec (count x)))
                                                "c")}))
                     ;; ends with .cljc
                     (cond-> m
           ...
           #{}
           (into []
                 (comp (filter #(let [name (.getName ^File %)]
                                 (and (or (.endsWith name ".cljs")
                                          (.endsWith name ".cljc"))
                                      (not= \. (first name))
                                      (not (contains? cljs-reserved-file-names
                                                      name)))))
                       (map #(.getPath ^File %)))
                 (file-seq dir))))))
      ...
      in dependency order."
       ...
       ([src-dir target-dir opts]
        ...
        (let [src-dir-file (io/file src-dir)
              inputs (deps/dependency-order (map #(ana/parse-ns %)
...
#?(:clj (defn find-source [file] (ana/parse-ns file)))

#?(:clj (defn find-root-sources
          [src-dir]
          (let [src-dir-file (io/file src-dir)]
...
(defn emit-constants-table
  ...
  (doseq [[sym value] table]
    (let [ns (namespace sym)
          name (name sym)]
      (emits "cljs.core." value " = ")
      (cond (keyword? sym) (emits-keyword sym)
            (symbol? sym) (emits-symbol sym)
            :else (throw (ex-info (str "Cannot emit constant for type "
                                       (type sym))
                                  {:error :invalid-constant-type})))
      (emits ";\n"))))

#?(:clj (defn emit-constants-table-to-file
          [table dest]
...
(defn emit-externs
  ([externs]
   (emit-externs []
                 externs
                 (atom #{})
                 (when env/*compiler* (::ana/externs @env/*compiler*))))
  ([prefix externs top-level known-externs]
   (loop [ks (seq (keys externs))]
     (when ks
       (let [k (first ks)
             [top :as prefix'] (conj prefix k)]
         (when (and (not= 'prototype k) (nil? (get-in known-externs prefix')))
           (if-not (or (contains? @top-level top) (contains? known-externs top))
             (do (emitln "var " (string/join "." (map munge prefix')) ";")
...
#?(:clj (defn emit-inferred-externs-to-file
          [externs dest]
          (io/make-parents dest)
          (with-open [out ^java.io.Writer (io/make-writer dest {})]
