...
(defn repl-quit? [v] (#{":repl/quit" ":cljs/quit"} v))
...
"
  ...
  (let [repl-opts (repl/repl-options repl-env)
        opts (merge {:def-emits-var true}
                    (closure/add-implicit-options
                      (merge-with (fn [a b] (if (nil? b) a b)) repl-opts opts)))
        EOF (Object.)
                            ...
                            (try
                              (when-not (identical? form EOF)
                                (let [start (System/nanoTime)
                                      ret
                                        (if (and (seq? form)
                                                 (is-special-fn? (first form)))
                                          (do ((get special-fns (first form))
                                                repl-env
                                                env
                                                form
                                                opts)
                                              "nil")
                                          (repl/eval-cljs repl-env
                                                          env
                                                          form
                                                          opts))
                                      ms (quot (- (System/nanoTime) start)
                                               1000000)]
  ...
  (let [valf (resolve-fn valf)
    ...
    (prepl repl-env
           ...
           #(binding [*out* out
             ...
             (locking lock
                      (prn (cond-> %1
                             (#{:ret :tap} (:tag %1)) (assoc :val
                                                        (valf (:val %1))))))))))

(comment ;; eval in order
         (defmacro clj-eval [form] `(quote ~(eval form)))
         (require '[cljs.repl.node :as node])
         (io-prepl :repl-env (node/repl-env))
         ;; wait a moment for Node REPL to be ready, then eval the following
         (cljs.core.server/clj-eval (cljs.analyzer.api/ns-resolve 'cljs.core
                                                                  'first))
