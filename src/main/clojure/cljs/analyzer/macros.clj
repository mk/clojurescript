...
(ns cljs.analyzer.macros (:refer-clojure :exclude [binding]))

(defmacro with-warning-handlers
  [handlers & body]
  `(cljs.core/binding [cljs.analyzer/*cljs-warning-handlers* ~handlers] ~@body))

(defmacro no-warn
  [& body]
  `(cljs.core/binding [cljs.analyzer/*cljs-warnings*
                         (zipmap (keys cljs.analyzer/*cljs-warnings*)
                                 (repeat false))]
     ~@body))

(defmacro with-core-macros
  [path & body]
  `(do (when (not= cljs.analyzer/*cljs-macros-path* ~path)
         (reset! cljs.analyzer/-cljs-macros-loaded false))
       (cljs.core/binding [cljs.analyzer/*cljs-macros-path* ~path] ~@body)))
...
(defmacro disallowing-recur
  [& body]
  `(cljs.core/binding [cljs.analyzer/*recur-frames*
                         (cons nil cljs.analyzer/*recur-frames*)]
     ~@body))

(defmacro allowing-redef
  [& body]
  `(cljs.core/binding [cljs.analyzer/*allow-redef* true] ~@body))

(defmacro disallowing-ns*
  [& body]
