...
(ns cljs.analyzer.api
  ...
  (:refer-clojure :exclude
                  [all-ns ns-interns ns-resolve resolve find-ns ns-publics
                   remove-ns])
...
(defmacro with-state
  ...
  [state body]
  `(env/with-compiler-env ~state ~@body))

(defn empty-env "Creates an empty analysis environment." [] (ana/empty-env))

(defmacro no-warn
  "Disable analyzer warnings for any analysis executed in body."
  [& body]
  (let [no-warnings (zipmap (keys ana/*cljs-warnings*) (repeat false))]
    `(binding [ana/*cljs-warnings* ~no-warnings] ~@body)))
...
(defn get-js-index
  ...
  compiler state."
  ([] (get-options env/*compiler*))
  ([state] (get @state :js-dependency-index)))
...
#?(:clj
     ...
     facilitate code walking without knowing the details of the op set."
       ...
       ([env form name opts]
        (analyze (if-not (nil? env/*compiler*)
                   env/*compiler*
...
#?(:clj
     (defn parse-ns
      ...
      requested via opts where :restore is false."
       ...
       ([src dest opts]
        (parse-ns (if-not (nil? env/*compiler*)
                    env/*compiler*
                    (env/default-compiler-env opts))
                  src
                  dest
...
#?(:clj
     (defn analyze-file
      ...
      meaningful value."
       ...
       ([f opts]
        (analyze-file (if-not (nil? env/*compiler*)
                        env/*compiler*
                        (env/default-compiler-env opts))
                      f
                      opts))
...
(defn resolve
  ...
  (try (binding [ana/*private-var-access-nowarn* true]
...
(defn all-ns
  ...
  ([] (all-ns env/*compiler*))
  ([state] (keys (get @state ::ana/namespaces))))

(defn find-ns
  "Given a namespace return the corresponding namespace analysis map. Analagous
  to clojure.core/find-ns."
  ([sym] (find-ns env/*compiler* sym))
  ([state sym] {:pre [(symbol? sym)]} (get-in @state [::ana/namespaces sym])))
...
(defn ns-interns
  ...
  ([state ns]
   {:pre [(symbol? ns)]}
   (merge (get-in @state [::ana/namespaces ns :macros])
...
(defmacro in-cljs-user
  ...
  `(binding [cljs.analyzer/*cljs-ns* 'cljs.user]
     (cljs.env/with-compiler-env ~env ~@body)))
