...
(defn emit
  ...
  ([ast]
   (emit
     ...
     ast))
  ([state ast] (env/with-compiler-env state (with-out-str (comp/emit ast)))))

(defn with-core-cljs
  "Ensure that core.cljs has been loaded."
  ([] (comp/with-core-cljs (when env/*compiler* (:options @env/*compiler*))))
  ([opts] (with-core-cljs opts (fn [])))
  ([opts body]
   (with-core-cljs (if-not (nil? env/*compiler*)
                     env/*compiler*
...
(defn requires-compilation?
  ...
  ([src dest opts]
   (requires-compilation? (if-not (nil? env/*compiler*)
                            env/*compiler*
                            (env/default-compiler-env opts))
                          src
                          dest
...
(defn compile-file
  ...
  ([src dest opts]
   (compile-file (if-not (nil? env/*compiler*)
                   env/*compiler*
                   (env/default-compiler-env opts))
                 src
                 dest
...
(defn compile-root
  ...
  ([src-dir target-dir opts]
   (compile-root (if-not (nil? env/*compiler*)
                   env/*compiler*
                   (env/default-compiler-env opts))
                 src-dir
                 target-dir
