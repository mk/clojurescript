...
(def known-repl-opts
  ...
  #{:analyze-path :bind-err :caught :compiler-env :def-emits-var :eval :flush
    :init :inits :need-prompt :print :print-no-newline :prompt :quit-prompt
    :read :reader :repl-requires :repl-verbose :source-map-inline :watch
    :watch-fn :wrap})

(defmacro ^:private err-out [& body] `(binding [*out* *err*] ~@body))
...
(defprotocol IParseStacktrace
  (-parse-stacktrace
    [repl-env stacktrace error build-options]
...
:working-dir to :output-dir."
  ...
  ([repl-env opts]
   ...
   (-> (into {} repl-env)
       (assoc :optimizations (or (:optimizations opts)
                                 (get repl-env :optimizations :none)))
       (assoc :output-dir (or (:output-dir opts)
                              (get repl-env :working-dir ".repl"))))))

(defn add-url
  [ijs]
  (cond-> ijs
    (not (contains? ijs :url)) (assoc :url (io/resource (:file ijs)))))

(defn ns->input
  [ns opts]
  (or (some-> (util/ns->source ns)
              (ana/parse-ns opts))
      (some-> (get-in @env/*compiler* [:js-dependency-index (str ns)])
              add-url)
      (some-> (deps/find-classpath-lib ns))
      (throw (ex-info (str ns " does not exist") {::error :invalid-ns}))))
...
(defn- load-sources
  ...
  (if (:output-dir opts)
    ...
    (doseq [{:keys [url provides]} sources] (-load repl-env provides url))))
...
(defn load-namespace
  ...
  ([repl-env ns opts]
   (let [ns (if (and (seq? ns) (= (first ns) 'quote)) (second ns) ns)
         sources (seq (when-not (ana/node-module-dep? ns)
     ...
     (when (:repl-verbose opts)
       (println (str "load-namespace " ns " , compiled:")
          ...
          (let [t (util/last-modified smf)]
            (if (or (and (= ns 'cljs.core)
                    ...
                    (and (not= ns 'cljs.core)
                         (> t
                            (get-in compiler-env
                                    [::source-maps ns :last-modified]
                                    0))))
              (swap! env/*compiler* assoc-in
                [::source-maps ns]
                {:last-modified t,
                 :source-map (sm/decode
...
(defn- mapped-frame
  ...
  (let [no-source-file? (if-not file true (.startsWith file "<"))
        rfile (when-not no-source-file?
                (io/file (URL. (.toURL (io/file (util/output-directory opts)))
                               file)))
        [sm {:keys [ns source-file], :as ns-info}]
          (when-not no-source-file? ((juxt read-source-map ns-info) rfile))
        [line' column' call]
          (if ns-info (mapped-line-column-call sm line column) [line column])
        name' (when (and ns-info function) function)
        file' (if no-source-file?
                file
                (string/replace
                  (.getCanonicalFile (if ns-info source-file (io/file rfile)))
...
(defn mapped-stacktrace
  ...
  ([stacktrace opts]
   (vec (let [mapped-frames (map (memoize #(mapped-frame % opts)) stacktrace)]
          ;; take each non-nil :call and optionally merge it into :function
          ;; one-level up
          ;; to avoid replacing with local symbols, we only replace munged name
          ;; if we can munge call symbol back to it
          (map
            #(merge-with
               (fn [munged-fn-name unmunged-call-name]
                 (if (= munged-fn-name
                        (string/replace (cljs.compiler/munge unmunged-call-name)
                                        "."
                                        "$"))
                   unmunged-call-name
                   munged-fn-name))
            ...
            (concat (rest (map #(if (:call %) (hash-map :function (:call %)) {})
                            mapped-frames))
                    [{}]))))))
...
(defn print-mapped-stacktrace
  ...
  ([stacktrace opts]
   (doseq [{:keys [function file line column]} (mapped-stacktrace stacktrace
                                                                  opts)]
     (err-out (println "\t"
...
(comment (def st (env/default-compiler-env))
         ...
         (env/with-compiler-env
           st
           (mapped-stacktrace
             [{:file "hello/core.js", :function "first", :line 6, :column 0}]
             {:output-dir "samples/hello/out"}))
         (env/with-compiler-env
           st
         ...
         (env/with-compiler-env
           st
           (mapped-stacktrace
             [{:file "cljs/core.js", :function "first", :line 2, :column 1}]
             {:output-dir "out"}))
         (env/with-compiler-env
           st
           (print-mapped-stacktrace
   ...
   (err-out
     ...
     (when-let [st (:stacktrace ret)]
       (if (and (true? (:source-map opts))
         ...
         (let [cst (try (-parse-stacktrace repl-env st ret opts)
                        (catch Throwable e
                          (when (:repl-verbose opts)
  ...
  ([repl-env env filename form wrap opts]
   (binding [ana/*cljs-file* filename]
     (let [env (merge env
           ...
           ast (if-not (#{:ns :ns*} (:op ast))
                 ...
                 (let [ijs (ana/parse-ns [form])]
                   (cljsc/handle-js-modules
                     opts
                     (deps/dependency-order (cljsc/add-dependency-sources [ijs]
                                                                          opts))
                     env/*compiler*)
                   (binding [ana/*check-alias-dupes* false]
                     (ana/no-warn (->ast form))))) ;; need new AST after we know
       ...
       (when (#{:ns :ns*} (:op ast))
         (let [ast (try (ana/no-warn (ana/analyze env form nil opts))
                        (catch #?(:clj Exception
                                  :cljs js/Error) e
       ...
       (let [ret (-evaluate repl-env filename (:line (meta form)) wrap-js)]
         (case (:status ret)
           :error (throw (ex-info (:value ret)
                                  {:type :js-eval-error,
                                   :error ret,
                                   :repl-env repl-env,
                                   :form form}))
           :exception (throw (ex-info (:value ret)
   ...
   (if (:output-dir opts)
     (let [src (cond (util/url? f) f
       ...
       (when-let [ns (and (:source-map opts) (first (:provides compiled)))]
         (spit (io/file (io/file (util/output-directory opts))
...
(defn- load-path->cp-path
  ...
  (let [src (if (= File/separatorChar (first path))
        ...
        src (.substring src 1)]
    (or (io/resource (str src ".cljs")) (io/resource (str src ".cljc")))))

(defn- wrap-fn
  [form]
  (cond
    (and (seq? form)
         (#{'ns 'require 'require-macros 'use 'use-macros 'import
            'refer-clojure}
          (first form)))
      identity
    ('#{*1 *2 *3 *e} form) (fn [x] `(cljs.core.pr-str ~x))
    :else (fn [x]
            `(try (cljs.core.pr-str (let [ret# ~x]
                                      (set! *3 *2)
                                      (set! *2 *1)
                                      (set! *1 ret#)
                                      ret#))
                  (catch :default e# (set! *e e#) (throw e#))))))
...
(defn eval-cljs
  ...
  ([repl-env env form opts]
   (evaluate-form repl-env
                  ...
                  "<cljs repl>"
                  form
                  ;; the pluggability of :wrap is needed for older JS runtimes
                  ;; like Rhino
...
(defn decorate-specs
  ...
  (if-let [k (some #{:reload :reload-all} specs)]
    (->> specs
         (remove #{k})
         (map #(vary-meta % assoc :reload k)))
    specs))

(comment (ana/canonicalize-specs '['foo.bar '[bar.core :as bar]])
         (ana/canonicalize-specs '['foo.bar '[bar.core :as bar] :reload])
         (map meta
           (decorate-specs (ana/canonicalize-specs
...
(def default-special-fns
  (let [load-file-fn (fn self
                       ([repl-env env form] (self repl-env env form nil))
        ...
        load-fn (fn self
                  ([repl-env env form] (self env repl-env form nil))
...
(defn repl-caught
  ...
  (if (and (instance? IExceptionInfo e)
    ...
    (let [{:keys [type repl-env error form js]} (ex-data e)]
      (case type
        :js-eval-error (display-error repl-env error form opts)
        :js-eval-exception (display-error repl-env
                                          error
                                          form
                                          (if (:repl-verbose opts)
                                            #(prn "Error evaluating:" form
                                                  :as js)
                                            (constantly nil))
                                          opts)))
...
(defn maybe-install-npm-deps
  ...
  (when (:install-deps opts)
    ...
    (swap! env/*compiler* update-in
      [:npm-deps-installed?]
      (fn [installed?]
        (if-not installed? (cljsc/maybe-install-node-deps! opts) installed?)))))

(defn initial-prompt [quit-prompt prompt] (quit-prompt) (prompt) (flush))

(defn repl*
  [repl-env
   {:keys [init inits need-prompt quit-prompt prompt flush read eval print
           caught reader print-no-newline source-map-inline wrap repl-requires
           ::fast-initial-prompt? compiler-env bind-err],
    :or {need-prompt #(if (readers/indexing-reader? *in*)
         ...
         repl-requires '[[cljs.repl :refer-macros
                          [source doc find-doc apropos dir pst]]
                         [cljs.pprint :refer [pprint] :refer-macros [pp]]],
         bind-err true},
    :as opts}]
  (doseq [[unknown-opt suggested-opt]
            (util/unknown-opts (set (keys opts))
                               (set/union known-repl-opts cljsc/known-opts))]
    (when suggested-opt
      (println (str "WARNING: Unknown option '"
                    unknown-opt
                    "'. Did you mean '"
                    suggested-opt
                    "'?"))))
  (when fast-initial-prompt? (initial-prompt quit-prompt prompt))
                  ...
                  (let [warnings (opts :warnings)]
                    (merge
                      ...
                      (if (or (true? warnings) (false? warnings))
                        ...
                        warnings)
                      (zipmap [:unprovided :undeclared-var :undeclared-ns
                               :undeclared-ns-form]
        ...
        (try
          (let [env {:context :expr, :locals {}}
                ...
                opts (comp/with-core-cljs
                       opts
                       (fn []
                         (if-let [merge-opts (:merge-opts (-setup repl-env
                                                                  opts))]
                           (merge opts merge-opts)
                           opts)))
                init (do (evaluate-form
                           repl-env
                           env
                           "<cljs repl>"
                ...
                maybe-load-user-file
                  #(when-let [user-resource (util/ns->source 'user)]
                    ...
                    (let [input
                      ...
                      (or ({request-exit request-exit,
                          ...
                          (if (and (seq? input) (is-special-fn? (first input)))
                            (do ((get special-fns (first input))
                                  repl-env
                                  env
                                  input
                                  opts)
              ...
              (fn []
                (binding [*repl-opts* opts]
                  (try (when analyze-path
                       ...
                       (maybe-load-user-file)
                       (catch Throwable e (caught e repl-env opts)))
                  (when-let [src (:watch opts)]
                    (.start
                      (Thread.
                        ((ns-resolve 'clojure.core 'binding-conveyor-fn)
                          (fn []
                            (let [log-file (io/file (util/output-directory opts)
                                                    "watch.log")]
                              (err-out (println
                                         "Watch compilation log available at:"
                                         (str log-file)))
                              (try (let [log-out (FileWriter. log-file)]
                                     (binding [*err* log-out
                                               *out* log-out]
                                       (cljsc/watch src
                                                    (dissoc opts :watch)
                    ...
                    (loop []
                      (when-not (try (identical? (read-eval-print) request-exit)
                                     (catch Throwable e
                                       (caught e repl-env opts)
                                       nil))
                        (when (need-prompt) (prompt) (flush))
                        (recur))))))))
          (finally (reset! done? true) (-tear-down repl-env)))))))
...
(def special-doc-map
  '{. {:forms [(.instanceMethod instance args*) (.-instanceField instance)],
       :doc
         "The instance member form works for methods and fields.
  They all expand into calls to the dot operator at macroexpansion time."},
    ns
      {:forms [(name docstring? attr-map? references*)],
       :doc
  ...
  Execution then jumps back to the recursion point, a loop or fn method."},
    ...
    throw {:forms [(throw expr)], :doc "The expr is evaluated and thrown."},
    try
     ...
     (binding [cljs.core/*print-newline* true]
       (with-out-str
         ~(if-let [special-name ('{& fn, catch try, finally try} name)]
            `(doc ~special-name)
            (cond
              (special-doc-map name) `(cljs.repl/print-doc (quote ~(special-doc
                                                                     name)))
              (repl-special-doc-map name) `(cljs.repl/print-doc
                                             (quote ~(repl-special-doc name)))
              (keyword? name) `(cljs.repl/print-doc
                                 {:spec ~name,
                                  :doc (cljs.spec.alpha/describe ~name)})
              ...
              (ana-api/resolve &env name)
                `(cljs.repl/print-doc
                   (quote
                     ~(let [var (ana-api/resolve &env name)
                            m (select-keys var
 ...
 contains a match for re-string-or-pattern"
  ...
  (let [re (re-pattern re-string-or-pattern)
                              ...
                              (update-in
                                (select-keys m
                                             [:ns :name :doc :forms :arglists
                                              :macro :url])
                                [:name]
                                #(if-not (nil? %) (clojure.core/name %) %)))
                         (sort-by :name (vals (ana-api/ns-interns ns)))))
               (ana-api/all-ns))
             (map #(select-keys (ana-api/find-ns %) [:name :doc])
               (ana-api/all-ns))
        ...
        ms (for [m ms
             ...
             m)]
    `(doseq [m# (quote ~ms)] (cljs.repl/print-doc m#))))
...
(comment (def cenv (env/default-compiler-env))
         (def aenv (assoc-in (ana/empty-env) [:ns :name] 'cljs.user))
         (binding [ana/*cljs-ns* 'cljs.user]
           (env/with-compiler-env
             cenv
             (comp/with-core-cljs {}
                                  (fn [] (source-fn aenv 'cljs.core/first))))))
...
(defn- resolve-ns
  ...
  (or (get-in @env/*compiler* [::ana/namespaces ana/*cljs-ns* :requires ns-sym])
      (get-in @env/*compiler*
              [::ana/namespaces ana/*cljs-ns* :require-macros ns-sym])
      ns-sym))
