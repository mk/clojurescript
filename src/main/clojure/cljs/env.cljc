...
;; multiple (lower-level) entry points.  Any state needed by the compiler
;; across
...
;; {})
;; * :js-dependency-index - result from calling
...
(defn default-compiler-env*
  [options]
  (merge {:cljs.analyzer/namespaces {'cljs.user {:name 'cljs.user}},
...
#?(:clj (defmacro ensure
          ...
          `(let [val# *compiler*]
             (if (nil? val#)
               (push-thread-bindings (hash-map (var *compiler*)
                                               (default-compiler-env))))
             (try ~@body (finally (if (nil? val#) (pop-thread-bindings)))))))
