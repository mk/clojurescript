...
(ns cljs.analyzer
  #?(:clj (:refer-clojure :exclude [macroexpand-1 ensure])
     :cljs (:refer-clojure :exclude
                           [macroexpand-1 ns-interns ensure js-reserved]))
  #?(:cljs (:require-macros [cljs.analyzer.macros :refer
                             [no-warn wrapping-errors disallowing-recur
                              allowing-redef disallowing-ns*]]
...
(defn- add-identity [m] (assoc m :identity (swap! identity-counter inc)))

#?(:clj (def transit-read-opts
          (try
            ...
            (when-some [ns (find-ns 'cognitect.transit)]
              (let [read-handler @(ns-resolve ns 'read-handler)
                ...
                {:handlers (read-handler-map
                             {"cljs/js" (read-handler (fn [v] (JSValue. v))),
                              "cljs/regex" (read-handler
                                             (fn [v] (Pattern/compile v)))})}))
            (catch Throwable t nil))))

#?(:clj (def transit-write-opts
          (try (require '[cognitect.transit])
               (when-some [ns (find-ns 'cognitect.transit)]
                 (let [write-handler @(ns-resolve ns 'write-handler)
                       write-handler-map @(ns-resolve ns 'write-handler-map)]
                   {:handlers
                      (write-handler-map
                        {JSValue (write-handler (fn [_] "cljs/js")
                                                (fn [js] (.val ^JSValue js))),
                         Pattern (write-handler (fn [_] "cljs/regex")
                                                (fn [pat]
                                                  (.pattern ^Pattern pat)))})}))
               (catch Throwable t nil))))

#?(:clj (def transit
          (delay (try (require '[cognitect.transit])
                      (when-some [ns (find-ns 'cognitect.transit)]
...
(def js-reserved
  #{"arguments" "abstract" "await" "boolean" "break" "byte" "case" "catch"
    "char" "class" "const" "continue" "debugger" "default" "delete" "do"
    "double" "else" "enum" "export" "extends" "final" "finally" "float" "for"
    "function" "goto" "if" "implements" "import" "in" "instanceof" "int"
    "interface" "let" "long" "native" "new" "package" "private" "protected"
    "public" "return" "short" "static" "super" "switch" "synchronized" "this"
    "throw" "throws" "transient" "try" "typeof" "var" "void" "volatile" "while"
    "with" "yield" "methods" "null" "constructor"})

(def es5-allowed #{"default"})
...
(defn gets
  ([m k0 k1]
   (let [m (get m k0 SENTINEL)] (when-not (identical? m SENTINEL) (get m k1))))
  ([m k0 k1 k2]
...
#?(:cljs (def CLJ_NIL_SYM 'clj-nil))

#?(:cljs (def NUMBER_SYM 'number))

#?(:cljs (def STRING_SYM 'string))

(def BOOLEAN_SYM 'boolean)

#?(:cljs (def JS_STAR_SYM 'js*))

#?(:cljs (def DOT_SYM '.))

#?(:cljs (def NEW_SYM 'new))

#?(:cljs (def CLJS_CORE_SYM 'cljs.core))

#?(:cljs (def CLJS_CORE_MACROS_SYM 'cljs.core$macros))

(def IGNORE_SYM 'ignore)

(def ANY_SYM 'any)

#?(:cljs (defn ^boolean cljs-seq? [x] (implements? ISeq x)))

#?(:cljs (defn ^boolean cljs-map? [x] (implements? IMap x)))

#?(:cljs (defn ^boolean cljs-vector? [x] (implements? IVector x)))

#?(:cljs (defn ^boolean cljs-set? [x] (implements? ISet x)))

#?(:cljs (defn munge-path [ss] (munge (str ss))))
...
#?(:cljs
     ...
     provide the file extension, defaults to :cljs."
       ...
       ([ns ext] (str (string/replace (munge-path ns) \. \/) "." (name ext)))))
...
#?(:cljs (defn topo-sort
           ...
           ([x depth state memo-get-deps]
            (let [deps (memo-get-deps x)]
              ...
              (doseq [dep deps] (topo-sort dep (inc depth) state memo-get-deps))
              (doseq [[<depth _] (subseq @state < depth)]
                (swap! state update-in [<depth] set/difference deps))
              (when (= depth 0) (distinct (apply concat (vals @state))))))))

(declare message namespaces)

(defn ast? #?(:cljs {:tag boolean}) [x] (and (map? x) (contains? x :op)))
...
(defmethod error-message :redef
  [warning-type info]
  (str (:sym info)
       " already refers to: " (symbol (str (:ns info)) (str (:sym info)))
...
(defmethod error-message :fn-arity
  [warning-type info]
  (str "Wrong number of args (" (:argc info)
       ") passed to " (or (:ctor info) (:name info))))
...
(defmethod error-message :protocol-invalid-method
  ...
  (if (:no-such-method info)
    (str "Bad method signature in protocol implementation, " (:protocol info)
         " does not declare method called " (:fname info))
    (str "Bad method signature in protocol implementation, " (:protocol info)
         " " (:fname info)
         " does not declare arity " (:invalid-arity info))))
...
(defmethod error-message :extending-base-js-type
  ...
  (str "Extending an existing JavaScript type - use a different symbol name "
       "instead of " (:current-symbol info)
       " e.g " (:suggested-symbol info)))
    ...
    (cljs.core/checked-aget cljs.core/checked-aget')
      (str
        "cljs.core/aget, arguments must be an array followed by numeric indices, got "
          types
        " instead" (when (or (= 'object (first types))
...
(defmethod error-message :invoke-ctor
  [warning-type info]
  (str "Cannot invoke type constructor "
       (-> info
           :fexpr
           :info
           :name)
...
(defmethod error-message :extend-type-invalid-method-shape
  ...
  (str "Bad extend-type method shape for protocol "
       protocol
       " method "
       method
...
(defmethod error-message :unsupported-js-module-type
  ...
  (str "Unsupported JavaScript module type "
       module-type
       " for foreign library "
       file
       "."))
...
(defmethod error-message :unsupported-preprocess-value
  ...
  (str "Unsupported preprocess value "
       preprocess
       " for foreign library "
       file
       "."))
...
(defmethod error-message :infer-warning
  ...
  (case warn-type
    ...
    :property (str "Cannot resolve property " property
                   " for inferred type " type
                   " in expression " form)
    :object (str "Adding extern to Object for property "
                 property
                 " due to "
...
(def ^:dynamic *cljs-warning-handlers* [default-warning-handler])

#?(:clj (defmacro with-warning-handlers
          [handlers & body]
          `(binding [*cljs-warning-handlers* ~handlers] ~@body)))
...
(defn- hex-format
  ...
  #?(:clj (str "_u" (format (str "%0" pad "x") (int (first s))) "_")
     :cljs (let [hex (.toString (.charCodeAt s 0) 16)
                 len (. hex -length)
                 hex
                   (if (< len pad) (str (repeat-char "0" (- pad len)) hex) hex)]
             (str "_u" hex "_"))))

(defn gen-constant-id
  [value]
  (let [prefix (cond (keyword? value) "cst$kw$"
                     (symbol? value) "cst$sym$"
                     :else (throw #?(:clj (Exception. (str "constant type "
                                                           (type value)
                                                           " not supported"))
                                     :cljs (js/Error. (str "constant type "
                                                           (type value)
                                                           " not supported")))))
         ...
         env
           (update-in
             ...
             (fn [{:keys [seen order], :or {seen #{}, order []}, :as constants}]
               (cond-> constants
                 (not (contains? seen val)) (assoc :seen
                                              (conj seen val) :order
                                              (conj order val))))))))))
...
#?(:clj (defmacro no-warn
          [& body]
          (let [no-warnings (zipmap (keys *cljs-warnings*) (repeat false))]
            `(binding [*cljs-warnings* ~no-warnings] ~@body))))

#?(:clj (defmacro all-warn
          [& body]
          (let [all-warnings (zipmap (keys *cljs-warnings*) (repeat true))]
            `(binding [*cljs-warnings* ~all-warnings] ~@body))))
...
(defn intern-macros
  ...
  ([ns reload]
   (when (or (nil? (gets @env/*compiler* ::namespaces ns :macros)) reload)
     (swap! env/*compiler* assoc-in
       [::namespaces ns :macros]
       (->> #?(:clj (ns-interns ns)
               :cljs (ns-interns* ns))
...
#?(:clj (defmacro with-core-macros
          [path & body]
          `(do (when (not= *cljs-macros-path* ~path)
                 (reset! -cljs-macros-loaded false))
               (binding [*cljs-macros-path* ~path] ~@body))))

#?(:clj (defmacro with-core-macros-file
          [path & body]
          `(do (when (not= *cljs-macros-path* ~path)
                 (reset! -cljs-macros-loaded false))
...
(defn empty-env
  ...
  []
  (ensure {:ns (get-namespace *cljs-ns*),
           :context :statement,
           :locals {},
           :fn-scope [],
           :js-globals (into {}
...
(defn source-info
  ...
  ([name env]
   (cond-> {:file (if (= (-> env
     ...
     (:root-source-info env) (merge (select-keys env [:root-source-info])))))
...
(defn warning
  [warning-type env extra]
  (doseq [handler *cljs-warning-handlers*] (handler warning-type env extra)))
...
(defn confirm-var-exist-warning
  ...
  (fn [env prefix suffix]
    (warning :undeclared-var
             ...
             {:prefix prefix,
              :suffix suffix,
              :macro-present? (not (nil? (get-expander (symbol (str prefix)
                                                               (str suffix))
                                                       env)))})))
...
(defn- internal-js-module-exists?
  ...
  (contains? (into #{} (mapcat (fn [[k v]] [k (:name v)])) js-module-index)
             (str module)))
...
(defn node-module-dep?
  #?(:cljs {:tag boolean})
  [module]
  #?(:clj (contains? (get-in @env/*compiler* [:node-module-index]) (str module))
     :cljs (try (and (= *target* "nodejs")
                     (boolean (js/require.resolve (str module))))
                (catch :default _ false))))

(defn dep-has-global-exports?
  [module]
  (let [global-exports (get-in @env/*compiler*
                               [:js-dependency-index (str module)
                                :global-exports])]
  ...
  ([env prefix suffix missing-fn]
   (let [sufstr (str suffix)
         suffix-str (if (and #?(:clj (not= ".." sufstr)
                                :cljs (not (identical? ".." sufstr))) ;; leave
                                                                      ;; cljs.core$macros/..
                                                                      ;; alone
                             #?(:clj (re-find #"\." sufstr)
...
(defn resolve-macro-ns-alias
  ...
  ([env name not-found]
   (let [sym (symbol name)] (get (:require-macros (:ns env)) sym not-found))))
...
(defn core-name?
  ...
  (and (or (some? (gets @env/*compiler* ::namespaces 'cljs.core :defs sym))
           (if-some [mac (get-expander sym env)]
             (let [^Namespace ns (-> mac
                                     meta
                                     :ns)]
               (= (.getName ns)
                  #?(:clj 'cljs.core
                     :cljs 'cljs.core$macros)))
             false))
       (not (contains? (-> env
                           :ns
                           :excludes)
...
(defn public-name?
  ...
  [ns sym]
  (let [var-ast (or
                  (gets @env/*compiler* ::namespaces ns :defs sym)
                  #?(:clj (gets @env/*compiler* ::namespaces ns :macros sym)
                     :cljs (gets @env/*compiler*
                                 ::namespaces (symbol (str (name ns) "$macros"))
                                 :defs sym)))]
    (and (some? var-ast) (not (or (:private var-ast) (:anonymous var-ast))))))

(defn js-tag? [x] (and (symbol? x) (or (= 'js x) (= "js" (namespace x)))))

(defn normalize-js-tag
  [x]
...
(defn has-extern?*
  ([pre externs]
   (let [pre (if-some [me (find (get-in externs '[Window prototype])
                                (first pre))]
               (if-some [tag (-> me
                                 first
                                 meta
   ...
   (cond (empty? pre) true
         :else
           (let [x (first pre)
                 me (find externs x)]
             (cond (not me) false
                   :else (let [[x' externs'] me
                               xmeta (meta x')]
                           (if (and (= 'Function (:tag xmeta)) (:ctor xmeta))
                             (or (has-extern?* (into '[prototype] (next pre))
                                               externs'
                                               top)
...
(defn has-extern?
  ([pre] (has-extern? pre (get @env/*compiler* ::externs)))
  ([pre externs]
   (or (has-extern?* pre externs)
       ...
       (-> (last pre)
           str
           (string/starts-with? "cljs$")))))

(defn js-tag
  ([pre] (js-tag pre :tag))
  ([pre tag-type] (js-tag pre tag-type (get @env/*compiler* ::externs)))
     ...
     (let [tag (-> p
       ...
       (if (= (count pre) 1)
         ...
         (or (js-tag (next pre) tag-type externs' top)
             (js-tag (into '[prototype] (next pre))
                     tag-type
                     (get top tag)
                     top)))))))
...
(defn dotted-symbol?
  [sym]
  (let [s (str sym)]
    #?(:clj (and (.contains s ".") (not (.contains s "..")))
...
(defn resolve-alias
  ...
  (if (and (= 'cljs.core ns) ('#{aget aset} sym) (checked-arrays))
    (get-in '{:warn {aget checked-aget, aset checked-aset},
...
(defmethod resolve* :js
  ...
  {:name (symbol (str full-ns) (str (name sym))), :op :js-var, :ns full-ns})
  ...
  (let [pre (into '[Object]
    ...
    (when-not (has-extern? pre)
      (swap! env/*compiler* update-in
        (into [::namespaces current-ns :externs] pre)
        merge
        {}))
    {:name (symbol (str current-ns)
...
(def ^:private private-var-access-exceptions
  ...
  '#{cljs.core/checked-aget cljs.core/checked-aset cljs.core/checked-aget'
     cljs.core/checked-aset'})

(defmethod resolve* :default
  [env sym full-ns current-ns]
  (let [sym-ast
          (gets @env/*compiler* ::namespaces full-ns :defs (symbol (name sym)))
...
(defn resolve-invokeable-ns
  [ns current-ns env]
  (let [ns (resolve-ns-alias env ns)
        module-type (ns->module-type ns)]
    (case module-type
      :js {:name (symbol (or (gets @env/*compiler* :js-module-index ns :name)
       ...
       (let [s (str sym)
         ...
         (cond
           ...
           (dotted-symbol? sym)
             (let [idx (.indexOf s ".")
               ...
               (if-some [lb (handle-symbol-local prefix (get locals prefix))]
                 {:op :local, :name (symbol (str (:name lb) "." suffix))}
                 (if-some [full-ns (gets @env/*compiler*
                                         ::namespaces current-ns
                                         :imports prefix)]
                   {:op :js-var, :name (symbol (str full-ns) suffix)}
                   (if-some [info (gets @env/*compiler*
                                        ::namespaces current-ns
                                        :defs prefix)]
                     (merge info
           ...
           (some? (gets @env/*compiler* ::namespaces current-ns :uses sym))
             (let [full-ns
                     (gets @env/*compiler* ::namespaces current-ns :uses sym)]
               (resolve* env sym full-ns current-ns))
           (some? (gets @env/*compiler* ::namespaces current-ns :renames sym))
             (let [qualified-symbol
                     (gets @env/*compiler* ::namespaces current-ns :renames sym)
                   full-ns (symbol (namespace qualified-symbol))
                   sym (symbol (name qualified-symbol))]
               (resolve* env sym full-ns current-ns))
           (some? (gets @env/*compiler* ::namespaces current-ns :imports sym))
             (recur env
                    (gets @env/*compiler* ::namespaces current-ns :imports sym)
                    confirm)
           (some? (gets @env/*compiler* ::namespaces current-ns :defs sym))
             (do (when (some? confirm) (confirm env current-ns sym))
                 (merge (gets @env/*compiler* ::namespaces current-ns :defs sym)
                        {:name (symbol (str current-ns) (str sym)),
                         :op :var,
                         :ns current-ns}))
           (core-name? env sym)
             (do (when (some? confirm) (confirm env 'cljs.core sym))
                 (merge (gets @env/*compiler* ::namespaces 'cljs.core :defs sym)
                        {:name (symbol "cljs.core" (str sym)),
                         :op :var,
                         :ns 'cljs.core}))
           (invokeable-ns? s env) (resolve-invokeable-ns s current-ns env)
           :else (do (when (some? confirm) (confirm env current-ns sym))
                     (merge
                       (gets @env/*compiler* ::namespaces current-ns :defs sym)
                       {:name (symbol (str current-ns) (str sym)),
                        :op :var,
                        :ns current-ns}))))))))
...
(defn resolve-macro-var
  ...
  (let [ns (-> env
    ...
    (cond (some? (namespace sym))
            (let [ns (namespace sym)
                  ns (if (= "clojure.core" ns) "cljs.core" ns)
                  full-ns (resolve-macro-ns-alias env ns)
                  #?@(:cljs [full-ns
                             (if-not (string/ends-with? (str full-ns) "$macros")
          ...
          (some? (get-in namespaces [ns :rename-macros sym]))
            (let [qualified-symbol (get-in namespaces [ns :rename-macros sym])
                  ...
                  sym (symbol (name qualified-symbol))]
              (get-in namespaces [full-ns :macros sym]))
          :else (let [ns (cond (some? (get-in namespaces [ns :macros sym])) ns
                               (core-name? env sym) #?(:clj 'cljs.core
                                                       :cljs
                                                         CLJS_CORE_MACROS_SYM))]
                  (when (some? ns)
...
#?(:clj (defmacro disallowing-recur
          [& body]
          `(binding [*recur-frames* (cons nil *recur-frames*)] ~@body)))

#?(:clj
     (defmacro allowing-redef [& body] `(binding [*allow-redef* true] ~@body)))

#?(:clj
     (defmacro disallowing-ns* [& body] `(binding [*allow-ns* false] ~@body)))
...
(defn find-matching-method
  ...
  (let [methods (or (:methods f)
                    (-> f
                        :info
                        :methods))
        c (count params)]
    (some (fn [m] (and (or (== (:fixed-arity m) c) (:variadic? m)) m))
          methods)))
        ...
        (cond
          (or #?(:clj (= then-tag else-tag)
              ...
              #?(:clj (= else-tag IGNORE_SYM)
                 :cljs (symbol-identical? else-tag IGNORE_SYM)))
            then-tag
          #?(:clj (= then-tag IGNORE_SYM)
             :cljs (symbol-identical? then-tag IGNORE_SYM))
            else-tag
          ...
          (and (or (some? (get NOT_NATIVE then-tag)) (type? env then-tag))
            ...
            'clj
          :else (if (and (some? (get BOOLEAN_OR_SEQ then-tag))
...
(defn infer-tag
  ...
  (if-some [tag (get-tag e)]
    ...
    (case (:op e)
      ...
      (:var :local :js-var :binding) (if-some [init (:init e)]
                                       (infer-tag env init)
...
(defn var-meta
  ...
  ([var expr-env]
   (let [sym (:name var)
         ...
         m (merge (let [user-meta (:meta var)
                        uks (keys user-meta)]
                    (zipmap uks (map #(list 'quote (get user-meta %)) uks)))
...
(defn var-ast
  ...
  ;; want the var's AST and `resolve-var` will check locals first. - António
  ;; Monteiro
    ...
    (let [env (dissoc env :locals)
      ...
      (when-some [var-ns (:ns var)]
        {:var (analyze expr-env sym),
         :sym (analyze expr-env
                       `(quote ~(symbol (name var-ns) (name (:name var))))),
         :meta (var-meta var expr-env)}))))

(defmethod parse 'var
  [op env [_ sym :as form] _ _]
  (when (not= 2 (count form)) (throw (error env "Wrong number of args to var")))
  (when-not (symbol? sym) (throw (error env "Argument to var must be symbol")))
  (merge {:env env, :op :the-var, :children [:var :sym :meta], :form form}
         (var-ast env sym)))
...
(defn- simple-predicate-induced-tag
  ...
  (when (and (list? test) (== 2 (count test)) (every? symbol? test))
    (let [analyzed-fn (no-warn (analyze (assoc env :context :expr)
                                        (first test)))]
      (when (= :var (:op analyzed-fn))
        (when-let [tag (predicate->tag (:name analyzed-fn))]
          (let [sym (last test)]
            (when (and (nil? (namespace sym)) (get-in env [:locals sym]))
              [sym tag])))))))
...
(defn- type-check-induced-tag
  ...
  [env test]
  (when (and (list? test) (== 3 (count test)) (every? symbol? test))
    (let [analyzed-fn (no-warn (analyze (assoc env :context :expr)
                                        (first test)))]
      (when (= :var (:op analyzed-fn))
        (when ('#{cljs.core/instance?} (:name analyzed-fn))
          (let [analyzed-type (no-warn (analyze (assoc env :context :expr)
                                                (second test)))
  ...
  (let [[local tag] (or (simple-predicate-induced-tag env test)
    ...
    (cond-> env
      local (update-in
              [:locals local :tag]
              (fn [prev-tag]
                (if (or (nil? prev-tag) (= 'any prev-tag)) tag prev-tag))))))

(defmethod parse 'if
  [op env [_ test then else :as form] name _]
  (when (< (count form) 3) (throw (error env "Too few arguments to if")))
  (when (> (count form) 4) (throw (error env "Too many arguments to if")))
  (let [test-expr (disallowing-recur (analyze (assoc env :context :expr) test))
        then-expr (allowing-redef (analyze (add-predicate-induced-tags env test)
                                           then))
...
(defmethod parse 'try
  ...
  (let [catchenv (update-in env [:context] #(if (= :expr %) :return %))
          ...
          (loop [parser {:state :start,
            ...
            (if (seq? (:forms parser))
              (let [[form & forms*] (:forms parser)
                ...
                (case (:state parser)
                  :start
                    (cond (catch? form) (recur (assoc parser :state :catches))
                          (finally? form) (recur (assoc parser :state :finally))
                          :else (recur (update-in parser* [:body] conj form)))
                  :catches
                    (cond (default? form) (recur (assoc parser*
                                                   :dblock form
                                                   :state :finally))
                          (catch? form)
                  ...
                  :finally (recur (assoc parser*
                                    :fblock form
                                    :state :done))
                  :done (throw (error env "Unexpected form after finally"))))
              parser))
        finally (when (seq fblock)
                  (-> (disallowing-recur (analyze (assoc env
                                                    :context :statement)
                                                  `(do ~@(rest fblock))))
                      (assoc :body? true)))
        ...
        locals (if e
                 (assoc locals
                   e {:name e, :line (get-line e env), :column (get-col e env)})
                 locals)
        catch (when cblock
                (disallowing-recur (analyze (assoc catchenv :locals locals)
                                            cblock)))
        try (disallowing-recur (analyze (if (or e finally) catchenv env)
                                        `(do ~@body)))]
    {:env env,
     :op :try,
     :form form,
     ...
     :catch catch,
     :children
       (vec (concat [:body] (when catch [:catch]) (when finally [:finally])))}))

(defn valid-proto [x] (when (symbol? x) x))

(defn elide-env [env ast opts] (dissoc ast :env))

(defn replace-env-pass [new-env] (fn [env ast opts] (assoc ast :env new-env)))
...
(defn const-expr->constant-value
  [{:keys [op], :as e}]
  (case op
...
(defmethod parse 'def
  [op env form _ _]
  (when (> (count form) 4) (throw (error env "Too many arguments to def")))
  (let [pfn (fn ([_ sym] {:sym sym})
        ...
        sym (cond
              (and sym-ns
                   (not #?(:clj (= (symbol sym-ns) ns-name)
                           :cljs (symbol-identical? (symbol sym-ns) ns-name))))
                (throw (error env
                              (str "Can't def ns-qualified name in namespace "
                                   sym-ns)))
    ...
    (when-some [doc (:doc args)]
      (when-not (string? doc) (throw (error env "Too many arguments to def"))))
    ...
    (when-some [v (get-in @env/*compiler* [::namespaces ns-name :defs sym])]
      ...
      (when (and (:declared v)
        ...
        (warning :declared-arglists-mismatch
                 env
                 {:ns-name ns-name,
                  :sym sym,
                  :declared (second (:arglists v)),
                  :defined (second (:arglists sym-meta))})))
    (let [env (if (or (and (not= ns-name 'cljs.core) (core-name? env sym))
                      (some? (get-in @env/*compiler*
                                     [::namespaces ns-name :uses sym])))
                (let [ev (resolve-existing-var (dissoc env :locals)
                                               ;; ::no-resolve true is to
                                               ;; suppress "can't take value
                                               ;; of macro warning" when sym
                                               ;; resolves to a macro
                                               (with-meta sym
                                                 {::no-resolve true}))
                  ...
                  (when (public-name? (:ns ev) sym)
                    (warning :redef
                             env
                             {:sym sym, :ns (:ns ev), :ns-name ns-name}))
                  (swap! env/*compiler* update-in
                    [::namespaces ns-name :excludes]
                    conj-to-set
                    sym)
          ...
          export-as (when-let [export-val (-> sym
                                              meta
                                              :export)]
      ...
      (when (or (nil? (get-in @env/*compiler* [::namespaces ns-name :defs sym]))
                (not (:declared sym-meta)))
        (when *file-defs* (swap! *file-defs* conj sym))
        (swap! env/*compiler* assoc-in
          [::namespaces ns-name :defs sym]
          (merge
            {:name var-name}
            ...
            (cond-> sym-meta (:test sym-meta) (assoc :test true))
            {:meta (-> sym-meta
                       (dissoc :test)
                       (update-in [:file]
                                  (fn [f]
            ...
            (when doc {:doc doc})
            (when const?
              (let [const-expr (binding [*passes* (conj *passes*
                                                        (replace-env-pass
                                                          {:context :expr}))]
                                 (analyze env (:init args)))]
                (when (constant-value? const-expr) {:const-expr const-expr})))
            (when (true? dynamic) {:dynamic true})
            (source-info var-name env)
            ;; the protocol a protocol fn belongs to
            (when protocol {:protocol protocol})
            ;; symbol for reified protocol
            (when-let [protocol-symbol (-> sym
                                           meta
                                           :protocol-symbol)]
              {:protocol-symbol protocol-symbol,
               ...
               :impls #{}})
            (when fn-var?
              (let [params (map #(vec (map :name (:params %)))
                             (:methods init-expr))]
                (merge
            ...
            (when (and (:declared sym-meta) (:arglists sym-meta))
              {:declared true,
               :fn-var true,
      ...
      (merge {:env env,
             ...
             (when (true? (:def-emits-var env)) {:var-ast (var-ast env sym)})
             (when-some [test (:test sym-meta)]
               {:test (analyze (assoc env :context :expr) test)})
             (when (some? tag) (if fn-var? {:ret-tag tag} {:tag tag}))
             (when (true? dynamic) {:dynamic true})
             (when (some? export-as) {:export export-as})
             (if (some? init-expr)
               {:init init-expr, :children [:var :init]}
               {:children [:var]})))))

(defn analyze-fn-method-param
  [env]
  (fn [[locals params] [arg-id name]]
    ...
    (let [line (get-line name env)
          ...
          nmeta (meta name)
          tag (:tag nmeta)
          shadow (when (some? locals) (handle-symbol-local name (locals name)))
          env (merge (select-keys env [:context]) {:line line, :column column})
          param {:op :binding,
                 :name name,
                 :line line,
                 :column column,
...
(defn fn-name-var
  ...
  (when (some? name)
    (let [ns (-> env
          ...
          shadow (when (nil? shadow) (get-in env [:js-globals name]))
...
(defmethod parse 'fn*
  ...
  (let [named-fn? (symbol? (first args))
        [name meths] (if named-fn? [(first args) (next args)] [name (seq args)])
        ;; turn (fn [] ...) into (fn ([]...))
        meths (if (vector? (first meths)) (list meths) meths)
        locals (:locals env)
        name-var (fn-name-var env locals name)
        env (if (some? name) (update-in env [:fn-scope] conj name-var) env)
        ...
        menv (merge menv
                    {:protocol-impl proto-impl, :protocol-inline proto-inline})
        methods (map #(disallowing-ns*
                        (analyze-fn-method menv locals % type (nil? name)))
                  meths)
        ...
        inferred-ret-tag (let [inferred-tags (map (partial infer-tag env)
                                               (map :body methods))]
                           (when (apply = inferred-tags) (first inferred-tags)))
        ast (merge {:op :fn,
                    :env env,
                    :form form,
                    :name name-var,
    ...
    (let [variadic-methods (into [] (comp (filter :variadic?) (take 1)) methods)
...
(defmethod parse 'letfn*
  ...
  (let [n->fexpr (into {} (map (juxt first second) (partition 2 bindings)))
          ...
          (reduce
            (fn [[{:keys [locals], :as env} bes] n]
              (let [ret-tag (-> n
                ...
                [(assoc-in env [:locals n] be) (conj bes be)]))
            [env []]
            names)
        ...
        [meth-env bes]
          (reduce
            (fn [[meth-env bes] {:keys [name shadow], :as be}]
              (let [env (assoc-in meth-env [:locals name] shadow)
                    ...
                    be' (assoc be
                          ...
                          :max-fixed-arity (:max-fixed-arity fexpr)
                          :method-params (map :params (:methods fexpr)))]
                [(assoc-in env [:locals name] be') (conj bes be')]))
            [meth-env []]
            bes)
        expr (-> (analyze (assoc meth-env
                            :context (if (= :expr context) :return context))
                          `(do ~@exprs))
                 (assoc :body? true))]
    {:env env,
     :op :letfn,
  ...
  (loop [bes []
    ...
    (if-some [[name init] (first bindings)]
      (let []
        ...
        (let [init-expr (analyze-let-binding-init env
                                                  init
                                                  (cons {:params bes}
                                                        *loop-lets*))
              line (get-line name env)
              ...
              be (if (= :fn (:op init-expr))
                   ...
                   (merge be
                          {:fn-var true,
                           ;; copy over the :fn-method information we need for
...
(defn analyze-let-body*
  [env context exprs]
  (analyze (assoc env :context (if (= :expr context) :return context))
           `(do ~@exprs)))
...
(defn analyze-let
  ...
  (let [context (:context encl-env)
        ...
        loop-lets (cond (true? is-loop) *loop-lets*
...
(defmethod parse 'recur
  ...
  (let [context (:context env)
        ...
        ;; Add dummy implicit target object if recuring to proto impl method
        ;; head
        ...
        exprs (disallowing-recur
                (vec (map #(analyze (assoc env :context :expr) %) exprs)))]
    (when-not frame (throw (error env "Can't recur here")))
    (when-not (= (count exprs) (count (:params frame)))
      (throw (error env
                    (str "recur argument count mismatch, expected: "
                           (count (:params frame))
                         " args, got: " (count exprs)))))
    (when (and (:protocol-impl frame) (not add-implicit-target-object?))
      (warning :protocol-impl-recur-with-target
               env
               {:form (:form (first exprs))}))
    (reset! (:flag frame) true)
    (swap! (:tags frame)
      (fn [tags] (mapv (fn [tag expr] (add-types tag (:tag expr))) tags exprs)))
...
(defn analyze-const
  ...
  (let [;; register constants
    ...
    {:op :const, :env env, :literal? true, :val form, :tag tag, :form form}))
...
(defmethod parse 'new
  ...
  (disallowing-recur
    (let [enve (assoc env :context :expr)
          ...
          record-args (when (and (:record ctor-var)
                                 (not (-> ctor
                                          meta
                                          :internal-ctor)))
      ...
      (binding [*private-var-access-nowarn* true]
        (let [enve (assoc env :context :expr)
              texpr
                (cond
                  (symbol? target)
                    (do (cond (and (= target '*unchecked-if*)   ;; TODO: proper
                                                                ;; resolve
                                   (or (true? val) (false? val)))
                                (set! *unchecked-if* val)
                              (and (= target '*unchecked-arrays*) ;; TODO:
                                                                  ;; proper
                                                                  ;; resolve
                                   (or (true? val) (false? val)))
                              ...
                              (and (= target '*warn-on-infer*)
                                ...
                                (set! *cljs-warnings*
                                      (assoc *cljs-warnings*
                                        :infer-warning val)))
                        (when (some? (:const (resolve-var (dissoc env :locals)
                                                          target)))
                          (throw (error env "Can't set! a constant")))
                        (let [local (handle-symbol-local target
                                                         (-> env
                                                             :locals
                                                             target))]
          ...
          ;; as top level fns are decomposed for Closure cross-module code
          ;; motion, we need to
              ...
              (when-let [info (and (= :fn (:op vexpr)) (:top-fn meta))]
                (swap! env/*compiler* update-in
                  [::namespaces
                   (-> env
                       :ns
                       :name) :defs sym :methods]
                  (fnil conj [])
                  ;; just use original fn meta, as the fn method is already
                  ;; desugared
                  ;; only get tag from analysis
                  (merge (select-keys info [:fixed-arity :variadic?])
                         (select-keys (-> vexpr
                                          :methods
                                          first)
                                      [:tag]))))))
          ...
          (cond (and (not (:def-emits-var env))                 ;; non-REPL
                                                                ;; context
                     (some? ('#{*unchecked-if* *unchecked-arrays*
                                *warn-on-infer*}
                             target)))
                  {:env env, :op :no-op}
                :else {:env env,
...
#?(:clj
     ...
     resource on the classpath or file from the root of the build."
       ...
       (or (util/ns->source ns)
           ;; Find sources available in inputs given to cljs.closure/build -
           ;; Juho Teperi
           (some (fn [source] (if (= ns (:ns source)) (:source-file source)))
                 (:sources @env/*compiler*))
           ;; Find sources in directory given to cljs.compiler/compile-root -
           ;; Juho Teperi
...
(defn analyze-deps
  ...
  ([lib deps env]
   (analyze-deps lib deps env (when env/*compiler* (:options @env/*compiler*))))
  ([lib deps env opts]
   (let [compiler @env/*compiler*]
     (binding [*cljs-dep-set* (vary-meta (conj *cljs-dep-set* lib)
                                         update-in
                                         [:dep-path]
                                         conj
                                         lib)]
...
(defn missing-use-macro?
  ...
  (when (symbol? lib)
    (let [the-ns #?(:clj (find-ns lib)
      ...
      (or (nil? the-ns)
          (nil? (.findInternedVar ^clojure.lang.Namespace the-ns sym))))))

(defn missing-rename-macro?
  [sym]
  (let [lib (symbol (namespace sym))
        sym (symbol (name sym))
        the-ns #?(:clj (find-ns lib)
                  :cljs (find-macros-ns lib))]
    (or (nil? the-ns)
        (nil? (.findInternedVar ^clojure.lang.Namespace the-ns sym)))))
...
(defn missing-renames
  [renames env]
  (let [cenv @env/*compiler*]
    (into {}
          (filter (fn [[_ qualified-sym]] (missing-rename? qualified-sym cenv))
            renames))))

;; returns (s/map-of symbol? symbol?)
(defn missing-use-macros
  [use-macros env]
  (let [cenv @env/*compiler*]
    (into {}
...
;; returns (s/map-of symbol? symbol?)
(defn inferred-use-macros
  [use-macros env]
  (let [cenv @env/*compiler*]
    (into {}
          (filter (fn [[sym lib]] (not (missing-use-macro? lib sym)))
            use-macros))))

;; returns (s/map-of symbol? symbol?)
(defn inferred-rename-macros
  [rename-macros env]
  (into {}
        (filter (fn [[_ qualified-sym]]
  ...
  (let [cenv @env/*compiler*]
    (doseq [[sym lib] uses]
      (when (missing-use? lib sym cenv)
        (throw (error env
                      (error-message :undeclared-ns-form
        ...
        ast' (-> ast
                 (update-in [:rename-macros]
                            #(-> %
                                 ...
                                 (merge (inferred-rename-macros maybe-macros
                                                                env))))
                 (update-in [:renames] remove-missing-renames))]
    (swap! env/*compiler* #(-> %
                               (update-in [::namespaces name :rename-macros]
                                          merge
                                          (:rename-macros ast'))
                               (update-in [::namespaces name :renames]
                                          remove-missing-renames)))
    ast'))

(defn parse-ns-error-msg
  [spec msg]
  (str msg "; offending spec: " (pr-str spec)))

(defn basic-validate-ns-spec
  [env macros? spec]
  ...
  (when (sequential? spec)
    ...
    (when-not (every? #{:as :refer :rename}
...
(defn parse-require-spec
  ...
  (if (or (symbol? spec) (string? spec))
    ...
    (do
      ...
      (let [[lib & opts] spec
            ;; We need to load JS modules by the name that has been created by
            ;; the
            ...
            [lib js-module-provides] (if-some [js-module-name (gets
                                                                @env/*compiler*
                                                                :js-module-index
                                                                (str lib)
                                                                :name)]
            ...
            {alias :as,
             ...
             :or {alias (if (string? lib) (symbol (munge lib)) lib)}}
              (apply hash-map opts)
            referred-without-renamed (seq (remove (set (keys renamed))
                                            referred))
...
(defn parse-import-spec
  [env deps spec]
  (when-not (or (and (sequential? spec) (every? symbol? spec))
    ...
    (let [ns (if (sequential? form) (first form) form)
          {:keys [use-macros require-macros]}
            (or (get-in @env/*compiler* [::namespaces ns])
                #?(:clj (when-let [res (util/ns->source ns)]
                          (:ast (parse-ns res)))))]
      (or (some #{ns} (vals use-macros)) (some #{ns} (vals require-macros))))))
...
#?(:clj
     (defn process-rewrite-form
       [[k & specs :as form]]
       (letfn [(process-spec
                 [maybe-spec]
                 (let [[lib & xs]
                         (if (sequential? maybe-spec) maybe-spec [maybe-spec])]
                   (if (and (symbol? lib) (aliasable-clj-ns? lib))
                     (let [lib' (clj-ns->cljs-ns lib)
                           spec (cons lib' xs)]
                       (into (if xs [spec] []) [(list lib' :as lib)]))
                     [maybe-spec])))]
         (if (#{:use :require} k) (cons k (mapcat process-spec specs)) form))))
...
(defn canonicalize-import-specs
  [specs]
  (letfn [(canonicalize [quoted-spec-or-kw]
                        (if (keyword? quoted-spec-or-kw)
                          quoted-spec-or-kw
...
(defn desugar-ns-specs
  ...
  [args]
  (let [{:keys [require], :as indexed} (->> args
                                            (map (fn [[k & specs]]
                                                   [k (into [] specs)]))
                                            (into {}))
        ...
        to-macro-specs
          (fn [specs]
            (->> specs
                 (filter (fn [x]
                           (or (and (sequential? x) (some sugar-keys x))
                               (reload-spec? x)
                               (macro-autoload-ns? x))))
                 (map (fn [x]
...
(defn find-def-clash
  [env ns segments]
  (let [to-check (map (fn [xs] [(symbol (string/join "." (butlast xs)))
                                (symbol (last xs))])
  ...
  (let [name (cond-> name (:macros-ns opts) macro-ns-name)]
    (let [segments (string/split (clojure.core/name name) #"\.")]
      ...
      (let [segment (some js-reserved segments)]
        (when (some? segment) (warning :munged-namespace env {:name name})))
      (find-def-clash env name segments)
      #?(:clj (when (some (complement util/valid-js-id-start?) segments)
    ...
    (let
      [docstring (when (string? (first args)) (first args))
       ...
       args (desugar-ns-specs #?(:clj (rewrite-cljs-aliases
       ...
       core-renames (reduce (fn [m [original renamed]]
                              (assoc m
                                renamed (symbol "cljs.core" (str original))))
                      {}
                      core-renames)
       deps (atom [])
       aliases (atom {:fns {}, :macros {}})
       spec-parsers
         {:require (partial parse-require-spec env false deps aliases),
         ...
         (reduce
           (fn [m [k & libs :as libspec]]
             ...
             (apply merge-with
               merge
               m
               (map (spec-parsers k) (remove #{:reload :reload-all} libs))))
           {}
       ...
       #?@(:cljs
             [[require-macros use-macros]
              (map (fn [spec-map]
                     (if (:macros-ns opts)
                       (let [ns (symbol
                                  (subs (str name) 0 (- (count (str name)) 7)))]
                         (reduce (fn [m [k v]]
                                   (cond-> m
                                     (not (symbol-identical? v ns)) (assoc k
                                                                      v)))
                           {}
                           spec-map))
                       spec-map))
      ...
      (let [ns-info {:name name,
        ...
        (merge
          ...
          (cond-> ns-info
            (@reload :use)
              (update-in [:uses] (fn [m] (with-meta m {(@reload :use) true})))
            (@reload :require)
              (update-in [:requires]
    ...
    (doseq [k [:requires :require-macros]]
      (let [old-aliases (get old k)
        ...
        (when-some [alias (some
          ...
          (throw (error env
                        (str "Alias " alias
                             " already exists in namespace " ns-name
...
(defmethod parse 'ns*
  ...
  (when-let [not-quoted (->> (remove keyword? quoted-specs)
                             ...
                             first)]
    (throw (error env
                  (str "Arguments to " (name (first quoted-specs))
  ...
  (let [specs (if (= :import (first quoted-specs))
        ...
        name (-> env
                 :ns
                 :name)
        args (desugar-ns-specs #?(:clj (list (process-rewrite-form specs))
                                  :cljs (list specs)))
        {excludes :excludes, core-renames :renames} (parse-ns-excludes env args)
        core-renames (reduce (fn [m [original renamed]]
                               (assoc m
          ...
          ns-info
            (let [ns-info' (get-in @env/*compiler* [::namespaces name])]
              (if (pos? (count ns-info'))
                (let [merge-keys [:use-macros :require-macros :rename-macros
...
(defmethod parse 'deftype* [_ env form _ _] (parse-type :deftype env form))

(defmethod parse 'defrecord* [_ env form _ _] (parse-type :defrecord env form))

;; dot accessor code

(def ^:private property-symbol?
  #(boolean (and (symbol? %) (re-matches #"^-.*" (name %)))))
...
(defmethod build-dot-form [::expr ::property ::list]
  [[target prop args]]
  #?(:clj (throw (Error. (str "Cannot provide arguments " args
                              " on property access " prop)))
     :cljs (throw (js/Error. (str "Cannot provide arguments " args
                                  " on property access " prop)))))
...
(defmethod build-dot-form :default
  [dot-form]
  #?(:clj (throw (Error. (str "Unknown dot form of " (list* '. dot-form)
                              " with classification " (classify-dot-form
                                                        dot-form))))
     :cljs (throw (js/Error. (str "Unknown dot form of " (list* '. dot-form)
                                  " with classification " (classify-dot-form
                                                            dot-form))))))
...
(defn js-star-seg
  ...
  (let [idx (.indexOf s "~{")]
    (if (== -1 idx)
      (list s)
      (let [end (.indexOf s "}" idx)]
        (lazy-seq (cons (subs s 0 idx) (js-star-seg (subs s (inc end)))))))))
...
(defn array-type?
  ...
  (cond (nil? t) true
        ...
        :else (boolean (when #?(:clj (set? t)
                                :cljs (cljs-set? t))
                         (or (contains? t 'any)
                             (contains? t 'js)
                             (some array-types t))))))

(defn analyze-js-star*
...
(defn analyzed
  ...
  [x]
  (cond (map? x) (assoc x ::analyzed true)
...
(defn- all-values?
  ...
  [exprs]
  (every?
    #(or (nil? %) (symbol? %) (string? %) (number? %) (true? %) (false? %))
    exprs))
...
(defn parse-invoke*
  ...
  (let [enve (assoc env :context :expr)
        ...
        ;; Higher order invokes with (some) argument expressions. Bind the
        ;; arguments
        ;; to avoid exponential complexity that is created by the IFn arity
        ;; check branch.
        bind-args? (and HO-invoke? (not (all-values? args)))]
    (when ^boolean fn-var?
      (let [{^boolean variadic :variadic?,
...
(defn parse-invoke [env form] (disallowing-recur (parse-invoke* env form)))

(defn desugar-dotted-expr
  [{:keys [op], :as expr}]
  (case op
    (:var :local)
...
(defn excluded?
  ...
  (or (some? (gets env :ns :excludes sym))
      (some? (gets @env/*compiler*
                   ::namespaces (gets env :ns :name)
                   :excludes sym))))
  ...
  (let [res (or (resolve-macro-ns-alias env nstr nil)
    ...
    (cond
      #?@(:clj [(= "clojure.core" nstr) (find-ns 'cljs.core)]
          :cljs [(identical? "clojure.core" nstr)
                 (find-macros-ns CLJS_CORE_MACROS_SYM)])
        #?@(:clj [(= "clojure.repl" nstr) (find-ns 'cljs.repl)]
            :cljs [(identical? "clojure.repl" nstr)
                   (find-macros-ns 'cljs.repl)])
      #?@(:clj [(.contains nstr ".") (find-ns (symbol nstr))]
          :cljs [(goog.string/contains nstr ".")
                 (find-macros-ns (symbol nstr))])
        :else
      (some-> env
              :ns
              :require-macros
    ...
    (let [nstr (namespace sym)]
      (cond
        (some? nstr) (let [ns (get-expander-ns env nstr)]
                       (when (some? ns)
                         (.findInternedVar ^clojure.lang.Namespace ns
                                           (symbol (name sym)))))
...
#?(:cljs (let [cached-var (delay (get (ns-interns* 'cljs.spec.alpha)
                                      'macroexpand-check))]
           (defn get-macroexpand-check-var
             []
             (when (some? (find-ns-obj 'cljs.spec.alpha)) @cached-var))))

(defn- do-macroexpand-check
  [form mac-var]
  (when (not (-> @env/*compiler*
                 :options
                 :spec-skip-macros))
    (let [mchk #?(:clj (some-> (find-ns 'clojure.spec.alpha)
                               (ns-resolve 'macroexpand-check))
                  :cljs (get-macroexpand-check-var))]
      (when (some? mchk) (mchk mac-var (next form))))))
...
(defn macroexpand-1*
  ...
  (let [op (first form)]
    (if (contains? specials op)
      (do (when (= 'ns op)
            (do-macroexpand-check form
                                  (get-expander 'cljs.core/ns-special-form
                                                env)))
          form)
      ;else
      (if-some [mac-var (when (symbol? op) (get-expander op env))]
        (#?@(:clj [binding [*ns* (create-ns *cljs-ns*)]]
         ...
         (let [form' (try (apply @mac-var form env (rest form))
                          #?(:clj (catch ArityException e
                                    (throw (ArityException. (- (.actual e) 2)
                                                            (.name e))))))]
           (if #?(:clj (seq? form')
                  :cljs (cljs-seq? form'))
             (let [sym' (first form')
                 ...
                 (let [sym (if (some? (namespace sym))
                       ...
                       numeric
                         #?(:clj (-> mac-var
                                     meta
                                     ::numeric)
                            :cljs (let [mac-var-ns (symbol (namespace
                                                             (.-sym mac-var)))
                                        mac-var-name (symbol (name (.-sym
                                                                     mac-var)))]
                                    (get-in @env/*compiler*
                                            [::namespaces mac-var-ns :defs
                                             mac-var-name :meta ::numeric])))
                       js-op
...
(defn analyze-seq*-wrap
  [op env form name opts]
  (wrapping-errors env (analyze-seq* op env form name opts)))

(defn analyze-seq
  ([env form name]
   (analyze-seq env form name (when env/*compiler* (:options @env/*compiler*))))
  ([env form name opts]
   (if ^boolean (:quoted? env)
...
;; :list is not used in the emitter any more, but analyze-list is called from
;; analyze-const
  ...
  (let [val (.-val form)
    ...
    (if (map? val)
      (let [keys (vec (keys val))
            vals (disallowing-recur (mapv #(analyze expr-env %) (vals val)))]
        {:op :js-object,
         :env env,
         :form form,
         :keys keys,
         ...
         :children [:vals],
         :tag 'object})
      (let [items (disallowing-recur (mapv #(analyze expr-env %) val))]
        {:op :js-array,
         :env env,
         :form form,
         :items items,
...
(defn record-ns+name
  [x]
  (map symbol
    #?(:clj ((juxt (comp #(string/join "." %) butlast) last)
              (string/split (.getName ^Class (type x)) #"\."))
       :cljs (string/split (pr-str (type x)) #"/"))))

(defn analyze-record
  [env x]
  (let [;; register constansts
        _items_ (disallowing-recur (analyze (assoc env :context :expr)
                                            (into {} x)))
        [ns name] (record-ns+name x)]
    {:op :const, :val x, :env env, :form x, :tag (symbol (str ns) (str name))}))

(defn elide-reader-meta
  [m]
  (dissoc m :file :line :column :end-column :end-line :source))

(defn elide-analyzer-meta [m] (dissoc m ::analyzed))

(defn elide-irrelevant-meta
  [m]
  (-> m
      elide-reader-meta
      elide-analyzer-meta))

(defn analyze-wrap-meta
  [expr]
  (let [form (:form expr)
...
(defn infer-type
  ...
  (if (or (nil? tag) (= 'function tag))
    ...
    (if (= :fn (:op ast))
      (update ast :methods (fn [ms] (into [] (map #(infer-type env % _)) ms)))
      (if-some [tag (infer-tag env ast)]
        (assoc ast :tag tag)
        ast))
    ast))

(defn- repl-self-require?
  [env deps]
          ...
          (if (#{:ns :ns*} op)
            (let [{:keys [name deps uses require-macros use-macros reload
              ...
              (when (and *analyze-deps* (seq deps))
                (analyze-deps (if (repl-self-require? env deps) 'cljs.user name)
                              deps
                              env
              ...
              (if *load-macros*
                ...
                (do (check-uses (when (and *analyze-deps* (seq uses))
                                  (missing-uses uses env))
                                env)
                    ast)))
            ast)))
...
(defn check-invoke-arg-types
  ...
  (when (and (not (analyzed? ast))
    ...
    (when-some [[name {:keys [valid? warning-type]}]
                  (find invoke-arg-type-validators
                        (-> ast
                            :fn
                            :info
                            :name))]
...
#?(:clj (defn analyze-form
          ...
          (cond
            ...
            :else (let [tag (cond (nil? form) 'clj-nil
                                  (number? form) 'number
                                  (string? form) 'string
...
#?(:cljs (defn analyze-form
           [env form name opts]
           (cond
             (symbol? form) (analyze-symbol env form)
             (and (cljs-seq? form) (some? (seq form)))
               (analyze-seq env form name opts)
             (record? form) (analyze-record env form)
             (cljs-map? form) (analyze-map env form)
             (cljs-vector? form) (analyze-vector env form)
             (cljs-set? form) (analyze-set env form)
             (keyword? form) (analyze-keyword env form)
             (instance? cljs.tagged-literals/JSValue form)
               (analyze-js-value env form)
             :else (let [tag (cond (nil? form) CLJ_NIL_SYM
...
(defn analyze
  ...
  ([env form name]
   (analyze env form name (when env/*compiler* (:options @env/*compiler*))))
  ([env form name opts]
   (wrapping-errors env
                    (if (analyzed? form)
    ...
    (fn [compiler-state sym value]
      (let [ns (symbol (namespace sym))]
        (update-in compiler-state
                   [::namespaces ns :defs (symbol (name sym))]
                   merge
                   {:const-expr (binding [*passes* (conj *passes*
                                                         (replace-env-pass
                                                           {:context :expr}))]
...
#?(:clj
     (defn- source-path
       ...
       [x]
       (cond (instance? File x) (.getAbsolutePath ^File x)
             :default (str x))))
     ...
     accepts a filename argument which will be used in any emitted errors."
       ...
       ([^Reader rdr filename]
        ...
        (let [eof-sentinel (Object.)
              opts (merge {:eof eof-sentinel}
                          (if (and filename (= (util/ext filename) "cljc"))
                            {:read-cond :allow, :features #{:cljs}}))
              pbr (readers/indexing-push-back-reader (PushbackReader. rdr)
                                                     1
                                                     filename)
...
#?(:clj (defn gen-user-ns
          [src]
          (if (sequential? src)
            (symbol (str "cljs.user.source$form$"
                         (util/content-sha (pr-str src) 7)))
...
#?(:clj
     (defn ^:dynamic parse-ns
      ...
      requested via opts where :restore is false."
       ([src]
        (parse-ns src nil (when env/*compiler* (:options @env/*compiler*))))
          ...
          (let [src (if (symbol? src) (util/ns->source src) src)
                ijs
                  (binding [env/*compiler* (if (false? (:restore opts))
                            ...
                            *cljs-ns* 'cljs.user
                            *cljs-file* src
                            *macro-infer* (or (when (contains? opts
                                                               :macro-infer)
                                                (:macro-infer opts))
                                              false)
                            *analyze-deps* (or (when (contains? opts
                                                                :analyze-deps)
                                                 (:analyze-deps opts))
                                               false)
                            *load-macros* (or (when (contains? opts
                                                               :load-macros)
                                                (:load-macros opts))
                                              false)]
                    (let [rdr (when-not (sequential? src) (io/reader src))]
                      (try
                        (loop [forms
                                 (if rdr (forms-seq* rdr (source-path src)) src)
                               ret
                                 (merge {:file dest,
                                        ...
                                        (when (and dest (.exists ^File dest))
                                          {:lines (with-open [reader (io/reader
                                                                       dest)]
                                                    (-> reader
                                                        line-seq
                                                        count))}))]
                          (if (seq forms)
                            (let [env (empty-env)
                                  ast (no-warn
                                        (analyze env (first forms) nil opts))]
                              (cond
                                (= :ns (:op ast))
                                  ...
                                  (let [deps (merge (:uses ast)
                                    ...
                                    (recur (rest forms)
                                           (cond-> (update-in ret
                                                              ...
                                                              into
                                                              (set (vals deps)))
                                             ;; we need to defer generating the
                                             ;; user namespace
                                             ;; until we actually need or it
                                             ;; will break when
                                             ;; `src` is a sequence of forms -
                                             ;; António Monteiro
                                             (not (:ns ret))
                                               (assoc :ns
                                                 (gen-user-ns src) :provides
                                                 [(gen-user-ns src)]))))
                                :else ret))
                            ret))
                        (finally (when rdr (.close ^Reader rdr))))))]
            (cond-> ijs
              (not (contains? ijs :ns)) (merge {:ns (gen-user-ns src),
                                                :provides [(gen-user-ns
                                                             src)]})))))))

#?(:clj (defn- cache-analysis-ext
          ([]
           (cache-analysis-ext (get-in @env/*compiler*
                                       [:options :cache-analysis-format]
                                       :transit)))
          ([format] (if (and (= format :transit) @transit) "json" "edn"))))

#?(:clj (defn build-affecting-options
          [opts]
          (select-keys opts
                       [:static-fns :fn-invoke-direct :optimize-constants
                        :elide-asserts :target :cache-key :checked-arrays
                        :language-out])))

#?(:clj (defn build-affecting-options-sha
          [path opts]
          (let [m (assoc (build-affecting-options opts) :path path)]
            (util/content-sha (pr-str m) 7))))

#?(:clj (defn ^File cache-base-path
               ...
               (map
                 (fn [[k v]]
                   [k
                    (io/file
                      path
                      (if (and (= (str "cljs" File/separatorChar "core$macros")
                                  name)
                               (= :source k))
...
#?(:clj
     (defn cache-file
      ...
      cache file. Defaults to the read path which is usually also the write path."
       ...
       ([src ns-info output-dir] (cache-file src ns-info output-dir :read nil))
       ...
       ([src ns-info output-dir mode opts]
        ...
        (let [ext (cache-analysis-ext)]
          (if-let [core-cache (and (= mode :read)
                                   (= (:ns ns-info) 'cljs.core)
                                   (io/resource (str "cljs/core.cljs.cache.aot."
                                                     ext)))]
            core-cache
            (let [aot-cache-file
                    (when (util/url? src)
              ...
              (if (and aot-cache-file (.exists ^File aot-cache-file))
                aot-cache-file
                (let [target-file (util/to-target-file output-dir
                                                       ns-info
                                                       (util/ext (:source-file
                                                                   ns-info)))]
...
#?(:clj
     (defn requires-analysis?
      ...
      Can optionally pass cache, the analysis cache file."
       ...
       ([src cache output-dir] (requires-analysis? src cache output-dir nil))
          ...
          (util/url? cache)
            (let [path (.getPath ^URL cache)]
              (if (or (.endsWith path "cljs/core.cljs.cache.aot.edn")
                ...
                false
                (throw (Exception.
                         (str "Invalid anlaysis cache, must be file not URL "
                              cache)))))
          (and (util/file? cache) (not (.exists ^File cache))) true
          :else (let [out-src (util/to-target-file output-dir (parse-ns src))
...
#?(:clj (defn- get-spec-vars
   ...
   :cljs (let [registry-ref (delay (get (ns-interns* 'cljs.spec.alpha$macros)
                                        'registry-ref))
               ;; Here, we look up the symbol '-speced-vars because ns-interns*
               ;; is implemented by invoking demunge on the result of js-keys.
               speced-vars (delay (get (ns-interns* 'cljs.spec.alpha$macros)
                                       '-speced-vars))]
           (defn- get-spec-vars
             []
             (when (some? (find-ns-obj 'cljs.spec.alpha$macros))
               {:registry-ref @registry-ref, :speced-vars @speced-vars}))))
  ...
  (let [spec-vars (get-spec-vars)
    ...
    (swap! env/*compiler* update-in
      ...
      (when-let [registry-ref (:registry-ref spec-vars)]
        {:cljs.spec/registry-ref (into []
                                       (filter (fn [[k _]]
                                                 (= ns-str (namespace k))))
                                       @@registry-ref)})
      (when-let [speced-vars (:speced-vars spec-vars)]
        {:cljs.spec/speced-vars (into []
                                      (filter (fn [v]
...
(defn register-specs
  ...
  [cached-ns]
  #?(:clj (try (locking load-mutex (clojure.core/require 'cljs.spec.alpha))
               (catch Throwable t)))
...
#?(:clj (defn write-analysis-cache
          ...
          ([ns ^File cache-file src]
           ...
           (let [ext (util/ext cache-file)
                 analysis (dissoc (get-in @env/*compiler* [::namespaces ns])
                                  :macros)]
             (case ext
               "edn" (spit cache-file
                           (str ";; Analyzed by ClojureScript "
                                  (util/clojurescript-version)
                                "\n" (pr-str analysis)))
               "json" (when-let [{:keys [writer write]} @transit]
                        (write (writer (FileOutputStream. cache-file)
                                       :json
                                       transit-write-opts)
                               analysis))))
           (when src
...
(defn analyze-form-seq
  ([forms]
   (analyze-form-seq forms (when env/*compiler* (:options @env/*compiler*))))
...
#?(:clj
     (defn analyze-file
      ...
      meaningful value."
       ([f] (analyze-file f (when env/*compiler* (:options @env/*compiler*))))
          ...
          (let [output-dir (util/output-directory opts)
            ...
            (ensure
              (let [ns-info (parse-ns res)
                ...
                (when-not (get-in @env/*compiler*
                                  [::namespaces (:ns ns-info) :defs])
                  (if (or skip-cache
                          (not cache)
                    ...
                    (try (read-analysis-cache cache res opts)
                         (catch Throwable e
