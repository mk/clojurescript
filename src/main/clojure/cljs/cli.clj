  ...
  (letfn [(desc-string [filled]
                       (string/join "\n"
                                    (map #(apply str
                                      ...
                                      filled)))]
    (let [[f & r] cs
          fstr (cond-> (if (= 1 (count cs)) (str "   " f) (format "%1$5s" f))
...
(defn- primary-groups-str
  [options]
  (str (group->str options ::main&compile)
       "\n"
...
(defn- all-groups-str
  [{:keys [groups], :as options}]
  (let [custom-groups
          (disj (set (keys groups)) ::main&compile ::main ::compile)]
    (apply str
      (primary-groups-str options)
      (map (fn [group] (str (group->str options group) "\n")) custom-groups))))
...
(defn- options-str [options] (str (all-groups-str options) (main-str options)))

(declare merged-commands)

(defn help-str
  [repl-env]
  (format help-template (options-str (merged-commands repl-env))))
...
(defn- validate-watch-paths
  [[path :as paths]]
  (when (or (nil? path)
            (and (not (.exists (io/file path)))
                 (or (string/blank? path) (string/starts-with? path "-"))))
    (throw (ex-info (str "Missing watch path(s)")
                    {:cljs.main/error :invalid-arg})))
  (when-let [non-existent (seq (remove #(.exists (io/file %)) paths))]
    (throw (ex-info (if (== 1 (count non-existent))
                      (str "Watch path " (first non-existent) " does not exist")
                      (str "Watch paths "
...
(defn- output-to-opt [cfg path] (assoc-in cfg [:options :output-to] path))
...
(defn missing-file
  [x]
  (throw (ex-info (str "File " x " does not exist")
                  {:cljs.main/error :invalid-arg})))

(defn missing-resource
  [x]
  (throw (ex-info (str "Resource "
                       (if (string/starts-with? x "@/") (subs x 2) (subs x 1))
                       " does not exist")
                  {:cljs.main/error :invalid-arg})))
...
(defn read-edn-opts
  ...
  (letfn [(read-rsrc [rsrc-str orig-str]
    ...
    (cond (string/starts-with? str "@/") (read-rsrc (subs str 2) str)
          (string/starts-with? str "@") (read-rsrc (subs str 1) str)
          :else (let [f (io/file str)]
                  (if (.exists f)
...
(defn- init-opt
  [cfg file]
  (let [file' (cond (string/starts-with? file "@/") (io/resource (subs file 2))
                    (string/starts-with? file "@") (io/resource (subs file 1))
                    :else (let [f (io/file file)]
                            (if (.exists f) f (missing-file file))))]
    (when-not file' (missing-resource file))
    (update-in cfg
               [:inits]
               (fnil conj [])
               {:type :init-script, :script file'})))
...
(defn get-dispatch
  ([commands k opt] (get-dispatch commands k opt nil))
...
(defn initialize
  ...
  [inits commands]
  (reduce (fn [ret [opt arg]] ((get-dispatch commands :init opt) ret arg))
    {}
    inits))
...
present"
  ...
  (let [opts (cond-> options
               (not (:output-dir options)) (assoc :output-dir
                                             (temp-out-dir) :temp-output-dir?
                                             true)
               (not (contains? options :aot-cache)) (assoc :aot-cache true))
...
(defn default-main
  ...
  (let [opts (cond-> options
        ...
        _ (when (or ana/*verbose* (:verbose opts))
            (util/debug-prn "REPL env options:" (pr-str reopts)))
        renv (apply repl-env (mapcat identity reopts))
        coptsf (when-let [od (:output-dir opts)] (io/file od "cljsc_opts.edn"))
        copts (when (and coptsf (.exists coptsf))
                (-> (edn/read-string (slurp coptsf))
                    (dissoc-entry-point-opts)))
        opts (merge copts
                    (build/add-implicit-options (merge (repl/repl-options renv)
                                                       opts)))]
      ...
      (comp/with-core-cljs
        ...
        (fn []
          (try
            ...
            (when script
              (cond (= "-" script) (repl/load-stream renv "<cljs repl>" *in*)
                    ...
                    (string/starts-with? script "@")
                      (if-let [rsrc (io/resource (subs script 1))]
                        ...
                        (missing-resource script))
                    (string/starts-with? script "-")
                      (throw (ex-info (str "Expected script or -, got flag "
                                           script
                                           " instead")
                                      {:cljs.main/error :invalid-arg}))
                    :else (throw (ex-info
...
(defn- help-opt [repl-env _ _] (println (help-str repl-env)))

(defn- script-opt
  [repl-env [path & args] cfg]
  ((::main (repl/repl-options (repl-env)) default-main)
    repl-env
    (merge cfg {:script path, :args args})))
...
(defn- serve-opt
  ...
  (let [[host port]
          (if address-port (string/split address-port #":") ["localhost" 9000])]
    (require 'cljs.repl.browser)
    ((ns-resolve 'cljs.repl.browser 'serve)
      {:host host,
       :port (if port (cond-> port (string? port) Integer/parseInt) 9000),
...
(defn default-compile
  ...
  (let [rfs #{"-r" "--repl"}
        ...
        main-ns
          (if (and ns (not ((into rfs sfs) ns))) (symbol ns) (:main options))
        opts (as-> (merge (select-keys env-opts
                                       (cond-> [:target]
                                         repl? (conj :browser-repl)))
                          options
                          (when main-ns {:main main-ns})) opts
               (cond-> opts
                 (not (:output-to opts))
                   (assoc :output-to
                     (.getPath (io/file (:output-dir opts "out") "main.js")))
                 (= :advanced (:optimizations opts)) (dissoc :browser-repl)
                 (not (:output-dir opts)) (assoc :output-dir "out")
                 (not (contains? opts :aot-cache)) (assoc :aot-cache true)
        ...
        cenv (env/default-compiler-env (closure/add-externs-sources
                                         (dissoc opts :foreign-libs)))]
    (env/with-compiler-env
      cenv
      (if-let [path (:watch opts)]
        (if repl? (build/build source opts cenv) (build/watch path opts cenv))
        (build/build source opts cenv))
      (when repl? (repl-opt repl-env args cfg))
      (when serve? (serve-opt repl-env args cfg)))))

(defn- compile-opt
  [repl-env [_ ns & args] cfg]
  ((::compile (repl/-repl-options (repl-env)) default-compile)
    repl-env
    (merge cfg {:args args, :ns ns})))
...
(defn dispatch? [commands k opt] (contains? (get-options commands k) opt))

(defn add-commands
  ([commands] (add-commands {:main-dispatch nil, :init-dispatch nil} commands))
  ([commands {:keys [groups main init]}]
   (letfn [(merge-dispatch
             [st k options]
             (update-in st
                        [k]
                        (fn [m]
                          (reduce (fn [ret [cs csm]]
                                    (merge ret (zipmap cs (repeat (:fn csm)))))
                            m
                            options))))]
     (-> commands
       ...
       {["-i" "--init"] {:group ::main,
        ...
        ["-d" "--output-dir"]
          {:group ::main&compile,
           ...
           :doc (str "Set the output directory to use. If "
                     ...
                     "will be used to set ClojureScript compiler " "options")},
        ["-w" "--watch"]
          {:group ::compile,
           :fn watch-opt,
           :arg "paths",
        ...
        ["-O" "--optimizations"]
          {:group ::compile,
           :fn optimize-opt,
           :arg "level",
           :doc (str "Set optimization level, only effective with "
        ...
        ["-t" "--target"]
          {:group ::main&compile,
           ...
           :doc
             (str
               "The JavaScript target. Configures environment bootstrap and "
               "defaults to browser. Supported values: node or nodejs, nashorn, "
               "graaljs, webworker, none")},
        ["-ro" "--repl-opts"]
          {:group ::main&compile,
           :fn repl-env-opts-opt,
           :arg "edn",
           :doc
             (str
               "Options to configure the repl-env, can be an EDN string or "
        ...
        ["-co" "--compile-opts"]
          {:group ::main&compile,
           ...
           :doc
             (str
               "Options to configure the build, can be an EDN string or "
               "system-dependent path-separated list of EDN files / classpath resources. Options "
               "will be merged left to right.")}},
     :main
       {["-r" "--repl"] {:fn repl-opt, :doc "Run a repl"},
        ["-m" "--main"]
          {:fn main-opt,
           :arg "ns",
        ...
        ["-s" "--serve"]
          {:fn serve-opt,
           :arg "host:port",
           :doc (str
                  "Start a simple web server to serve the current directory")},
        [nil] {:fn null-opt},
        ["-h" "--help" "-?"] {:fn help-opt,
...
(defn normalize
  ...
  (if (not (contains? (get-options commands :main) (first args)))
    (let [pred (complement #{"-v" "--verbose"})
          [pre post] ((juxt #(take-while pred %) #(drop-while pred %)) args)]
      (cond (= pre args) pre
            ...
            :else (concat pre
                          [(first post) (fnext post)]
                          (normalize commands (nnext post)))))
    args))

(defn merged-commands
  [repl-env]
  (add-commands default-commands (::commands (repl/repl-options (repl-env)))))
...
(defn main
  ...
  [repl-env & args]
  (try (let [commands (merged-commands repl-env)]
         (if args
           (loop [[opt arg & more :as args] (normalize commands args)
                  inits []]
