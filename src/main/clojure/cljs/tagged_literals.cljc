  ...
  (when-not (vector? form)
    (throw
      #?(:clj (RuntimeException.
         ...
         :cljs (js/Error. "Queue literal expects a vector for its elements."))))
...
#?(:clj (defn read-uuid
          ...
          (try (java.util.UUID/fromString form)
               (catch Throwable e
...
#?(:cljs (defn read-uuid
           ...
           (when-not (string? form)
             (throw (js/Error.
                      "UUID literal expects a string as its representation.")))
           (try (uuid form)
                (catch :default e (throw (js/Error. (. e -message)))))))
...
#?(:clj (defn read-inst
          ...
          (try (inst/read-instant-date form)
               (catch Throwable e
                 (throw (RuntimeException. (.getMessage e)))))))
...
#?(:cljs (defn read-inst
           ...
           (when-not (string? form)
             (throw (js/Error.
                      "Instance literal expects a string for its timestamp.")))
           (try (#'reader/read-date form)
                (catch :default e (throw (js/Error. (. e -message)))))))

(defn valid-js-literal-key?
  [k]
...
(defn read-js
  ...
  (when-not (or (vector? form) (map? form))
    (throw #?(:clj (RuntimeException.
              ...
              :cljs (js/Error.
                      "JavaScript literal must use map or vector notation"))))
  (when-not (or (not (map? form)) (every? valid-js-literal-key? (keys form)))
    (throw
      #?(:clj
           (RuntimeException.
