...
(defn ^:private zero-fill-right-and-truncate
  [s width]
...
(defn ^:private divisible? [num div] (zero? (mod num div)))

(defn ^:private indivisible? [num div] (not (divisible? num div)))

(defn ^:private leap-year?
  [year]
  (and (divisible? year 4) (or (indivisible? year 100) (divisible? year 400))))

(def ^:private days-in-month
  (let [dim-norm [nil 31 28 31 30 31 30 31 31 30 31 30 31]
        dim-leap [nil 31 29 31 30 31 30 31 31 30 31 30 31]]
    (fn [month leap-year?] (get (if leap-year? dim-leap dim-norm) month))))

(def ^:private timestamp-regex
  #"(\d\d\d\d)(?:-(\d\d)(?:-(\d\d)(?:[T](\d\d)(?::(\d\d)(?::(\d\d)(?:[.](\d+))?)?)?)?)?)?(?:[Z]|([-+])(\d\d):(\d\d))?")

(defn ^:private parse-int
  [s]
  (let [n (js/parseInt s 10)] (if-not (js/isNaN n) n)))
...
(defn parse-and-validate-timestamp
  [s]
  (let [[_ years months days hours minutes seconds fraction offset-sign
         offset-hours offset-minutes :as v]
    ...
    (if-not v
      ...
      (let [years (parse-int years)
            ...
            fraction (or (parse-int (zero-fill-right-and-truncate fraction 3))
                         0)
...
(defn parse-timestamp
  ...
  (if-let [[years months days hours minutes seconds ms offset]
    ...
    (js/Date. (- (.UTC js/Date years (dec months) days hours minutes seconds ms)
                 (* offset 60 1000)))
...
(defn ^:private read-js
  [form]
  (cond (vector? form) (let [arr (array)]
                         (doseq [x form] (.push arr x))
                         arr)
        (map? form) (let [obj (js-obj)]
                      (doseq [[k v] form] (gobject/set obj (name k) v))
                      obj)
        :else (throw (js/Error.
...
(def ^:dynamic *default-data-reader-fn* (atom nil))
...
(defn read
  ...
  ([reader]
   (edn/read
     {:readers @*tag-table*, :default @*default-data-reader-fn*, :eof nil}
     reader))
  ([{:keys [eof], :as opts} reader]
   (edn/read (update (merge opts {:default @*default-data-reader-fn*})
                     :readers
...
(defn read-string
  ...
  ([s]
   (edn/read-string
     ...
     s))
  ([opts s]
   (edn/read-string (update (merge {:default @*default-data-reader-fn*} opts)
                            :readers
                            (fn [m] (merge @*tag-table* m)))
                    s)))
