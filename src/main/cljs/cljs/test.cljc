...
(defn assert-any
  ...
  `(let [value# ~form]
     (if value#
       (do-report
         {:type :pass, :message ~msg, :expected '~form, :actual value#})
       (do-report
         {:type :fail, :message ~msg, :expected '~form, :actual value#}))
     value#))

(defmacro ^:private cljs-output-dir
  []
  (let [{:keys [output-dir]} (ana-api/get-options)] (or output-dir "out")))
...
(defmethod assert-expr :default
  [menv msg form]
  (if (and (sequential? form) (function? menv (first form)))
    (assert-predicate msg form)
    (assert-any msg form)))

(defmethod assert-expr 'instance?
  [menv msg form]
...
(defmacro are
  ...
  [argv expr & args]
  (if (or ;; (are [] true) is meaningless but ok
...
(defmacro testing
  ...
  ([string & body]
   `(do (update-current-env! [:testing-contexts] conj ~string)
        (try ~@body (finally (update-current-env! [:testing-contexts] rest))))))
...
(defmacro deftest
  ...
  [name & body]
  (when ana/*load-tests*
    `(do (def ~(vary-meta name assoc :test `(fn [] ~@body))
       ...
       (concat
         ~@(map (fn [ns]
                  `(concat (test-ns-block ~env ~ns)
                           [(fn []
                              (cljs.core/vswap! ~summary
                                                (partial merge-with +)
                                                (:report-counters
                                                  (get-and-clear-env!))))]))
             (if is-ns (concat [env-or-ns] namespaces) namespaces))
         [(fn []
            (set-env! ~env)
...
(defmacro run-all-tests
  ...
  ([re env]
   `(run-tests
      ~env
      ~@(map (fn [ns] `(quote ~ns))
          (cond->> (ana-api/all-ns) re (filter #(re-matches re (name %))))))))
      ...
      (concat
        ...
        (test-vars-block
          [~@(->> (ana-api/ns-interns ns)
                  ...
                  (map (fn [[k _]] `(var ~(symbol (name ns) (name k))))))])
        [(fn [] (when (nil? env#) (clear-env!)))]))))

(defmacro test-all-vars
  "Calls test-vars on every var with :test metadata interned in the
  namespace, with fixtures."
  [[quote ns :as form]]
  `(run-block (concat (test-all-vars-block ~form)
                      [(fn []
...
(defmacro test-ns-block
  ...
  ([env [quote ns :as form]]
   (assert (and (= quote 'quote) (symbol? ns))
           "Argument to test-ns must be a quoted symbol")
   ...
   `[(fn []
     ...
     (fn [] (do-report {:type :end-test-ns, :ns ~form}))]))
...
(defmacro test-ns
  ...
  *initial-report-counters*.  "
  ...
  ([env [quote ns :as form]]
   `(run-block (concat (test-ns-block ~env ~form) [(fn [] (clear-env!))]))))
...
(defmacro use-fixtures
  [type & fns]
  (condp = type
    :once `(def ~'cljs-test-once-fixtures [~@fns])
    :each `(def ~'cljs-test-each-fixtures [~@fns])
    :else
      (throw
        (#?(:clj Exception.
            :cljs js/Error.)
         "First argument to cljs.test/use-fixtures must be :once or :each"))))
