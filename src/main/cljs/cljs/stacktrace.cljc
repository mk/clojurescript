...
(ns cljs.stacktrace
  (:require #?@(:clj [[cljs.util :as util] [clojure.java.io :as io]]
...
(defmethod parse-stacktrace :default [repl-env st err opts] st)

(defn parse-file-line-column
  [flc]
  (if-not (re-find #":" flc)
    [flc nil nil]
    (let [xs (string/split flc #":")
          [pre [line column]]
            (reduce (fn [[pre post] [x i]]
      ...
      [(cond-> file (starts-with? file "(") (string/replace "(" ""))
       (parse-int (cond-> line (ends-with? line ")") (string/replace ")" "")))
       (parse-int (cond-> column
                    (ends-with? column ")") (string/replace ")" "")))])))

(defn parse-file
  "Given a browser file url convert it into a relative path that can be used
   to locate the original source."
  [{:keys [host host-port port], :as repl-env} file
   {:keys [asset-path], :as opts}]
  (let [urlpat (if host
                 (string->regex
                   (str "http://" host ":" (or host-port port) "/"))
                 "")
        match (if host (re-find urlpat file) (contains? opts :output-dir))]
    (if match
      (-> file
          ...
          (string/replace
            (string->regex
              ...
              (str "^"
                   (or (and asset-path (string/replace asset-path #"^/" ""))
                       (output-directory opts))
                   "/"))
            ""))
      (if-let [asset-root (:asset-root opts)]
        (string/replace file asset-root "")
        (throw (ex-info (str "Could not relativize URL " file)
...
(comment (safari-st-el->frame
           {:host "localhost", :port 9000}
           "cljs$core$seq@http://localhost:9000/out/cljs/core.js:4259:17"
           {})
         (safari-st-el->frame
           {:host "localhost", :port 9000}
           "cljs$core$seq@http://localhost:9000/js/cljs/core.js:4259:17"
           {:asset-path "js"}))
...
(defn firefox-clean-function
  [f]
  (as-> f f
    (cond (string/blank? f) nil
          (not= (.indexOf f "</") -1) (let [idx (.indexOf f "</")]
                                        (.substring f (+ idx 2)))
          :else f)
    (-> f
        (string/replace #"<" "")
              ...
              (let [frame-str (string/replace frame-str #"\s+at\s+" "")
                ...
                {:file (string/replace (.substring file-part 1)
                 ...
                 :line (when (and line-part (not (string/blank? line-part)))
                         (parse-int
                           (.substring line-part 0 (dec (count line-part))))),
                 :column 0})))]
    (->> (string/split st #"\n")
         (map process-frame)
         (remove nil?)
...
(defn mapped-line-column-call
  ...
  (let [source-map (get sms
                        (symbol (string/replace (remove-ext file) "/" ".")))]
    ...
    (letfn [(get-best-column [columns column]
              ...
              (adjust [mapped] (vec (map #(%1 %2) [inc inc identity] mapped)))]
...
(defn mapped-stacktrace
  ...
  ([stacktrace sms opts]
   (letfn [(call->function [x] (if (:call x) (hash-map :function (:call x)) {}))
             (call-merge
               [function call]
               (merge-with
     ...
     (let [mapped-frames (map (memoize #(mapped-frame % sms opts)) stacktrace)]
       ...
       ;; one-level
       ;; up to avoid replacing with local symbols, we only replace munged name
       ;; if
...
(defn mapped-stacktrace-str
  ...
  ([stacktrace sms opts]
   (with-out-str (doseq [{:keys [function file line column]}
...
(comment
  ...
  (cljsc/build "samples/hello/src"
               {:optimizations :none,
                ...
                :source-map true})
  (def sms
    {'hello.core (sm/decode
                   (json/read-str (slurp "samples/hello/out/hello/core.js.map")
                                  :key-fn
                                  keyword))})
  (pp/pprint sms)
