...
(defn- get-js-opt
  [args]
  (if (= 2 (count args))
    (let [repl-ns (symbol (str "cljs.repl."
                               (if (= 1 (count args)) "nashorn" (nth args 1))))]
      (try (require repl-ns)
           (if-let [repl-env (ns-resolve repl-ns 'repl-env)]
             repl-env
             (throw
               (ex-info
                 (str "REPL namespace " repl-ns " does not define repl-env var")
                 {:repl-ns repl-ns})))
           (catch Throwable _
             (throw (ex-info (str "REPL namespace " repl-ns " does not exist")
                             {:repl-ns repl-ns})))))
    browser/repl-env))
...
(defn normalize
  [args]
  (let [[js-opt args] (normalize* args)] (concat js-opt args)))
...
(defn -main
  ...
  (let [args (normalize (cli/normalize cli/default-commands args))
    ...
    (try (apply cli/main repl-opt (concat pre args))
