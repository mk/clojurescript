...
(ns cljs.js
  ...
  (:require-macros [cljs.js :refer [dump-core]] [cljs.env.macros :as env])
...
(defn- debug-prn
  [& args]
  (binding [*print-fn* *print-err-fn*] (apply println args)))
...
(defn file->ns
  [file]
  (let [lib-name (subs (string/replace file "/" ".") 0 (- (count file) 5))]
...
(defn- valid-name? [x] (or (nil? x) (symbol? x) (string? x)))

(defn- valid-opts? [x] (or (nil? x) (map? x)))
...
(defonce
  ...
  nil.",
    :dynamic true}
  *load-fn*
  (fn [m cb] (throw (js/Error. "No *load-fn* set"))))
...
(defonce
  ...
  The result of evaluation should be the return value.",
    :dynamic true}
  *eval-fn*
  (fn [m] (throw (js/Error. "No *eval-fn* set"))))
...
(defn load-source-map!
  [state ns sm-json]
  (let [sm (sm/decode (.parse js/JSON sm-json))]
    (swap! state assoc-in [:source-maps ns] sm)))

(defn- sm-data [] (atom {:source-map (sorted-map), :gen-col 0, :gen-line 0}))

(defn- prefix [s pre] (str pre s))

(defn- append-source-map
  [state name source sb sm-data
   {:keys [output-dir asset-path source-map-timestamp], :as opts}]
  (let [t (.valueOf (js/Date.))
        mn (if name (munge (str name)) (str "cljs-" t))
        smn (cond-> mn name (string/replace "." "/"))
        ts (.valueOf (js/Date.))
    ...
    (swap! state assoc-in
      [:source-maps (symbol mn)]
      (sm/invert-reverse-map (:source-map sm-data)))
    (.append
      sb
      (str "\n//# sourceURL=" file
           "\n//# sourceMappingURL=data:application/json;base64,"
             (-> (js/encodeURIComponent json)
                 (string/replace #"%([0-9A-F]{2})"
                                 (fn [[_ match]]
...
(defn- run-async!
  ...
  (if (seq coll)
    (proc (first coll)
          (fn [res]
            (if (break? res) (cb res) (run-async! proc (rest coll) break? cb))))
    (cb nil)))
...
(defn- process-deps
  ...
  (run-async! (fn [name cb] (require bound-vars name nil opts cb))
              names
              :error
              cb))
...
(defn- process-libs-deps
  [bound-vars cache opts cb]
  (process-deps bound-vars
                (distinct (concat (vals (:requires cache))
                                  (vals (:imports cache))))
...
(defn- pre-file-side-effects
  [st name file opts]
  (when (:verbose opts) (debug-prn "Pre-file side-effects" file))
  ;; In case any constants are defined in the namespace, flush any analysis
  ;; metadata
...
(defn- post-file-side-effects
  [file opts]
  (when (:verbose opts) (debug-prn "Post-file side-effects" file))
  ;; Note, we don't (set! *unchecked-arrays* false) here, as that would
  ;; interpreted
  ;; an intrinsic affecting the compilation of this file, emitting a no-op. We
  ;; bypass this
...
(defn require
  ...
  ([bound-vars name reload opts cb]
   (let [bound-vars (merge {:*compiler* (env/default-compiler-env),
     ...
     (when (:verbose opts)
       (debug-prn
         (str "Loading " name (when (:macros-ns opts) " macros") " namespace")))
     (if-not (contains? @*loaded* aname)
       (let [env (:*env* bound-vars)]
         (try
           ((:*load-fn* bound-vars)
             {:name name, :macros (:macros-ns opts), :path (ns->relpath name)}
             (fn [resource]
               ...
               (if resource
                 (let [{:keys [lang source cache source-map file]} resource]
                   (condp keyword-identical? lang
                     :clj (do (pre-file-side-effects (:*compiler* bound-vars)
                                                     aname
                                                     file
                                                     opts)
                              (eval-str* bound-vars
                                         ...
                                         (fn [res]
                                           ...
                                           (if (:error res)
                                             (cb res)
                                             (do (swap! *loaded* conj aname)
                                                 (cb {:value true}))))))
                     :js (process-macros-deps
                           bound-vars
                           cache
                           opts
                           (fn [res]
                       ...
                       (ana/error env
                                  (ana/error-message
                                    (if (:macros-ns opts)
                                      ...
                                      :undeclared-ns)
                                    {:ns-sym name,
                                     :js-provide (cljs.core/name name)})))))))
           (catch :default cause
             (cb (wrap-error
                ...
                (swap! compiler update-in
                  ...
                  (fn [m]
                    (let [replaced (walk/postwalk-replace {from to} m)]
                      (if (and add-if-present? (some #{to} (vals replaced)))
                ...
                (when m
                  (reduce (fn [acc [renamed qualified-sym :as entry]]
                            (if (= (str from) (namespace qualified-sym))
                              (assoc acc
                                renamed (symbol (str to) (name qualified-sym)))
                              (merge acc entry)))
                    {}
                    m)))))]
    (patch :requires true)
...
(defn- load-deps
  ...
  ([bound-vars ana-env lib deps reload opts cb]
   (when (:verbose opts) (debug-prn "Loading dependencies for" lib))
   (binding [ana/*cljs-dep-set*
               (let [lib (if (self-require? deps opts) 'cljs.user lib)]
         ...
         (if (seq deps)
           (let [dep (first deps)
             ...
             (require
               ...
               opts'
               (fn [res]
                 (when (:verbose opts) (debug-prn "Loading result:" res))
                 (if-not (:error res)
                   ...
                   (if-let [cljs-dep (let [cljs-ns (ana/clj-ns->cljs-ns dep)]
                     ...
                     (require
                       ...
                       (fn [res]
                         (if (:error res)
                           (cb res)
                           (do (patch-alias-map (:*compiler* bound-vars)
                                                lib
                                                dep
                                                cljs-dep)
                               (load-deps bound-vars
                                          ana-env
                                          lib
                                          (next deps)
                                          nil
...
(defn- analyze-deps
  ...
  ([bound-vars ana-env lib deps opts cb]
   (binding [ana/*cljs-dep-set* (vary-meta (conj (:*cljs-dep-set* bound-vars)
                                                 lib)
                                           update-in
                                           [:dep-path]
                                           conj
                   ...
                   (if-not resource
                     (if-let [cljs-dep (let [cljs-ns (ana/clj-ns->cljs-ns dep)]
                       ...
                       (do (patch-alias-map (:*compiler* bound-vars)
                                            ...
                                            dep
                                            cljs-dep)
                           (analyze-deps bound-vars
                                         ana-env
                                         lib
                                         (cons cljs-dep (next deps))
                                         opts
                                         (fn [res]
                                           (if (:error res)
                                             (cb res)
                     ...
                     (let [{:keys [name lang source file]} resource]
                       (condp keyword-identical? lang
                         :clj
                           (do
                             (pre-file-side-effects (:*compiler* bound-vars)
                                                    name
                                                    file
                                                    opts)
                             (analyze-str* bound-vars
                                           source
                                           name
                                           (assoc opts :cljs-file file)
                                           (fn [res]
                         ...
                         (wrap-error
                           (ana/error ana-env
                                      (str "Invalid :lang specified "
                                           ...
                                           ", only :clj or :js allowed"))))))))
               (catch :default cause
                 (cb (wrap-error (ana/error ana-env
                                            (str "Could not analyze dep " dep)
                                            cause))))))
           (cb {:value nil})))))))

(defn- load-macros
  [bound-vars k macros lib reload reloads opts cb]
  (if (seq macros)
...
(defn- rewrite-ns-ast
  ([ast smap] (rewrite-ns-ast ast smap false))
  ([ast smap macros?]
...
(defn- check-macro-autoload-inferring-missing
  ...
  (let [namespaces (-> @cenv
                       ::ana/namespaces)
        missing-require-macros
          (into {}
                (filter (fn [[_ full-ns]]
                          (let [{:keys [use-macros require-macros]}
                                  (get namespaces full-ns)]
    ...
    (swap! cenv update-in
      [::ana/namespaces name :require-macros]
      merge
      missing-require-macros)
    ast'))
...
(defn- ns-side-effects
  ...
  ([load bound-vars ana-env {:keys [op], :as ast} opts cb]
   (when (:verbose opts) (debug-prn "Namespace side effects for" (:name ast)))
   (if (#{:ns :ns*} op)
     (letfn
          ...
          (let [env (:*compiler* bound-vars)
            ...
            (if (:error res)
              ...
              (if (:*load-macros* bound-vars)
                (do
                  (when (:verbose opts)
                    (debug-prn "Processing :use-macros for" name))
                  (load-macros
                    bound-vars
                    :use-macros
                    use-macros
                    ...
                    (fn [res]
                      (if (:error res)
                        (cb res)
                        (let [{:keys [require-macros], :as rewritten-ast}
                                (rewrite-ns-ast rewritten-ast
                                                (:aliased-loads res)
                                                true)]
                          (when (:verbose opts)
                            (debug-prn "Processing :require-macros for"
                                       (:name ast)))
                          (load-macros
                            bound-vars
                            :require-macros
                            ...
                            (fn [res']
                              (if (:error res')
                                (cb res')
                                (let [{:keys [use-macros], :as rewritten-ast}
                                        (rewrite-ns-ast rewritten-ast
                                                        (:aliased-loads res)
                                                        true)
                                      res'
                                        (try
                                          (when (seq use-macros)
                                            (when (:verbose opts)
                                              (debug-prn
                                                "Checking :use-macros for"
                                                (:name ast)))
                                            (binding [ana/*analyze-deps*
                                                        (:*analyze-deps*
                                                          bound-vars)
                                                      env/*compiler*
                                                        (:*compiler*
                                                          bound-vars)]
                                              (ana/check-use-macros use-macros
                                                                    env)))
                                          {:value nil}
                                          (catch :default cause
                                            (wrap-error
                                      ...
                                      (binding [ana/*analyze-deps*
                                        ...
                                        (let
                                          [ast'
                                             (->
                                               rewritten-ast
                                               (ana/check-use-macros-inferring-missing
                                                 env)
                                               (ana/check-rename-macros-inferring-missing
                                                 env)
                                               (check-macro-autoload-inferring-missing
                                                 env))]
                                          (cb {:value ast'})))
                                      (catch :default cause
                                        (cb (wrap-error
                ...
                (try
                  ...
                  (ana/check-uses (when (and (:*analyze-deps* bound-vars)
                                             (seq uses))
         ...
         (and load (seq (:deps ast)))
           (let [{:keys [reload name deps]} ast]
             (load-deps bound-vars
                        ...
                        #(check-uses-and-load-macros
                           %
                           (rewrite-ns-ast ast (:aliased-loads %)))))
         (and (not load) (:*analyze-deps* bound-vars) (seq (:deps ast)))
           (analyze-deps bound-vars
                         ...
                         #(check-uses-and-load-macros
                            %
                            (rewrite-ns-ast ast (:aliased-loads %))))
         :else (check-uses-and-load-macros {:value nil} ast)))
     (cb {:value ast}))))
...
(defn- node-side-effects
  ...
  (doseq [dep deps]
    (.append sb
             (with-out-str (comp/emitln (munge ns-name)
                                        "."
                                        (ana/munge-node-lib dep)
                                        " = require('"
                                        dep
                                        "');"))))
  (when (and (seq deps) emit-nil-result?) (.append sb "null;")))
    ...
    (trampoline
      (fn analyze-loop [last-ast ns]
        (binding [env/*compiler* (:*compiler* bound-vars)
                  ...
                  ana/*fn-invoke-direct* (and (:static-fns opts)
          ...
          (let [res (try
                      {:value (read eof rdr)}
                      (catch :default cause
                        (wrap-error (ana/error aenv
                                               (str "Could not analyze " name)
                                               cause))))]
            (if (:error res)
              (cb res)
              (let [form (:value res)]
                (if-not (identical? eof form)
                  (let [aenv (cond-> (assoc aenv
                                       :ns (ana/get-namespace ana/*cljs-ns*))
                               (:context opts) (assoc :context (:context opts))
                               (:def-emits-var opts) (assoc :def-emits-var
                                                       true))
                        res (try {:value (ana/analyze aenv form nil opts)}
                                 (catch :default cause
                                   (wrap-error
                                     (ana/error aenv
                                                (str "Could not analyze " name)
                    ...
                    (if (:error res)
                      ...
                      (let [ast (:value res)]
                        (if (#{:ns :ns*} (:op ast))
                          ((trampoline-safe ns-side-effects)
                            bound-vars
                            aenv
                            ast
                            opts
...
(defn analyze-str
  ...
  ([state source name opts cb]
   {:pre [(atom? state) (string? source) (valid-name? name) (valid-opts? opts)
          (fn? cb)]}
   (analyze-str* {:*compiler* state,
                  :*data-readers* tags/*cljs-data-readers*,
  ...
  (let [the-ns (or (:ns opts) 'cljs.user)
    ...
    (binding [env/*compiler* (:*compiler* bound-vars)
              ...
              *ns* (create-ns (:*cljs-ns* bound-vars))
              r/*alias-map* (alias-map @(:*compiler* bound-vars)
                                       (:*cljs-ns* bound-vars))
              r/*data-readers* (:*data-readers* bound-vars)
              r/resolve-symbol resolve-symbol
              comp/*source-map-data* (:*sm-data* bound-vars)]
                ...
                (fn [res]
                  (if (:error res)
                    ...
                    (let [ns-name (:name ast)
                      ...
                      (.append sb
                               (with-out-str
                                 (comp/emitln (str "goog.provide(\""
                                                   (comp/munge ns-name)
                                                   "\");"))))
                      (when-not (nil? node-deps)
                        (node-side-effects bound-vars
                                           sb
                                           node-deps
                                           ns-name
                                           (:def-emits-var opts)))
                      (global-exports-side-effects
                        bound-vars
                        sb
                ...
                (if-not (identical? eof form)
                  (let [aenv (cond-> (assoc aenv
                    ...
                    (if (:error res)
                      ...
                      (let [ast (:value res)
                            [node-deps ast]
                              (if (keyword-identical? (:target opts) :nodejs)
                                (let [{node-libs true, libs-to-load false}
                                        (group-by ana/node-module-dep?
                                                  (:deps ast))]
                                  [node-libs (assoc ast :deps libs-to-load)])
                        ...
                        (if (#{:ns :ns*} (:op ast))
                          ...
                          (do (.append sb (with-out-str (comp/emit ast)))
                              #(compile-loop ns))))))
                  (do (when (:source-map opts)
      ...
      (fn compile-loop [ns]
        (binding [env/*compiler* (:*compiler* bound-vars)
                  ...
                  *ns* (create-ns ns)
                  ...
                  comp/*source-map-data* (:*sm-data* bound-vars)
                  ana/*cljs-file* (:cljs-file opts)]
          (let [res (try {:value (read eof rdr)}
                         (catch :default cause
                           (wrap-error (ana/error aenv
                                                  (str "Could not eval " name)
                                                  cause))))]
            (if (:error res)
              (cb res)
              (let [form (:value res)]
                                ...
                                (if (:error res)
                                  ...
                                  (let [ns-name (:name ast)]
                                    ...
                                    (global-exports-side-effects
                                      bound-vars
                                      sb
                                      (filter ana/dep-has-global-exports?
                                        (:deps ast))
                                      ns-name
                                      (:def-emits-var opts))
                                    (trampoline compile-loop ns'))))))
                          (do (env/with-compiler-env
                                (assoc @(:*compiler* bound-vars) :options opts)
                  ...
                  (do
                    (when (:source-map opts)
                      (append-source-map env/*compiler*
                                         ...
                                         @comp/*source-map-data*
                                         opts))
                    (when (symbol? aname) (ana/dump-specs aname))
                    (let [js-source (.toString sb)
                          evalm {:lang :clj,
                                 ...
                                 :cache (get-in @env/*compiler*
                                                [::ana/namespaces aname])}
                          complete
                            (fn [res]
                              (if (:error res)
                                (cb res)
                                (do (when (:verbose opts) (debug-prn js-source))
                                    (let [res (try {:ns ns,
                                                    :value (*eval-fn* evalm)}
                                                   (catch :default cause
                                                     (wrap-error
...
(defn- clear-fns! "Clears saved functions." [] (vreset! fn-refs {}))
...
(defn- emit-fn [f] (print "cljs.js.get_fn(" (put-fn f) ")"))

(defmethod comp/emit-constant* js/Function [f] (emit-fn f))

(defmethod comp/emit-constant* cljs.core/Var [f] (emit-fn f))

(defn- eval-impl
  ([form] (eval-impl form (.-name *ns*)))
  ([form ns]
   (let [result (atom nil)]
     (let [st env/*compiler*]
       (eval st
             form
             {:ns ns, :context :expr, :def-emits-var true}
             (fn [{:keys [value error]}]
               (if error (throw error) (reset! result value)))))
     @result)))

(set! *eval* eval-impl)

(comment
  (require '[cljs.js :as cljs] '[cljs.analyzer :as ana])
  ...
  (cljs/analyze-str st
                    "(+ 1 1)"
                    nil
                    {:passes [ana/infer-type elide-env], :eval node-eval}
                    (fn [{:keys [value]}] (println value)))
  (cljs/eval st
             '(defn foo [a b] (+ a b))
             {:eval node-eval}
             (fn [res] (println res)))
  (cljs/compile-str st
                    ...
                    (fn [{:keys [value]}] (println "Source:") (println value)))
  (cljs/eval-str st
                 "(defn foo [a b] (+ a b))
     (defn bar [c d] (+ c d))"
                 nil
  ...
  (cljs/eval-str st
                 "1"
                 nil
                 {:eval node-eval, :context :expr}
                 (fn [res] (println res)))
  (cljs/eval-str st
                 "(def x 1)"
                 nil
                 {:eval node-eval, :context :expr, :def-emits-var true}
                 (fn [res] (println res)))
  (cljs/eval st '(ns foo.bar) {:eval node-eval} (fn [res] (println res)))
  (cljs/eval
    st
    '(def x 1)
    {:eval node-eval, :context :expr, :def-emits-var true, :ns 'foo.bar}
    (fn [res] (println res)))
  (cljs/compile-str st
                    "(defn foo\n[a b]\n(+ a b))"
                    'cljs.foo
                    {:verbose true, :source-map true}
                    (fn [js-source] (println "Source:") (println js-source)))
  (cljs/eval-str
    st
    "(ns foo.bar (:require [bootstrap-test.core]))\n(bootstrap-test.core/foo 3 4)"
    'foo.bar
    {:verbose true, :source-map true, :eval node-eval, :load node-load}
    (fn [ret] (println ret)))
  (cljs/eval-str
    st
    ...
    (fn [{:keys [error], :as res}]
      (if error
        (do (println "Error:" error) (println (.. error -cause -stack)))
        (println "Result:" res))))
  (cljs/compile-str
    st
    "(ns foo.bar (:require-macros [bootstrap-test.macros :refer [foo]]))\n(foo 4 4)"
  ...
  (cljs/eval-str
    ...
    (fn [{:keys [error], :as res}]
      (if error
        (do (println error) (println (.. error -cause -stack)))
        (println res))))
  (cljs/eval-str
    st
    "(ns foo.bar)\n(map inc [1 2 3])"
