...
(s/def ::seq-binding-form
  (s/and vector?
         (s/cat :forms (s/* ::binding-form)
                :rest-forms (s/? (s/cat :ampersand #{'&}
                                        :form ::binding-form))
                :as-form (s/? (s/cat :as #{:as}
                                     :as-sym ::local-name)))))
...
(s/def ::map-special-binding (s/keys :opt-un [::as ::or ::keys ::syms ::strs]))
...
(s/fdef core/defn :args ::defn-args :ret any?)
...
(s/def ::libspec
  (s/alt :lib ::lib
         :lib+opts
           (s/spec (s/cat :lib ::lib
                          :options (s/keys* :opt-un [::as ::refer ::refer-macros
                                                     ::include-macros])))))
...
(s/def ::package-list
  (s/spec (s/cat :package simple-symbol?
...
;; same as ::libspec-macros, but also supports the ::filters options in the
;; libspec
...
(s/fdef core/ns-special-form :args ::ns-form)
...
(s/fdef core/import :args ::quoted-import-list)
