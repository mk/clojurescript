...
(defmacro deftype
  ...
  (let [name-str (name type-name)
    ...
    `(do (defrecord ~type-name [~'type-tag ~@fields])
         (defn- ~(symbol (str "make-" name-str))
           ~(vec fields)
           (~(symbol (str type-name ".")) ~(keyword name-str) ~@fields))
         (defn- ~(symbol (str name-str "?"))
           [x#]
           (= (:type-tag x#) ~(keyword name-str))))))

(defn- parse-lb-options
  [opts body]
  (loop [body body
         acc []]
    (if (opts (first body))
...
(defmacro pprint-logical-block
  ...
  and :suffix."
  [& args]
  (let [[options body] (parse-lb-options #{:prefix :per-line-prefix :suffix}
                                         args)]
    `(do (if (cljs.pprint/level-exceeded)
           (~'-write cljs.core/*out* "#")
           (do (cljs.core/binding [cljs.pprint/*current-level*
                                     (inc cljs.pprint/*current-level*)
...
(defmacro print-length-loop
  ...
  (let [count-var (gensym "length-count")
    ...
    `(loop ~(apply vector count-var 0 bindings)
           (if (or (not cljs.core/*print-length*)
                   (< ~count-var cljs.core/*print-length*))
             (do ~@mod-body)
             (~'-write cljs.core/*out* "...")))))
...
(defmacro ^{:private true} defdirectives
  [& directives]
  `(def ^{:private true} ~'directive-table
...
format-in can be either a control string or a previously compiled format."
  ...
  `(let [format-in# ~format-in
     ...
     (fn [stream# & args#]
       (let [navigator# (my-i-n# args#)] (my-e-f# stream# cf# navigator#)))))
...
format-in can be either a control string or a previously compiled format."
  ...
  `(let [format-in# ~format-in
         cf# (if (string? format-in#)
               (cljs.pprint/cached-compile format-in#)
               format-in#)]
     (fn [& args#]
...
(defmacro with-pprint-dispatch
  ...
  [function & body]
  `(cljs.core/binding [cljs.pprint/*print-pprint-dispatch* ~function] ~@body))
