...
(defn seg-combine
  ...
  (let [[gcol source line col name] seg
        ...
        nseg [(+ gcol rgcol) (+ (or source 0) rsource) (+ (or line 0) rline)
              (+ (or col 0) rcol) (+ (or name 0) rname)]]
    ...
    (update-in
      ...
      (fnil (fn [m]
              (update-in
                m
                [line]
                (fnil (fn [m] (update-in m [col] (fnil (fn [v] (conj v d)) [])))
                      (sorted-map))))
            (sorted-map)))))
...
(defn decode-reverse
  ...
  JavaScript."
  ([source-map] (decode-reverse (gobj/get source-map "mappings") source-map))
  ([mappings source-map]
                 ...
                 (if (string/blank? line)
                   ...
                   (let [segs (seq (string/split line #","))]
                     (loop [segs segs
                       ...
                       (if segs
                         (let [seg (first segs)
                               nrelseg (seg-combine (base64-vlq/decode seg)
                                                    relseg)]
                           (recur (next segs)
                                  nrelseg
                                  (update-reverse-result result
                                                         (seg->map nrelseg
                                                                   source-map)
                                                         gline)))
                         [result relseg]))))]
...
(defn update-result
  ...
  (let [{:keys [gcol source line col name]} segmap
    ...
    (update-in result
               [gline]
               (fnil (fn [m] (update-in m [gcol] (fnil #(conj % d) [])))
                     (sorted-map)))))
...
(defn lines->segs
  ...
  (let [relseg (atom [0 0 0 0 0])]
    (reduce
      (fn [segs cols]
        (swap! relseg (fn [[_ source line col name]] [0 source line col name]))
        (conj segs
              (reduce (fn [cols [gcol sidx line col name :as seg]]
                        (let [offset (map - seg @relseg)]
                          (swap! relseg (fn [[_ _ _ _ lname]]
  ...
  (let [lines (atom [[]])
    ...
    (doseq [[source-idx [_ lines]] (map-indexed (fn [i v] [i v]) m)]
      (doseq [[line cols] lines]
        (doseq [[col infos] cols] (encode-cols infos source-idx line col))))
              ...
              #js
               {"version" 3,
                ...
                "sources" (let [paths (keys m)
                                f (comp (if (true? (:source-map-timestamp opts))
                ...
                "mappings" (->> (lines->segs (concat preamble-lines @lines))
                                (map #(string/join "," %))
                                (string/join ";")),
                "names" (into-array (map (set/map-invert @names->idx)
                                      (range (count @names->idx))))}
              (:sources-content opts)
                (doto (gobj/set "sourcesContent"
...
(defn merge-source-maps
  ...
  [cljs-map js-map]
  (loop [line-map-seq (seq cljs-map)
         new-lines (sorted-map)]
    (if line-map-seq
      (let [[line col-map] (first line-map-seq)
            new-cols
              (loop [col-map-seq (seq col-map)
                     new-cols (sorted-map)]
                (if col-map-seq
                  (let [[col infos] (first col-map-seq)]
                    (recur
                      (next col-map-seq)
                      (assoc new-cols
                        col (reduce (fn [v {:keys [gline gcol]}]
...
(comment (invert-reverse-map
           {1 {1 [{:gcol 0, :gline 0, :name "cljs.core/map"}],
