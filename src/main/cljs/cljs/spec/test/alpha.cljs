...
(ns cljs.spec.test.alpha
  (:require-macros [cljs.spec.test.alpha :as m :refer
                    [with-instrument-disabled setup-static-dispatches]])
...
(defn get-ua-product
  ...
  (if (not= "browser" *target*)
    (keyword *target*)
    (cond product/SAFARI :safari
          product/CHROME :chrome
          product/FIREFOX :firefox
          product/IE :ie)))

(defn get-env [] {:ua-product (get-ua-product)})
...
(defn- explain-data*
  [spec v]
  (when-not (s/valid? spec v nil) (s/explain-data spec v)))

(defn- find-caller
  [st]
  (letfn [(search-spec-fn
            [frame]
            (when frame
            ...
            (let [conformed (s/conform spec data)]
              (if (= ::s/invalid conformed)
                (let [caller (find-caller
                               (st/parse-stacktrace (get-host-port)
                                                    (.-stack (js/Error.))
                                                    (get-env)
                                                    nil))
        ...
        ret (fn [& args]
              (if *instrument-enabled*
                (with-instrument-disabled
                  ...
                  (binding [*instrument-enabled* true] (apply' f args)))
                (apply' f args)))]
...
(defn- unstrument-1*
  ...
  (when v
    (when-let [{:keys [raw wrapped]} (get @instrumented-vars v)]
      ...
      (let [current @v] (when (= wrapped current) raw)))))
...
that can be instrumented."
  ...
  ([opts]
   (assert (every? ident? (keys (:gen opts)))
           "instrument :gen expects ident keys")
   (reduce into
     #{}
     [(filter fn-spec-name? (keys (s/registry))) (keys (:spec opts))
      (:stub opts) (keys (:replace opts))])))
...
(defn- make-check-result
  ...
  (merge {:spec spec, ::stc/ret test-check-ret}
         (when check-sym {:sym check-sym})
         (when-let [result (-> test-check-ret
                               :result)]
           (when-not (true? result) {:failure result}))
         (when-let [shrunk (-> test-check-ret
                               :shrunk)]
...
(comment (require '[cljs.pprint :as pp]
         ...
         (m/unstrument `[ranged-rand])
         (ranged-rand 8 5)
         (defn foo ([a]) ([a b] (ranged-rand 8 5)))
         (foo 1 2)
         (m/unstrument-1 `ranged-rand)
         (m/check-1 `ranged-rand nil nil {})
         (m/check-fn inc
                     (s/fspec :args (s/cat :x
                                           int?)
                              :ret int?))
         (m/checkable-syms)
         (m/check `ranged-rand))
