...
(ns ^{:doc "Set operations such as union/intersection.", :author "Rich Hickey"}
    clojure.set)
...
(defn union
  ...
  ([s1] s1)
  ([s1 s2]
   (if (< (count s1) (count s2)) (reduce conj s2 s1) (reduce conj s1 s2)))
  ([s1 s2 & sets]
...
(comment (refer 'set)
         (def xs
           #{{:a 11, :b 1, :c 1, :d 4} {:a 2, :b 12, :c 2, :d 6}
             {:a 3, :b 3, :c 3, :d 8, :f 42}})
         (def ys
           #{{:a 11, :b 11, :c 11, :e 5} {:a 12, :b 11, :c 12, :e 3}
             {:a 3, :b 3, :c 3, :e 7}})
         (join xs ys)
