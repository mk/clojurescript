...
(defn append [parent & children] (apply gdom/append parent children) parent)
...
(extend-protocol DOMBuilder
    ...
    (-element
      ...
      ([this attrs children]
       ...
       (let [str-attrs (if (and (map? attrs) (seq attrs))
         ...
         (log-obj str-attrs)
         (if (seq children)
           (apply gdom/createDom (name this) str-attrs (map -element children))
           (gdom/createDom (name this) str-attrs)))))
  PersistentVector
    (-element [this]
  ...
  js/Element
    (-element [this] (log "js/Element (-element " this ")") this))

(defn element
  ([tag-or-text] (log "(element " tag-or-text ")") (-element tag-or-text))
  ([tag & children]
...
(defn remove-children
  ...
  [id]
  (let [parent (gdom/getElement (name id))] (do (gdom/removeChildren parent))))

(defn get-element [id] (gdom/getElement (name id)))

(defn html->dom [s] (gdom/htmlToDocumentFragment s))

(defn insert-at [parent child index] (gdom/insertChildAt parent child index))
...
(defn get-value "Get the value of an element." [e] (.-value (ensure-element e)))
...
(defn click-element [e] (.click (ensure-element e) ()))
