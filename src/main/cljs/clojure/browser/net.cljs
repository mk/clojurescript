...
(def event-types
  (into {}
        (map (fn [[k v]] [(keyword (.toLowerCase k)) v])
          (merge (js->clj EventType)))))
...
(extend-type XhrIo
  IConnection
    (transmit ([this uri] (transmit this uri "GET" nil nil *timeout*))
              ([this uri method] (transmit this uri method nil nil *timeout*))
...
;; TODO jQuery/sinatra/RestClient style API: (get [uri]), (post [uri payload]),
;; (put [uri payload]), (delete [uri])

(def xpc-config-fields
  (into {}
        (map (fn [[k v]] [(keyword (.toLowerCase k)) v]) (js->clj CfgFields))))

(defn xhr-connection "Returns an XhrIo connection" [] (XhrIo.))
...
(extend-type CrossPageChannel
  ICrossPageChannel
    (register-service
      ([this service-name fn] (register-service this service-name fn false))
      ([this service-name fn encode-json?]
       (.registerService this (name service-name) fn encode-json?)))
  IConnection
    (connect
      ([this] (connect this nil))
      ([this on-connect-fn] (.connect this on-connect-fn))
...
(defn xpc-connection
  ...
  ([]
   (when-let [config (.getParameterValue (Uri. (.-href (.-location js/window)))
                                         "xpc")]
     (CrossPageChannel. (gjson/parse config))))
  ([config]
   (CrossPageChannel. (reduce (fn [sum [k v]]
...
(extend-type WebSocket
  ...
  IConnection
    (connect ([this url] (connect this url nil))
             ([this url protocol] (.open this url protocol)))
    (transmit [this message] (.send this message))
    (close [this] (.close this ()))
  event/IEventType
    (event-types [this]
      (into {}
            (map (fn [[k v]] [(keyword (. k (toLowerCase))) v])
              (merge (js->clj WebSocket.EventType))))))

(defn websocket-connection
  ([] (websocket-connection nil nil))
  ([auto-reconnect?] (websocket-connection auto-reconnect? nil))
