...
(defn flush-print-queue!
  ...
  (doseq [str print-queue]
    (net/transmit conn :print (json/serialize #js {"repl" *repl*, "str" str})))
  (garray/clear print-queue))

(defn repl-print
  [data]
  (.push print-queue (pr-str data))
  (when @parent-connected? (flush-print-queue! @xpc-connection)))
...
(defn send-print
  ...
  up to 10 times."
  ([url data] (send-print url data 0))
  ([url data n]
     ...
     (event/listen
       ...
       (fn [_]
         (if (< n 10)
           ...
           (.log js/console
                 (str "Could not send " data " after " n " attempts.")))))
...
(defn wrap-message
  [repl t data]
  (pr-str {:repl repl, :type t, :content data, :order (swap! order inc)}))
    ...
    (let [connection (net/xhr-connection)
          ...
          try-handshake (fn try-handshake []
                          (when-not @repl-connected?
                            ...
                            ;; ack once.
                            (js/setTimeout try-handshake 10)))]
      ...
      (net/register-service
        ...
        (fn [_]
          (when-not @repl-connected?
            ...
            ;; the server.
            (send-result connection url (wrap-message nil :ready "ready")))))
      ...
      (net/register-service
        ...
        (fn [json]
          (let [obj (json/parse json)
            ...
            (send-result connection url (wrap-message repl :result result)))))
      (net/register-service
        repl-connection
        :print
        (fn [json]
    ...
    (set! (.-writeScriptTag_ js/goog)
          (fn [src opt_sourceText]
            (if load-queue
              ...
              (do (set! load-queue #js [])
                  (js/goog.writeScriptTag__ src opt_sourceText)))))
    ;; In the latest Closure library implementation, there is no
    ;; goog.writeScriptTag_,
    ;; to monkey-patch. The behavior of interest is instead in
    ;; goog.Dependency.prototype.load,
    ;; which first checks and uses CLOSURE_IMPORT_SCRIPT if defined. So we hook
    ;; our desired
    ;; behavior here.
    (when goog/debugLoader_
      (set! js/CLOSURE_IMPORT_SCRIPT (.-writeScriptTag_ js/goog)))
    ;; we must reuse Closure library dev time dependency management, under
    ;; namespace
    ;; reload scenarios we simply delete entries from the correct private
    ;; locations
    (set!
      (.-require js/goog)
      (fn [src reload]
        (when (= reload "reload-all") (set! (.-cljsReloadAll_ js/goog) true))
