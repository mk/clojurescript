...
(defn node "Returns the node at loc" [loc] (loc 0))
...
(defn path "Returns a seq of nodes leading to this loc" [loc] (:pnodes (loc 1)))
...
(comment (load-file "/Users/rich/dev/clojure/src/zip.clj")
         (refer 'zip)
         (def data '[[a * b] + [c * d]])
         ...
         (loop [loc dz]
           (if (end? loc)
             (root loc)
             (recur (next (if (= '* (node loc)) (replace loc '/) loc)))))
         (loop [loc dz]
           (if (end? loc)
             (root loc)
             (recur (next (if (= '* (node loc)) (remove loc) loc))))))
